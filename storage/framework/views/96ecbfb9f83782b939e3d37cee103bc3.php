        ...
        get previewText() {
            if (this.sendMode === 'meta_template') {
                ...
                for (let i = 1; i <= count; i++) {
                    const val = (this.templateVariables[i] || this.templateVariables[String(i)] || '').trim() || '<?php echo e(' + i + '); ?>';
    ...
    <?php if($canSend && !empty($connectionMeta['send_warnings'])): ?>
        <div class="rounded-2xl border-2 border-sky-200 bg-sky-50 px-5 py-4 text-sm text-sky-950 space-y-2">
            ...
            <ul class="list-disc list-inside space-y-1 text-sky-900/90">
                ...
                <li>للتسويق أو أول تواصل: اختر <strong>قالب Meta معتمد</strong> من نفس الصفحة، أو أنشئ قالباً من <a href="<?php echo e(route('admin.whatsapp.templates.index')); ?>" class="underline font-semibold">قوالب Meta</a>.</li>
...
unset($__errorArgs, $__bag); ?>
            ...
            <section class="<?php echo e($waSectionClass); ?>">
                ...
                <div class="p-5 sm:p-6 space-y-5">
                    <div>
                        <label class="<?php echo e($waLabelClass); ?>">نوع الإرسال</label>
                    ...
                    <div x-show="sendMode === 'text'" x-cloak>
                        <label class="<?php echo e($waLabelClass); ?>">قالب نصي محلي (اختياري)</label>
                        <select name="template_id" x-model="templateId" @change="onLocalTemplateChange()" class="<?php echo e($waSelectClass); ?>">
                        ...
                        <p class="text-xs text-slate-500 mt-1.5">
                            ...
                            · <a href="<?php echo e(route('admin.messages.templates')); ?>" class="text-emerald-600 hover:underline">إدارة القوالب النصية</a>
                        </p>
                    </div>

                    <div x-show="sendMode === 'meta_template'" x-cloak class="space-y-3">
                        <div>
                            <label class="<?php echo e($waLabelClass); ?>">قالب Meta المعتمد</label>
                            <?php if(count($metaTemplates ?? []) > 0): ?>
                                <select x-model="metaTemplateKey" @change="onMetaTemplateChange()" class="<?php echo e($waSelectClass); ?> dir-ltr text-right">
                                    <option value="">— اختر قالباً —</option>
                                    <?php $__currentLoopData = $metaTemplates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($mt['key']); ?>"><?php echo e($mt['label']); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                                <p class="text-xs text-slate-500 mt-1.5">
                                    يُرسل عبر WhatsApp Cloud API كقالب رسمي — لا يحتاج نافذة 24 ساعة
                                    · <a href="<?php echo e(route('admin.whatsapp.templates.index')); ?>" class="text-sky-600 hover:underline">إدارة قوالب Meta</a>
                                </p>
                            <?php else: ?>
                                <div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-900">
                                    لا توجد قوالب Meta معتمدة.
                                    <?php if(!empty($metaTemplatesError)): ?>
                                        <span class="block mt-1"><?php echo e($metaTemplatesError); ?></span>
                                    <?php endif; ?>
                                    <a href="<?php echo e(route('admin.whatsapp.templates.index')); ?>" class="font-bold underline mt-1 inline-block">أنشئ واعتمد قالباً</a>
                                </div>
                            <?php endif; ?>
                        </div>
                                ...
                                <template x-for="i in metaVariableIndexes()" :key="'mv-' + i">
                                    <div>
                                        ...
                                        <input type="text" :name="'template_variables[' + i + ']'" x-model="templateVariables[i]"
                                               class="<?php echo e($waInputClass); ?> text-sm mt-0.5" :placeholder="'قيمة المتغير ' + i">
                                    </div>
                                </template>
                            </div>
                        </template>
...
unset($__errorArgs, $__bag); ?>
                        <?php $__errorArgs = ['meta_template_name'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?><p class="text-rose-600 text-xs mt-1"><?php echo e($message); ?></p><?php unset($message);
