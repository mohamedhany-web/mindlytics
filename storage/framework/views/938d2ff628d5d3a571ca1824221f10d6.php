...
<div
    ...
    x-cloak
    class="fixed z-[60] bottom-5 <?php echo e(app()->getLocale() === 'ar' ? 'left-5' : 'right-5'); ?> font-sans stc-widget"
...
function salesTeamChat() {
    ...
    const routes = {
        bootstrap: <?php echo json_encode(route('employee.team-chat.bootstrap'), 15, 512) ?>,
        conversations: <?php echo json_encode(route('employee.team-chat.conversations'), 15, 512) ?>,
        unread: <?php echo json_encode(route('employee.team-chat.unread'), 15, 512) ?>,
        openTeam: <?php echo json_encode(route('employee.team-chat.conversations.team'), 15, 512) ?>,
        openDirect: <?php echo json_encode(route('employee.team-chat.conversations.direct'), 15, 512) ?>,
        messages: (id) => <?php echo json_encode(url('/employee/team-chat/conversations'), 15, 512) ?>.replace(/\/$/, '') + '/' + id + '/messages',
        send: (id) => <?php echo json_encode(url('/employee/team-chat/conversations'), 15, 512) ?>.replace(/\/$/, '') + '/' + id + '/messages',
        read: (id) => <?php echo json_encode(url('/employee/team-chat/conversations'), 15, 512) ?>.replace(/\/$/, '') + '/' + id + '/read',
        react: (id) => <?php echo json_encode(url('/employee/team-chat/messages'), 15, 512) ?>.replace(/\/$/, '') + '/' + id + '/reactions',
    };
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\mindly tics\Mindlytics\resources\views\components\sales-team-chat-widget.blade.php ENDPATH**/ ?>