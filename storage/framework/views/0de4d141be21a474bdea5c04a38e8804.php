        ...
        <div class="grid grid-cols-1 xl:grid-cols-3">
                ...
                <form method="GET" action="<?php echo e(route('admin.payments.index')); ?>" id="filterForm" class="space-y-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="search" value="<?php echo e(htmlspecialchars(request('search') ?? '')); ?>" maxlength="255" placeholder="رقم الدفعة، اسم العميل، أو رقم الهاتف" oninput="sanitizeInput(this)" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 pr-10 text-sm text-slate-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" />
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-2 flex items-center gap-2">
            ...
            <div class="xl:col-span-2">
                <div class="border-b border-slate-200 px-6 py-5 flex items-center justify-between">
                    ...
                    <span class="text-xs font-semibold text-blue-600 bg-blue-50 px-3 py-1 rounded-lg"><?php echo e($payments->total()); ?> دفعة</span>
                </div>
                <div class="p-6">
                    <div class="space-y-3">
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="flex-1 space-y-2">
                                        <div class="flex items-start justify-between gap-4">
                                            <div>
                                                <p class="text-sm font-bold text-slate-900"><?php echo e(htmlspecialchars($payment->payment_number)); ?></p>
                                                <p class="text-xs text-slate-600 mt-0.5"><?php echo e(htmlspecialchars($payment->user->name ?? 'غير معروف')); ?> - <?php echo e(htmlspecialchars($payment->user->phone ?? '-')); ?></p>
                                                <?php if($payment->reference_number): ?>
                                                    <p class="text-xs text-slate-500 mt-1">مرجع: <?php echo e(htmlspecialchars($payment->reference_number)); ?></p>
                                                <?php endif; ?>
                                            </div>
                                            <?php $badge = $statusBadges[$payment->status] ?? null; ?>
                                            <?php if($badge): ?>
                                        ...
                                        <div class="flex items-center justify-between gap-4 pt-2 border-t border-slate-100">
                                            <div class="flex items-center gap-4 text-xs text-slate-600">
                                                <span><i class="fas fa-coins text-emerald-600 ml-1"></i> <?php echo e(number_format($payment->amount, 2)); ?> ج.م</span>
                                                <span><i class="fas fa-credit-card text-blue-600 ml-1"></i> <?php echo e(htmlspecialchars($paymentMethodLabels[$payment->payment_method] ?? $payment->payment_method)); ?></span>
                                                <span><i class="fas fa-calendar text-slate-500 ml-1"></i> <?php echo e($payment->paid_at ? $payment->paid_at->format('Y-m-d') : $payment->created_at->format('Y-m-d')); ?></span>
                                            </div>
                                            <a href="<?php echo e(route('admin.payments.show', $payment)); ?>" class="inline-flex items-center gap-1 rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-600 hover:bg-blue-100 transition-colors duration-200">
                        ...
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                            <div class="rounded-xl border border-slate-200 bg-white p-12 text-center">
                                <div class="flex flex-col items-center gap-4">
                                    ...
                                    <a href="<?php echo e(route('admin.payments.create')); ?>" class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-blue-500 px-4 py-2 text-sm font-semibold text-white shadow hover:from-blue-700 hover:to-blue-600 transition-all duration-200">
                                        <i class="fas fa-plus"></i>
                                        إضافة دفعة جديدة
                                    </a>
                                </div>
