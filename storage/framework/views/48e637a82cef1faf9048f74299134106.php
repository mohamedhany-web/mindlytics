...
<div class="w-full max-w-7xl mx-auto space-y-8 pb-12">
            ...
            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                <div>
                    ...
                    <p class="mt-3 text-sm text-white/75 leading-relaxed max-w-3xl">
                        ...
                        العملة المرجعية: <strong class="text-white"><?php echo e($currency); ?></strong>.
                    </p>
                </div>
    ...
    <section class="rounded-3xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 bg-gradient-to-l from-slate-50 to-white flex flex-wrap items-center justify-between gap-3">
            ...
            <span class="text-xs font-semibold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-full"><?php echo e(count($roots)); ?> جذور رئيسية</span>
        </div>
            ...
            <?php $__currentLoopData = $roots; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $root): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                    <div class="flex flex-wrap items-center gap-3 px-4 py-3.5 border-b border-slate-100 bg-gradient-to-l from-indigo-50/60 to-white">
                        <span class="font-mono text-sm font-black text-indigo-700 bg-indigo-100 px-2.5 py-1 rounded-lg"><?php echo e($root['code'] ?? ''); ?></span>
                        <span class="text-base font-black text-slate-900"><?php echo e($root['name'] ?? ''); ?></span>
                    </div>
                    <?php if(!empty($root['description'])): ?>
                        <p class="px-4 py-2.5 text-xs text-slate-600 border-b border-slate-50 bg-slate-50/30"><?php echo e($root['description']); ?></p>
                    <?php endif; ?>
                    <div class="px-3 py-4 sm:px-5">
                        <?php echo $__env->make('admin.accounting.partials.chart-node', ['nodes' => $root['children'] ?? [], 'depth' => 0], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                    </div>
                </div>
    ...
    <p class="text-center text-xs text-slate-500 px-4">
        ...
        <a href="<?php echo e(route('admin.accounting.reports')); ?>" class="font-semibold text-sky-600 hover:underline">التقارير المحاسبية</a>
        والتصدير إلى Excel.
    </p>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\mindly tics\Mindlytics\resources\views/admin/accounting/chart.blade.php ENDPATH**/ ?>