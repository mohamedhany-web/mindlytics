                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                    ...
                    <div class="lg:col-span-2">
                        <div class="course-card p-6 md:p-8 fade-in-up" style="animation-delay: 0.3s;">
                            ...
                            <form action="<?php echo e(isset($course) ? route('public.course.checkout.complete', $course->id) : route('public.learning-path.checkout.complete', Str::slug($learningPath->name))); ?>" method="POST" enctype="multipart/form-data" x-data="{ paymentMethod: '', walletId: '', isSubmitting: false }" @submit="isSubmitting = true">
                                <?php echo csrf_field(); ?>
...
unset($__errorArgs, $__bag); ?>
                                    ...
                                    <div x-show="walletId" x-cloak class="mt-4 p-5 bg-gradient-to-br from-blue-50 to-green-50 rounded-xl border-2 border-blue-200 fade-in-up">
                                        ...
                                        <template x-if="walletId">
                                            <?php $__currentLoopData = $wallets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $wallet): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <div x-show="walletId == '<?php echo e($wallet->id); ?>'" class="space-y-3 text-sm">
                                                    <div class="flex justify-between items-center bg-white/60 px-3 py-2 rounded-lg">
                                                        <span class="text-gray-600">النوع:</span>
                                                        <span class="font-bold text-gray-900"><?php echo e(\App\Models\Wallet::typeLabel($wallet->type)); ?></span>
                                                    </div>
                                                    <?php if($wallet->account_number): ?>
                                                    <div class="flex justify-between items-center bg-white/60 px-3 py-2 rounded-lg">
                                                        <span class="text-gray-600">رقم الحساب:</span>
                                                        <span class="font-bold text-gray-900 font-mono"><?php echo e($wallet->account_number); ?></span>
                                                    </div>
                                                    <?php endif; ?>
                                                    <?php if($wallet->bank_name): ?>
                                                    <div class="flex justify-between items-center bg-white/60 px-3 py-2 rounded-lg">
                                                        <span class="text-gray-600">اسم البنك:</span>
                                                        <span class="font-bold text-gray-900"><?php echo e($wallet->bank_name); ?></span>
                                                    </div>
                                                    <?php endif; ?>
                                                    <?php if($wallet->account_holder): ?>
                                                    <div class="flex justify-between items-center bg-white/60 px-3 py-2 rounded-lg">
                                                        <span class="text-gray-600">صاحب الحساب:</span>
                                                        <span class="font-bold text-gray-900"><?php echo e($wallet->account_holder); ?></span>
                                                    </div>
                                                    <?php endif; ?>
                                                    <?php if($wallet->notes): ?>
                                                    <div class="mt-3 pt-3 border-t border-blue-200 bg-white/60 px-3 py-2 rounded-lg">
                                                        ...
                                                        <span class="text-sm text-gray-700"><?php echo e($wallet->notes); ?></span>
                                                    </div>
                                                    <?php endif; ?>
                                                </div>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
