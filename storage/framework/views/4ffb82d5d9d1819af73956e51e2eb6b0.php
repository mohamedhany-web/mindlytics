    ...
    <section class="rounded-3xl bg-white/95 backdrop-blur border border-slate-200 shadow-lg overflow-hidden">
        ...
        <div class="px-5 py-6 sm:px-8 lg:px-12 space-y-6">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white/85 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        ...
                        <?php if($payment->gateway_fee_amount && (float) $payment->gateway_fee_amount > 0): ?>
                        <div class="flex items-center justify-between py-2 border-b border-slate-100">
                            <span class="text-sm text-slate-500">عمولة بوابة الدفع</span>
                            <span class="text-lg font-bold text-amber-700"><?php echo e(number_format($payment->gateway_fee_amount, 2)); ?> ج.م</span>
                        </div>
                        <div class="flex items-center justify-between py-2 border-b border-slate-100">
                            <span class="text-sm text-slate-500">صافي بعد العمولة (تقديري)</span>
                            <span class="text-lg font-bold text-violet-800"><?php echo e(number_format((float) $payment->amount - (float) $payment->gateway_fee_amount, 2)); ?> ج.م</span>
                        </div>
                        <?php if($payment->gateway_fee_detail): ?>
                        <div class="py-2 border-b border-slate-100 text-xs text-slate-500">
                            تفاصيل احتساب العمولة: <code class="bg-slate-100 px-1 rounded"><?php echo e(json_encode($payment->gateway_fee_detail, JSON_UNESCAPED_UNICODE)); ?></code>
                        </div>
                        <?php endif; ?>
                        <?php endif; ?>
                        ...
                        <div class="flex items-center justify-between py-2 border-b border-slate-100">
                            ...
                            <a href="<?php echo e(route('admin.orders.show', $payment->order_id)); ?>" class="text-sm font-semibold text-sky-700 hover:underline">طلب #<?php echo e($payment->order_id); ?></a>
                        </div>
                        <?php endif; ?>
                        <div class="flex items-center justify-between py-2 border-b border-slate-100">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between py-2 border-b border-slate-100">
                            ...
                            <span class="text-sm font-semibold text-slate-900"><?php echo e(\App\Models\Payment::gatewayLabel($payment->payment_gateway)); ?></span>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="rounded-2xl border border-amber-200 bg-amber-50/80 p-6">
                ...
                <a href="<?php echo e(asset('storage/' . $payment->proof_path)); ?>" target="_blank" rel="noopener" class="inline-flex items-center gap-2 text-sm font-semibold text-amber-800 hover:underline">
                    <i class="fas fa-external-link-alt"></i>
                    فتح الصورة في نافذة جديدة
                </a>
                <div class="mt-4 rounded-xl border border-amber-200/80 bg-white p-2 max-w-md">
                    <img src="<?php echo e(asset('storage/' . $payment->proof_path)); ?>" alt="إيصال" class="max-h-64 w-full object-contain rounded-lg">
                </div>
            </div>
            <?php endif; ?>
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div class="flex justify-between gap-6">
                        ...
                        <dd class="font-semibold text-slate-900">
                            <?php if($payment->user->role == 'student'): ?> طالب
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/admin/payments/show.blade.php ENDPATH**/ ?>