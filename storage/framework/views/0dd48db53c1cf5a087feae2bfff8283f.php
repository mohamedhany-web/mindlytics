...
<div class="space-y-6">
    ...
    <form method="get" class="flex flex-wrap gap-3 items-end bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
        <div>
            ...
            <select name="category_id" class="border rounded-lg px-3 py-2 text-sm min-w-[140px]">
        ...
        <div>
            <label class="block text-xs text-gray-500 mb-1">دفعة استيراد</label>
            <select name="import_batch" class="border rounded-lg px-3 py-2 text-sm min-w-[160px]">
                <option value="">الكل</option>
                <?php $__currentLoopData = $importBatches; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $batch): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($batch); ?>" <?php if(request('import_batch') === $batch): echo 'selected'; endif; ?>><?php echo e($batch); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
        </div>
        <?php endif; ?>
        ...
        <?php if(request()->hasAny(['stage','priority','follow_up','sort','stale','search','category_id','import_batch'])): ?>
        ...
        <table class="w-full min-w-[980px] text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="text-right py-3 px-4 font-semibold text-gray-700">التصنيف</th>
                ...
                <tr class="<?php echo e($row); ?>">
                    ...
                    <td class="py-3 px-4">
                        <?php if($lead->category): ?>
                            <span class="inline-flex px-2 py-0.5 rounded-lg text-xs font-semibold" style="color: <?php echo e($lead->category->color); ?>; background: <?php echo e($lead->category->color); ?>18"><?php echo e($lead->category->name); ?></span>
                        <?php else: ?>
