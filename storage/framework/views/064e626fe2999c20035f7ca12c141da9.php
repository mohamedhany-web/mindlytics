...
<?php $__env->startSection('title', 'مواعيد العمل'); ?>
<?php $__env->startSection('header', 'مواعيد العمل'); ?>
...
<div class="space-y-6">
            ...
            <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="flex items-end gap-2">
                    ...
                    <?php if(request()->hasAny(['search', 'status'])): ?>
                        <a href="<?php echo e(route('admin.work-schedules.index')); ?>" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg font-medium transition-colors">
            ...
            <div class="relative z-10">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="text-3xl font-black text-indigo-700"><?php echo e($stats['assigned_employees']); ?></p>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 border border-gray-200">
                <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-sky-50 to-indigo-100/50">
                    <div class="flex items-center justify-between gap-2">
                        <h3 class="text-lg font-semibold text-gray-900 truncate"><?php echo e($schedule->name); ?></h3>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium shrink-0 <?php echo e($schedule->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'); ?>">
                            <?php echo e($schedule->is_active ? 'نشط' : 'موقوف'); ?>
                ...
                <div class="px-6 py-4">
                    <div class="space-y-2">
                        <div class="flex items-center text-sm">
                            ...
                            <span class="text-gray-900 mr-2 font-medium tabular-nums"><?php echo e($schedule->timeRangeLabel()); ?></span>
                        </div>

                        <div class="flex items-center text-sm">
                            <i class="fas fa-hourglass-half text-gray-400 w-4 ml-2"></i>
                            <span class="text-gray-600">الساعات المطلوبة:</span>
                            <span class="text-gray-900 mr-2 font-medium"><?php echo e($schedule->required_hours); ?> ساعة</span>
                        </div>
                        ...
                        <div class="flex items-center text-sm">
                            ...
                            <span class="text-gray-900 mr-2 font-medium"><?php echo e($schedule->employees_count); ?> موظف</span>
                        </div>

                        <?php if($schedule->grace_minutes): ?>
                        <div class="flex items-center text-sm">
                            <i class="fas fa-stopwatch text-gray-400 w-4 ml-2"></i>
                            <span class="text-gray-600">سماح التأخير:</span>
                            <span class="text-gray-900 mr-2 font-medium"><?php echo e($schedule->grace_minutes); ?> دقيقة</span>
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="flex items-start text-sm pt-1">
                            ...
                            <span class="text-gray-600 line-clamp-2"><?php echo e($schedule->description); ?></span>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-end gap-2">
                    <a href="<?php echo e(route('admin.work-schedules.edit', $schedule)); ?>"
                       class="px-3 py-1.5 bg-yellow-600 hover:bg-yellow-700 text-white text-sm rounded-lg font-medium transition-colors">
                        <i class="fas fa-edit mr-1"></i>تعديل
                    </a>
                    <form action="<?php echo e(route('admin.work-schedules.destroy', $schedule)); ?>" method="POST"
                          onsubmit="return confirm('هل أنت متأكد من حذف موعد العمل؟');">
                        <?php echo csrf_field(); ?>
                        <?php echo method_field('DELETE'); ?>
                        <button type="submit"
                                class="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-sm rounded-lg font-medium transition-colors <?php echo e($schedule->employees_count > 0 ? 'opacity-50 cursor-not-allowed' : ''); ?>"
                                <?php if($schedule->employees_count > 0): echo 'disabled'; endif; ?>
                                title="<?php echo e($schedule->employees_count > 0 ? 'لا يمكن الحذف — مرتبط بموظفين' : 'حذف'); ?>">
    ...
    <?php else: ?>
        <div class="bg-white rounded-xl shadow-lg p-12 text-center border border-gray-200">
            ...
            <a href="<?php echo e(route('admin.work-schedules.create')); ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\mindly tics\Mindlytics\resources\views/admin/work-schedules/index.blade.php ENDPATH**/ ?>