...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
    ...
    <div>
        <label class="<?php echo e($hrLabelClass ?? 'block text-xs font-semibold text-slate-700 mb-1.5'); ?>">حالة الوظيفة *</label>
        <select name="status" required class="<?php echo e($hrSelectClass ?? 'w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm'); ?>">
            <?php $__currentLoopData = \App\Models\HrJobPosting::STATUSES; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($k); ?>" <?php if(old('status', $job->status ?? 'open') === $k): echo 'selected'; endif; ?>><?php echo e($label); ?></option>
    ...
    <div class="md:col-span-2 rounded-xl border-2 border-pink-100 bg-pink-50/40 p-4 space-y-4">
        ...
        <div>
            <label class="<?php echo e($hrLabelClass ?? 'block text-xs font-semibold text-slate-700 mb-1.5'); ?>">المهارات المطلوبة</label>
            <textarea name="required_skills" rows="2" placeholder="Excel, SQL, Power BI, Python — افصل بفاصلة"
                      class="<?php echo e($hrTextareaClass ?? 'w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm resize-y'); ?>"><?php echo e($skillsValue); ?></textarea>
            <p class="text-[11px] text-slate-500 mt-1">تُستخدم لمطابقة مهارات السيرة — وزنها 60% من النتيجة.</p>
        </div>
        <div class="grid sm:grid-cols-2 gap-4">
            <div>
                <label class="<?php echo e($hrLabelClass ?? 'block text-xs font-semibold text-slate-700 mb-1.5'); ?>">سنوات الخبرة المطلوبة</label>
                <input type="number" name="required_experience" min="0" max="50" step="1"
                       value="<?php echo e(old('required_experience', $job->required_experience ?? '')); ?>"
            ...
            <div>
                <label class="<?php echo e($hrLabelClass ?? 'block text-xs font-semibold text-slate-700 mb-1.5'); ?>">المؤهل الدراسي المطلوب</label>
                <select name="required_education" class="<?php echo e($hrSelectClass ?? 'w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm'); ?>">
                    ...
                    <?php $__currentLoopData = $educationLevels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $meta): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($k); ?>" <?php if(old('required_education', $job->required_education ?? '') === $k): echo 'selected'; endif; ?>><?php echo e($meta['label'] ?? $k); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
...
<?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/admin/hr/jobs/_form.blade.php ENDPATH**/ ?>