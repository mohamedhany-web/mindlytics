...
<div class="space-y-6" x-data="{
    ...
    category: <?php echo \Illuminate\Support\Js::from(old('category', $template?->category ?? 'UTILITY'))->toHtml() ?>,
    language: <?php echo \Illuminate\Support\Js::from(old('language', $template?->language ?? 'ar'))->toHtml() ?>,
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="rounded-lg border border-violet-100 bg-white/80 p-3 text-xs text-slate-700 space-y-1">
                    ...
                    <?php $__currentLoopData = $workshopVariableLabels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $num => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <p><code dir="ltr" class="bg-slate-50 px-1 rounded border">{{{{ $num }}}}</code> — <?php echo e($label); ?></p>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
...
unset($__errorArgs, $__bag); ?>
    ...
    <div>
        ...
        <textarea name="body_text" rows="6" required class="<?php echo e($waTextareaClass); ?>" x-model="bodyText"
            ...
            <template x-for="(btn, i) in buttons" :key="i">
                <div class="grid grid-cols-1 sm:grid-cols-12 gap-2 p-3 rounded-xl border border-slate-200 bg-slate-50/50">
                    ...
                    <div class="sm:col-span-4" x-show="btn.type === 'URL'">
                        <input type="text" :name="'buttons['+i+'][url]'" x-model="btn.url" placeholder="https://chat.whatsapp.com/{{3}}" dir="ltr" class="<?php echo e($waInputClass); ?> !text-xs">
                    </div>
                    <div class="sm:col-span-4" x-show="btn.type === 'URL' && /\{\{\d+\}\}/.test(btn.url || '')">
                        <input type="text" :name="'buttons['+i+'][url_example]'" x-model="btn.url_example" placeholder="كود الدعوة فقط (مثل Ld0j8PUAprmCnDi65uUqTC)" dir="ltr" class="<?php echo e($waInputClass); ?> !text-xs">
                    </div>
