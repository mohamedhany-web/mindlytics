            ...
            <form method="GET" action="<?php echo e(route('admin.employee-agreements.index')); ?>" id="filterForm" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="status" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all">
                        ...
                        <option value="suspended" <?php echo e(request('status') == 'suspended' ? 'selected' : ''); ?>>مجمّد</option>
                        <option value="terminated" <?php echo e(request('status') == 'terminated' ? 'selected' : ''); ?>>موقوف / مغادرة</option>
                        ...
                        <tr class="hover:bg-slate-50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-1 flex-wrap">
                                    ...
                                    <?php if(in_array($agreement->status, ['active', 'draft'], true)): ?>
                                        <form action="<?php echo e(route('admin.employee-agreements.stop', $agreement)); ?>" method="POST" class="inline-flex items-center gap-1 agreement-stop-form"
                                              data-employee="<?php echo e(htmlspecialchars($agreement->employee->name ?? '', ENT_QUOTES, 'UTF-8')); ?>">
                                            <?php echo csrf_field(); ?>
                                    ...
                                    <?php elseif($agreement->status === 'suspended'): ?>
                                        <form action="<?php echo e(route('admin.employee-agreements.reactivate', $agreement)); ?>" method="POST" class="inline-block"
                                              onsubmit="return confirm('إعادة تفعيل اتفاقية <?php echo e(htmlspecialchars($agreement->employee->name ?? '', ENT_QUOTES, 'UTF-8')); ?>؟');">
                                            <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/admin/employee-agreements/index.blade.php ENDPATH**/ ?>