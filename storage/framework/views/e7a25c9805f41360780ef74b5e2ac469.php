...
<?php $__env->startSection('title', $template->displayTitle() . ' — قالب واتساب'); ?>
...
<div class="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6" style="background: #f8fafc; min-height: 100vh;">
    ...
    <?php if($template->displayRejectionReason()): ?>
        <div class="rounded-2xl border-2 border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-900">
            ...
            <p><?php echo e($template->displayRejectionReason()); ?></p>
        </div>
    <?php endif; ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-4">
            <section class="<?php echo e($waSectionClass); ?> p-5">
                ...
                <?php endif; ?>
                <?php if(is_array($template->buttons) && $template->buttons !== []): ?>
            ...
            <?php if(is_array($template->components) && $template->components !== []): ?>
                <section class="<?php echo e($waSectionClass); ?>">
                    ...
                    <pre class="p-5 text-xs overflow-x-auto dir-ltr text-left bg-slate-50 text-slate-700 max-h-64"><?php echo e(json_encode($template->components, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE)); ?></pre>
                </section>
            <?php endif; ?>
                    ...
                    <?php else: ?>
                        <form method="POST" action="<?php echo e(route('admin.whatsapp.templates.access', $template)); ?>" class="space-y-4">
                            ...
                            <div class="grid sm:grid-cols-2 gap-2 max-h-56 overflow-y-auto border border-slate-100 rounded-xl p-3 bg-slate-50/50">
                                <?php $assignedIds = ($template->assignedUsers ?? collect())->pluck('id')->all(); ?>
        ...
        <aside class="space-y-4">
            <section class="<?php echo e($waSectionClass); ?> p-5 text-sm space-y-3">
                <form method="POST" action="<?php echo e(route('admin.whatsapp.templates.display-name', $template)); ?>" class="space-y-2 pb-3 border-b border-slate-100">
                    <?php echo csrf_field(); ?>
                    <?php echo method_field('PUT'); ?>
                    ...
                    <input type="text" name="display_name" maxlength="255"
                           value="<?php echo e(old('display_name', $template->display_name)); ?>"
                           placeholder="<?php echo e($template->name); ?>"
                           class="<?php echo e($waInputClass); ?> !text-sm">
                    <button type="submit" class="<?php echo e($waBtnSecondary); ?> text-xs w-full justify-center">
                        <i class="fas fa-tag"></i> حفظ التسمية
                    </button>
                    <p class="text-[11px] text-slate-500">يمكن تعديل التسمية في أي وقت — كود Meta يبقى كما هو.</p>
                </form>
                <p><span class="text-slate-500">كود Meta:</span> <span class="font-mono text-xs dir-ltr"><?php echo e($template->name); ?></span></p>
            ...
            <div class="space-y-2">
                ...
                <?php elseif($template->canDuplicateForResubmit()): ?>
                    <form method="POST" action="<?php echo e(route('admin.whatsapp.templates.duplicate', $template)); ?>"><?php echo csrf_field(); ?>
                        <button type="submit" class="<?php echo e($waBtnPrimary); ?> w-full justify-center">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\mindly tics\Mindlytics\resources\views/admin/whatsapp/templates/show.blade.php ENDPATH**/ ?>