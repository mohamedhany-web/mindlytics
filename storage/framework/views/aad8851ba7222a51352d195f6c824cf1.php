    ...
    <div x-show="showSuggestedComposeModal" x-cloak
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col" @click.outside="showSuggestedComposeModal = false">
            ...
            <div class="px-5 py-4 border-t border-slate-100 flex flex-wrap gap-2 justify-end shrink-0 bg-slate-50/80">
                <button type="button" @click="showSuggestedComposeModal = false" class="<?php echo e($waBtnSecondary); ?> text-sm">إلغاء</button>
                <button type="button" @click="confirmSuggestedCompose(false)" class="<?php echo e($waBtnSecondary); ?> text-sm">إدراج</button>
                <button type="button" @click="confirmSuggestedCompose(true)" class="<?php echo e($waBtnPrimary); ?> text-sm">إدراج وإرسال</button>
            </div>
        </div>
    </div>
    ...
    <div x-show="showMetaTemplatePreviewModal" x-cloak
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" @click.outside="showMetaTemplatePreviewModal = false">
            ...
            <div class="px-5 py-4 border-t border-slate-100 flex flex-wrap gap-2 justify-end bg-slate-50/80">
                <button type="button" @click="showMetaTemplatePreviewModal = false" class="<?php echo e($waBtnSecondary); ?> text-sm">رجوع</button>
                <button type="button" @click="confirmMetaTemplatePreview()" :disabled="sending" class="<?php echo e($waBtnPrimary); ?> text-sm">
    ...
    return {
        ...
        substituteSuggestedVariables(body, values) {
            ...
            Object.entries(values || {}).forEach(([key, val]) => {
                ...
                out = out.split('<?php echo e(' + key + '); ?>').join(safe);
            });
            return out;
        },
        ...
        renderMetaTemplatePreview() {
            ...
            if (headerType === 'text' && headerContent) {
                ...
                if ((meta.header_variable_count || 0) > 0) {
                    headerText = headerText.split('<?php echo e(1); ?>').join((this.templateVariables.header_1 || '').trim());
                }
            ...
            if (body) {
                const count = meta.body_variable_count || 0;
                for (let i = 1; i <= count; i++) {
                    body = body.split('<?php echo e(' + i + '); ?>').join((this.templateVariables[i] || '').trim());
                }
                parts.push(body);
            }
