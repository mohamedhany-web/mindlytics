...
<div class="space-y-6">
            ...
            <div class="dashboard-card rounded-2xl p-6 card-hover-effect border-2 border-emerald-200/50 hover:border-emerald-300/70 shadow-xl hover:shadow-2xl transition-all duration-300" style="background: linear-gradient(135deg, rgba(255, 255, 255, 0.98) 0%, rgba(236, 253, 245, 0.95) 50%, rgba(209, 250, 229, 0.9) 100%);">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-black text-emerald-700"><?php echo e(number_format($stats['current_month_additions'] ?? 0, 2)); ?> ج.م</p>
                    </div>
            ...
            <div class="dashboard-card rounded-2xl p-6 card-hover-effect border-2 border-amber-200/50 hover:border-amber-300/70 shadow-xl hover:shadow-2xl transition-all duration-300 md:col-span-2" style="background: linear-gradient(135deg, rgba(255, 255, 255, 0.98) 0%, rgba(255, 251, 235, 0.95) 50%, rgba(254, 243, 199, 0.9) 100%);">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-black text-amber-700"><?php echo e(number_format($stats['current_month_commission'] ?? 0, 2)); ?> ج.م</p>
                    </div>
    ...
    <?php if(isset($recentCommissionTxns) && $recentCommissionTxns->count() > 0): ?>
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            <div class="flex items-center justify-between gap-3 mb-4">
                <h2 class="text-xl font-bold text-gray-900"><i class="fas fa-coins text-amber-600 mr-2"></i>سجل الكوميشن</h2>
                <p class="text-sm text-gray-600">الإجمالي: <strong class="text-amber-700"><?php echo e(number_format($stats['total_commission'] ?? 0, 2)); ?> ج.م</strong></p>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php $__currentLoopData = $recentCommissionTxns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tx): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><?php echo e($tx->transaction_number); ?></td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-amber-700"><?php echo e(number_format((float) $tx->amount, 2)); ?> ج.م</td>
                                <td class="px-6 py-4 text-sm text-gray-700"><?php echo e($tx->description); ?></td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600"><?php echo e($tx->created_at?->format('Y-m-d H:i')); ?></td>
                            </tr>
    ...
    <?php if($activeAgreement): ?>
        ...
        <?php if(isset($recentAdditions) && $recentAdditions->count() > 0): ?>
        <div class="bg-white rounded-xl shadow-lg p-6 border border-emerald-100">
            ...
            <div class="space-y-3">
                <?php $__currentLoopData = $recentAdditions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $addition): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="border border-emerald-100 rounded-lg p-4 hover:bg-emerald-50/30 transition-colors">
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            <h4 class="font-semibold text-gray-900"><?php echo e($addition->title); ?></h4>
                            <?php if($addition->description): ?>
                                <p class="text-sm text-gray-600 mt-1"><?php echo e($addition->description); ?></p>
                            <?php endif; ?>
                            <p class="text-xs text-gray-500 mt-1"><?php echo e($addition->addition_date->format('Y-m-d')); ?> — <?php echo e($addition->addition_number); ?></p>
                        </div>
                        <div class="text-left shrink-0">
                            <p class="text-lg font-bold text-emerald-600">+<?php echo e(number_format($addition->amount, 2)); ?> ج.م</p>
                            <span class="px-2 py-1 text-xs font-semibold rounded-full bg-emerald-100 text-emerald-800">
                                <?php echo e(\App\Models\EmployeeSalaryAddition::typeLabels()[$addition->type] ?? $addition->type); ?>
...
<?php echo $__env->make('layouts.employee', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/employee/accounting/index.blade.php ENDPATH**/ ?>