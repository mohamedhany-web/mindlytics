...
<p class="text-sm font-semibold text-gray-700"><?php echo e(__('student.total_progress')); ?></p>
    ...
    <?php if(!empty($pendingScholarshipRegistrations) && $pendingScholarshipRegistrations->isNotEmpty()): ?>
                    ...
                    <ul class="mt-3 space-y-2">
                        <?php $__currentLoopData = $pendingScholarshipRegistrations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $scholarshipRegistration): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <li class="rounded-xl bg-white border border-amber-100 px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                <div>
                                    <p class="font-semibold text-slate-900"><?php echo e($scholarshipRegistration->program?->name); ?></p>
                                    <p class="text-xs text-slate-500 mt-0.5">تاريخ التسجيل: <?php echo e($scholarshipRegistration->registered_at?->format('Y-m-d H:i')); ?></p>
                                </div>
