<?php $__env->startSection('title', 'تعديل نمط تعليمي'); ?>
<?php $__env->startSection('header', 'تعديل نمط تعليمي'); ?>
...
<div class="w-full max-w-full px-4 sm:px-6 lg:px-8 space-y-6">
    ...
    <form action="<?php echo e(route('instructor.learning-patterns.update', [$course, $pattern])); ?>" method="POST" id="patternForm">
        <?php echo csrf_field(); ?>
        ...
        <div class="rounded-2xl bg-white border border-slate-200 p-4 shadow-sm mb-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
                <?php $__currentLoopData = $types; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $typeKey => $typeInfo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="pattern-type-card rounded-lg p-2.5 <?php echo e($pattern->type === $typeKey ? 'selected' : ''); ?>" onclick="selectType('<?php echo e($typeKey); ?>')" data-type="<?php echo e($typeKey); ?>">
            ...
            <input type="hidden" name="type" id="selectedType" value="<?php echo e($pattern->type); ?>" required>
        ...
        <div class="rounded-2xl bg-white border border-slate-200 p-6 shadow-sm mb-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="title" value="<?php echo e(old('title', $pattern->title)); ?>" required class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-200">
...
unset($__errorArgs, $__bag); ?>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-200"><?php echo e(old('description', $pattern->description)); ?></textarea>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">التعليمات</label>
                    <textarea name="instructions" rows="4" class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-200"><?php echo e(old('instructions', $pattern->instructions)); ?></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl bg-white border border-slate-200 p-6 shadow-sm mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="points" value="<?php echo e(old('points', $pattern->points)); ?>" min="0" class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-200">
                </div>
                <div>
                    ...
                    <select name="difficulty_level" required class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-200">
                        <?php for($i = 1; $i <= 5; $i++): ?>
                            <option value="<?php echo e($i); ?>" <?php echo e(old('difficulty_level', $pattern->difficulty_level) == $i ? 'selected' : ''); ?>>مستوى <?php echo e($i); ?></option>
                        <?php endfor; ?>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">الحد الزمني (دقائق)</label>
                    <input type="number" name="time_limit_minutes" value="<?php echo e(old('time_limit_minutes', $pattern->time_limit_minutes)); ?>" min="1" placeholder="اختياري" class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-200">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">الحد الأقصى للمحاولات</label>
                    <input type="number" name="max_attempts" value="<?php echo e(old('max_attempts', $pattern->max_attempts)); ?>" min="1" placeholder="غير محدود" class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-200">
                </div>
            </div>
            <div class="mt-4 space-y-3">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_required" value="1" <?php echo e(old('is_required', $pattern->is_required) ? 'checked' : ''); ?> class="w-5 h-5 rounded border-slate-300 text-sky-500 focus:ring-sky-500">
                    <span class="text-sm font-semibold text-slate-700">إلزامي (يجب إكماله للمتابعة)</span>
                </label>
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="allow_multiple_attempts" value="1" <?php echo e(old('allow_multiple_attempts', $pattern->allow_multiple_attempts) ? 'checked' : ''); ?> class="w-5 h-5 rounded border-slate-300 text-sky-500 focus:ring-sky-500">
                ...
                <div>
                    ...
                    <select name="course_section_id" class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-200">
                        ...
                        <?php $__currentLoopData = $sections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $section): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($section->id); ?>" <?php echo e(($currentSection && $currentSection->id == $section->id) || old('course_section_id') == $section->id ? 'selected' : ''); ?>><?php echo e($section->title); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
            </div>
...
<script>
const patternData = <?php echo json_encode($pattern->pattern_data ?? [], 15, 512) ?>;
...
<?php echo $__env->make('instructor.learning-patterns.partials.pattern-form-script', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

// تحميل النموذج عند تحميل الصفحة مع ملء البيانات
document.addEventListener('DOMContentLoaded', function() {
    loadPatternDataForm('<?php echo e($pattern->type); ?>');
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/instructor/learning-patterns/edit.blade.php ENDPATH**/ ?>