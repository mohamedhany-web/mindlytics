...
<?php $__env->startSection('title', 'موافقة صفقات Win'); ?>
<?php $__env->startSection('header', 'موافقة صفقات الفوز'); ?>
...
<div class="space-y-6">
    <?php if(session('success')): ?>
        <div class="rounded-xl border-2 border-emerald-200 bg-emerald-50 text-emerald-800 px-5 py-4 text-sm font-semibold"><?php echo e(session('success')); ?></div>
    <?php endif; ?>
    <?php if($errors->any()): ?>
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 p-4">
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-4">
                ...
                <p class="text-2xl font-black text-slate-900 tabular-nums"><?php echo e(number_format($stats['pending_value'], 2)); ?> <span class="text-sm font-bold">ج.م</span></p>
            </div>
            <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                <p class="text-xs font-semibold text-emerald-800">كوميشن مقدّر</p>
                <p class="text-2xl font-black text-emerald-900 tabular-nums"><?php echo e(number_format($stats['pending_commission_est'], 2)); ?> <span class="text-sm font-bold">ج.م</span></p>
            </div>
        </div>
    </section>
        ...
        <div class="px-4 py-3 border-b border-slate-200 bg-slate-50">
            <form method="get" class="flex flex-wrap gap-3 items-end">
                <div class="min-w-[180px] flex-1">
                    ...
                    <input type="search" name="search" value="<?php echo e(request('search')); ?>" placeholder="اسم، هاتف، شركة..." class="<?php echo e($inputClass); ?>">
                </div>
                <div class="min-w-[160px]">
                    <label class="block text-xs font-semibold text-slate-600 mb-1">موظف السيلز</label>
                    <select name="assigned_to" class="<?php echo e($inputClass); ?>">
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php $__empty_1 = true; $__currentLoopData = $leads; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lead): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="hover:bg-slate-50/50 align-top">
                            <td class="p-3">
                                <a href="<?php echo e(route('admin.sales.leads.show', $lead)); ?>" class="font-bold text-sky-700 hover:underline"><?php echo e($lead->name); ?></a>
                                <p class="text-xs text-slate-500 mt-0.5"><?php echo e($lead->phone ?? '—'); ?> · <?php echo e($lead->company ?? '—'); ?></p>
                            </td>
                            <td class="p-3">
                                <p class="font-semibold text-slate-900"><?php echo e($rep?->name ?? '—'); ?></p>
                                <?php if($rep): ?>
                                    <p class="text-[10px] text-slate-500"><?php echo e($rep->salesCommissionLabel()); ?></p>
                                <?php endif; ?>
                            </td>
                            <td class="p-3 font-bold tabular-nums"><?php echo e(number_format((float) ($lead->expected_value ?? 0), 2)); ?> ج.م</td>
                            <td class="p-3 font-bold text-emerald-700 tabular-nums"><?php echo e(number_format($defaultCommission, 2)); ?> ج.م</td>
                            <td class="p-3 text-xs text-slate-600"><?php echo e($lead->closed_at?->format('Y-m-d H:i') ?? $lead->updated_at?->format('Y-m-d H:i')); ?></td>
                            <td class="p-3">
                                <form method="post" action="<?php echo e(route('admin.sales.win-approvals.approve', $lead)); ?>" class="space-y-2 mb-3 p-3 rounded-xl border border-emerald-200 bg-emerald-50/40">
                                    <?php echo csrf_field(); ?>
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="text-[10px] font-bold text-slate-600">مبلغ الكوميشن</label>
                                            <input type="number" step="0.01" min="0" name="commission_amount" value="<?php echo e(number_format($defaultCommission, 2, '.', '')); ?>" class="<?php echo e($inputClass); ?> !py-1.5 text-xs">
                                        </div>
                                        <div class="col-span-2">
                                            <label class="text-[10px] font-bold text-slate-600">ملاحظات (اختياري)</label>
                                            <input type="text" name="commission_notes" class="<?php echo e($inputClass); ?> !py-1.5 text-xs" placeholder="ملاحظة للاعتماد">
                                        </div>
                                    </div>
                                ...
                                <details class="text-xs">
                                    ...
                                    <form method="post" action="<?php echo e(route('admin.sales.win-approvals.reject', $lead)); ?>" class="mt-2 space-y-2 p-2 rounded-lg border border-rose-200 bg-rose-50/50">
                                        <?php echo csrf_field(); ?>
                                        <textarea name="rejection_reason" rows="2" required class="<?php echo e($inputClass); ?> !text-xs" placeholder="سبب الرفض للموظف"></textarea>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\mindly tics\Mindlytics\resources\views\admin\sales\win-approvals\index.blade.php ENDPATH**/ ?>