...
<body class="bg-gray-50 text-gray-900" x-data="{ mobileMenu: false, method: '<?php echo e(old('payment_method', 'bank_transfer')); ?>' }" :class="{ 'overflow-hidden': mobileMenu }">
                    ...
                    <div class="lg:col-span-1">
                        <div class="course-card p-6 sticky top-24">
                            ...
                            <div class="mb-6 pb-6 border-b border-gray-200 space-y-3 text-sm">
                                ...
                                <?php if($group->locationModel || $group->location): ?>
                                    <div class="flex items-start gap-2 text-gray-700">
                                        <i class="fas fa-map-marker-alt text-red-500 mt-0.5"></i>
                                        <span><?php echo e($group->locationModel->name ?? $group->location ?? '—'); ?></span>
                                    </div>
                                <?php endif; ?>
                                <?php if($group->start_date): ?>
                                    <div class="flex items-center gap-2 text-gray-700">
                                        <i class="fas fa-calendar text-indigo-500"></i>
                                        <span><?php echo e($group->start_date->format('Y-m-d')); ?> <?php if($group->end_date): ?> → <?php echo e($group->end_date->format('Y-m-d')); ?> <?php endif; ?></span>
                                    </div>
                                <?php endif; ?>
                                <div class="flex items-center gap-2">
                                    <i class="fas fa-user-friends text-emerald-600"></i>
                                    <span class="font-semibold text-gray-900">مقاعد متاحة للحجز الآن: <?php echo e($effectiveRemaining); ?></span>
                                    <span class="text-gray-500">/ <?php echo e($group->max_students); ?> سعة المجموعة</span>
                                </div>
                            </div>
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-gray-600 text-sm">سعر الكورس:</span>
                                <span class="text-xl font-black text-blue-600"><?php echo e(number_format((float) $course->price, 2)); ?> <span class="text-sm text-gray-600">ج.م</span></span>
                            </div>
                            <div class="flex items-center justify-between pt-4 border-t-2 border-gray-200">
                                ...
                                <span class="text-2xl font-black text-green-600"><?php echo e(number_format((float) $course->price, 2)); ?> <span class="text-base text-gray-600">ج.م</span></span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="course-card p-6 md:p-8">
                            ...
                            <?php if(!$courseScheduleOpen): ?>
                                <div class="p-5 bg-amber-50 border-2 border-amber-200 rounded-xl text-amber-900 text-sm font-medium space-y-2">
                                    ...
                                    <?php elseif(($scheduleBlockReason ?? null) === 'ended'): ?>
                                        <p><i class="fas fa-door-closed ml-2"></i> <strong>انتهت فترة الحجز.</strong> نهاية الحجز كانت:
                                            <span class="font-mono bg-white/80 px-2 py-0.5 rounded"><?php echo e($course->bookingClosesAtEffective()?->timezone(config('app.timezone'))->format('Y-m-d H:i')); ?></span>
                                        </p>
                                    <?php else: ?>
                            ...
                            <?php else: ?>
                                ...
                                <form action="<?php echo e(route('public.offline-groups.book', $group->public_slug)); ?>" method="POST" enctype="multipart/form-data" class="space-y-5">
                                    ...
                                    <div>
                                        ...
                                        <textarea name="student_notes" rows="3" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500" placeholder="اسمك كما في التحويل أو أي تفاصيل"><?php echo e(old('student_notes')); ?></textarea>
                                    </div>
...
<?php /**PATH C:\xampp\htdocs\mindly tics\Mindlytics\resources\views/public/offline-group-checkout.blade.php ENDPATH**/ ?>