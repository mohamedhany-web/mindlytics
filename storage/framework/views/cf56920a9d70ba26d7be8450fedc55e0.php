...
<div class="p-4 md:p-6 space-y-6" style="background:#f8fafc;min-height:100vh;">
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <a href="<?php echo e(route('admin.sales.kpi.targets')); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="rounded-2xl border border-rose-200 bg-rose-50/70 p-5">
            ...
            <p class="text-3xl font-black text-rose-800 tabular-nums"><?php echo e((int) ($slaSummary['overdue_followups'] ?? 0)); ?></p>
        </div>
        <div class="rounded-2xl border border-amber-200 bg-amber-50/70 p-5">
            ...
            <p class="text-3xl font-black text-amber-800 tabular-nums"><?php echo e((int) ($slaSummary['stale_open_leads'] ?? 0)); ?></p>
        </div>
        <div class="rounded-2xl border border-blue-200 bg-blue-50/70 p-5">
            ...
            <p class="text-3xl font-black text-blue-800 tabular-nums"><?php echo e(($slaSummary['avg_response_minutes'] ?? 0) > 0 ? number_format((float) $slaSummary['avg_response_minutes'], 1) : '—'); ?></p>
        </div>
    </div>
                ...
                <?php $__currentLoopData = $lossReasons; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <div>
                        <div class="flex justify-between gap-3 mb-1 text-sm">
                            <span class="font-semibold text-gray-800 truncate"><?php echo e($row->lost_reason); ?></span>
                            <span class="tabular-nums text-rose-700 font-bold"><?php echo e($row->total); ?></span>
                        </div>
                        <div class="h-2.5 rounded-full bg-gray-100 overflow-hidden">
                            <div class="h-full rounded-full bg-gradient-to-l from-rose-500 to-rose-400" style="width: <?php echo e($w); ?>%"></div>
                        </div>
                    </div>
                ...
                <table class="min-w-[700px] w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php $__currentLoopData = $sourcePerformance; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr>
                                <td class="px-3 py-2 font-semibold text-slate-800"><?php echo e($row['label']); ?></td>
                                <td class="px-3 py-2 text-center tabular-nums"><?php echo e($row['created']); ?></td>
                                <td class="px-3 py-2 text-center tabular-nums text-emerald-700 font-bold"><?php echo e($row['won']); ?></td>
                                <td class="px-3 py-2 text-center tabular-nums <?php echo e(($row['conversion'] ?? 0) >= 20 ? 'text-emerald-700 font-bold' : 'text-amber-700 font-semibold'); ?>">
                                    <?php echo e($row['conversion'] !== null ? number_format((float) $row['conversion'], 1) . '%' : '—'); ?>

                                </td>
                                <td class="px-3 py-2 text-left tabular-nums"><?php echo e(number_format((float) $row['revenue'], 0)); ?></td>
                            </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </tbody>
                </table>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm p-5 space-y-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
                <form method="GET" action="<?php echo e(route('admin.sales.kpi.index')); ?>" class="flex items-center gap-2">
                ...
                <span class="text-xs text-gray-500"><?php echo e($periodLabel ?? 'اليوم'); ?></span>
            </div>
        </div>
        <p class="text-xs text-slate-500">الفترة: <?php echo e(optional($rangeStart)->format('Y-m-d')); ?> إلى <?php echo e(optional($rangeEnd)->format('Y-m-d')); ?></p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div class="rounded-xl border border-indigo-200 bg-indigo-50/60 p-4">
                ...
                <p class="text-2xl font-black text-indigo-900 tabular-nums"><?php echo e((int) ($reminderMonitoringSummary['sent_total'] ?? 0)); ?></p>
            </div>
            <div class="rounded-xl border border-rose-200 bg-rose-50/60 p-4">
                ...
                <p class="text-2xl font-black text-rose-900 tabular-nums"><?php echo e((int) ($reminderMonitoringSummary['reps_with_alerts'] ?? 0)); ?></p>
            </div>
            <div class="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4">
                ...
                <p class="text-2xl font-black text-emerald-900 tabular-nums"><?php echo e(($reminderMonitoringSummary['avg_compliance_pct'] ?? 0) > 0 ? number_format((float) $reminderMonitoringSummary['avg_compliance_pct'], 1).'%' : '—'); ?></p>
            </div>
        </div>
            ...
            <table class="min-w-[900px] w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php $__empty_1 = true; $__currentLoopData = ($reminderMonitoringRows ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr>
                            <td class="px-3 py-2 font-semibold text-slate-800"><?php echo e($row['rep']->name); ?></td>
                            <td class="px-3 py-2 text-center tabular-nums"><?php echo e($row['sent_count'] ?? 0); ?></td>
                            <td class="px-3 py-2 text-center tabular-nums"><?php echo e($row['overdue_at_reminder'] ?? 0); ?></td>
                            <td class="px-3 py-2 text-center tabular-nums"><?php echo e($row['today_at_reminder'] ?? 0); ?></td>
                            <td class="px-3 py-2 text-center tabular-nums"><?php echo e($row['stale_at_reminder'] ?? 0); ?></td>
                            <td class="px-3 py-2 text-center tabular-nums <?php echo e(($row['current_overdue'] ?? 0) > 0 ? 'text-rose-700 font-bold' : 'text-emerald-700 font-bold'); ?>"><?php echo e($row['current_overdue'] ?? 0); ?></td>
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php $__empty_1 = true; $__currentLoopData = $rows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="hover:bg-emerald-50/30">
                            <td class="px-4 py-3 font-semibold text-gray-900"><?php echo e($u->name); ?></td>
                            ...
                            <?php $__currentLoopData = ['results','activity','quality','discipline']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pk): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <td class="px-4 py-3 text-center tabular-nums text-gray-700"><?php echo e($r['pillars'][$pk]['score'] ?? '—'); ?></td>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <td class="px-4 py-3 text-left tabular-nums font-medium"><?php echo e(number_format($r['month_revenue'], 0)); ?></td>
                            <td class="px-4 py-3 text-center"><?php echo e($r['month_won']); ?></td>
                            <td class="px-4 py-3 text-center"><?php echo e($r['open_pipeline']); ?></td>
                            <td class="px-4 py-3 text-center tabular-nums <?php echo e(($r['overdue_followups'] ?? 0) > 0 ? 'text-rose-700 font-bold' : 'text-gray-500'); ?>"><?php echo e($r['overdue_followups'] ?? 0); ?></td>
                            <td class="px-4 py-3 text-center tabular-nums <?php echo e(($r['stale_open_leads'] ?? 0) > 0 ? 'text-amber-700 font-bold' : 'text-gray-500'); ?>"><?php echo e($r['stale_open_leads'] ?? 0); ?></td>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/admin/sales/kpi/index.blade.php ENDPATH**/ ?>