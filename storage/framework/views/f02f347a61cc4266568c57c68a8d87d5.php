...
<div class="p-6 bg-gray-50 min-h-screen">
    ...
    <?php if(session('success')): ?>
        <div class="mb-6 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 px-5 py-4 font-semibold"><?php echo e(session('success')); ?></div>
    <?php endif; ?>
    <?php if(session('error')): ?>
        <div class="mb-6 rounded-xl bg-red-50 border border-red-200 text-red-800 px-5 py-4 font-semibold"><?php echo e(session('error')); ?></div>
    <?php endif; ?>
    ...
    <div class="mb-8 rounded-2xl border-2 border-violet-200 bg-gradient-to-br from-violet-50 to-white p-6 shadow-sm">
        ...
        <?php if(!empty($workshopActivation) && $workshopActivation->isUsable()): ?>
            <div class="rounded-xl bg-emerald-50 border border-emerald-200 p-4 text-emerald-800 text-sm font-semibold">
                <i class="fas fa-check-circle ml-1"></i>
                خصم مفعّل: <?php echo e($workshopActivation->promoCode?->discountLabel() ?? ''); ?>

                <?php if($workshopActivation->promoCode?->expires_at): ?>
                    — ينتهي <?php echo e($workshopActivation->promoCode->expires_at->format('Y-m-d')); ?>

                <?php endif; ?>
            </div>
        <?php else: ?>
            <form action="<?php echo e(route('promo-code.activate')); ?>" method="POST" class="flex flex-col sm:flex-row gap-3">
                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/student/referrals/index.blade.php ENDPATH**/ ?>