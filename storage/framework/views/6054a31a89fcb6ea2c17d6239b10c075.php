...
<?php $__env->startSection('title', $lecture->title . ' - محاضرة ' . (($channel ?? 'offline') === 'online' ? 'أونلاين' : 'أوفلاين')); ?>
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-5">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <a href="<?php echo e(route('instructor.offline-courses.lectures.edit', ['offlineCourse' => $offlineCourse, 'lecture' => $lecture, 'channel' => ($channel ?? 'offline')])); ?>" class="px-4 py-2 bg-violet-600 text-white rounded-xl font-semibold hover:bg-violet-700">تعديل</a>
        </div>
    </div>

    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-6 space-y-4">
        <?php if($lecture->groupSession): ?>
            <div>
                <h3 class="text-sm font-bold text-slate-600 mb-2">جلسة التقويم</h3>
                <p class="text-slate-800 font-medium">
                    <?php echo e($lecture->groupSession->session_date->format('Y/m/d')); ?>

                    <?php $gst = $lecture->groupSession->start_time; ?>
                    <?php echo e(is_string($gst) ? substr($gst, 0, 5) : $gst); ?>

                    <?php if($lecture->groupSession->group): ?>
                        — <?php echo e($lecture->groupSession->group->name); ?>

                    <?php endif; ?>
                    <?php if($lecture->groupSession->title): ?>
                        — <?php echo e($lecture->groupSession->title); ?>
        ...
        <?php if(filled($lecture->session_agenda)): ?>
            <div>
                ...
                <ul class="text-slate-700 text-sm space-y-1 list-disc list-inside">
                    <?php $__currentLoopData = array_values(array_filter(array_map('trim', preg_split('/\r\n|\r|\n/', $lecture->session_agenda)))); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $line): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        ...
        <?php endif; ?>
        <?php if($lecture->meeting_url): ?>
            <div>
                <h3 class="text-sm font-bold text-slate-600 mb-2">رابط الميتينج</h3>
                <a href="<?php echo e($lecture->meeting_url); ?>" target="_blank" rel="noopener" class="text-indigo-600 hover:underline font-medium"><?php echo e($lecture->meeting_url); ?></a>
            </div>
        <?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/instructor/offline-courses/lectures/show.blade.php ENDPATH**/ ?>