...
<form method="POST" action="<?php echo e($formAction); ?>" class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <label class="flex items-center gap-3 rounded-xl border border-slate-200 p-4 bg-white">
            <input type="checkbox" name="enabled" value="1" class="rounded" <?php if($s['enabled'] ?? true): echo 'checked'; endif; ?>>
            <span class="text-sm font-semibold text-slate-800">تفعيل التقرير اليومي الإلزامي</span>
        </label>
        <label class="flex items-center gap-3 rounded-xl border border-slate-200 p-4 bg-white">
            <input type="checkbox" name="work_days_only" value="1" class="rounded" <?php if($s['work_days_only'] ?? true): echo 'checked'; endif; ?>>
        ...
        <div>
            ...
            <input type="time" name="deadline_time" value="<?php echo e($s['deadline_time'] ?? '23:59'); ?>" required class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 text-sm">
        </div>
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-2">هدف KPI — نسبة التسليم الشهرية %</label>
            <input type="number" name="kpi_submission_target_pct" min="50" max="100" step="1" value="<?php echo e($s['kpi_submission_target_pct'] ?? 95); ?>" required class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 text-sm">
        </div>
    </div>
    ...
    <div class="rounded-2xl border-2 border-rose-200 bg-rose-50/50 p-5 space-y-4">
        ...
        <label class="flex items-center gap-3">
            <input type="checkbox" name="penalty_enabled" value="1" class="rounded" <?php if($s['penalty_enabled'] ?? true): echo 'checked'; endif; ?>>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="penalty_amount" step="0.01" min="0.01" value="<?php echo e($s['penalty_amount'] ?? 50); ?>" required class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 text-sm">
            </div>
            <div>
                ...
                <select name="penalty_status" class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 text-sm">
                    <?php $__currentLoopData = ['pending' => 'معلّق', 'applied' => 'مطبّق', 'cancelled' => 'ملغى']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($val); ?>" <?php if(($s['penalty_status'] ?? 'pending') === $val): echo 'selected'; endif; ?>><?php echo e($label); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-slate-700 mb-2">عنوان الخصم</label>
                <input type="text" name="penalty_title" value="<?php echo e($s['penalty_title'] ?? ''); ?>" required class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 text-sm">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-slate-700 mb-2">وصف الخصم</label>
                <textarea name="penalty_description" rows="2" class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 text-sm"><?php echo e($s['penalty_description'] ?? ''); ?></textarea>
            </div>
            <div>
                ...
                <select name="penalty_type" class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 text-sm">
                    <?php $__currentLoopData = ['penalty' => 'غرامة', 'other' => 'أخرى', 'tax' => 'ضريبة', 'insurance' => 'تأمين', 'loan' => 'قرض']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($val); ?>" <?php if(($s['penalty_type'] ?? 'penalty') === $val): echo 'selected'; endif; ?>><?php echo e($label); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
        </div>
...
<?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/admin/sales/daily-reports/_settings_form.blade.php ENDPATH**/ ?>