<div class="flex flex-col h-full bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 shadow-2xl border-l border-slate-700/50 text-white">
    ...
    <div class="p-6 border-b-2 border-slate-700/50 bg-slate-900/90 flex-shrink-0 emp-sidebar-logo-wrap">
        <a href="<?php echo e(route('employee.dashboard')); ?>" class="flex items-center gap-4" title="Mindlytics">
            <div class="relative">
    ...
    <nav class="flex-1 overflow-y-auto overflow-x-hidden px-4 py-6 space-y-2 employee-sidebar-nav">
        ...
        <?php if(auth()->user()->isSalesStaff()): ?>
        <?php $waQueueCount = app(\App\Services\WhatsAppQueueService::class)->pendingCount(); ?>
        <div class="border-t border-slate-700/50 my-2 pt-2">
            <p class="px-4 text-xs font-semibold text-amber-400/90 uppercase tracking-wider mb-1">واتساب — طلبات</p>
            <a href="<?php echo e(route('employee.sales.whatsapp.queue.index')); ?>"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales.whatsapp.queue.*') ? 'bg-amber-500 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
                <i class="fas fa-bell text-base"></i>
                <span class="flex-1">طلبات جديدة</span>
                <span id="wa-queue-badge" class="min-w-[1.25rem] h-5 px-1.5 rounded-full text-xs font-bold flex items-center justify-center <?php echo e($waQueueCount > 0 ? 'bg-amber-500 text-white' : 'hidden'); ?>"><?php echo e($waQueueCount); ?></span>
            </a>
        </div>
        <?php endif; ?>
        ...
        <div class="border-t border-slate-700/50 my-2 pt-2">
            ...
            <a href="<?php echo e(route('employee.sales.follow-ups.index')); ?>"
               title="متابعاتي"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales.follow-ups.*') ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
            ...
            <a href="<?php echo e(route('employee.sales.whatsapp.inbox.index')); ?>"
               ...
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales.whatsapp.inbox.*') ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
        ...
        <div class="border-t border-slate-700/50 my-2 pt-2">
            ...
            <a href="<?php echo e(route('employee.sales-manager.dashboard')); ?>"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales-manager.dashboard') ? 'bg-teal-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
                <i class="fas fa-users-cog text-base"></i>
                <span>مركز الفريق</span>
            </a>
            <a href="<?php echo e(route('employee.sales-manager.leads.index')); ?>"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales-manager.leads.*') ? 'bg-teal-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
                <i class="fas fa-user-plus text-base"></i>
                <span>عملاء الفريق</span>
            </a>
            <a href="<?php echo e(route('employee.sales-manager.follow-ups.index')); ?>"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales-manager.follow-ups.*') ? 'bg-teal-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
                <i class="fas fa-clipboard-list text-base"></i>
                <span>رقابة المتابعات</span>
            </a>
            <a href="<?php echo e(route('employee.sales-manager.whatsapp.inbox.index')); ?>"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales-manager.whatsapp.inbox.*') ? 'bg-teal-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
                <i class="fab fa-whatsapp text-base"></i>
                <span>محادثات الفريق</span>
            </a>
            <a href="<?php echo e(route('employee.sales-manager.daily-reports.index')); ?>"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales-manager.daily-reports.*') ? 'bg-teal-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
                <i class="fas fa-clipboard-list text-base"></i>
                <span>تقارير الأعضاء</span>
            </a>
            <a href="<?php echo e(route('employee.sales-manager.team-reports.index')); ?>"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales-manager.team-reports.*') ? 'bg-teal-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
                <i class="fas fa-clipboard-check text-base"></i>
                <span>تقرير الفريق للإدارة</span>
            </a>
            <a href="<?php echo e(route('employee.sales-manager.attendance.index')); ?>"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales-manager.attendance.*') ? 'bg-teal-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
                <i class="fas fa-clock text-base"></i>
                <span>حضور الفريق</span>
            </a>
            <a href="<?php echo e(route('employee.sales-manager.presence.index')); ?>"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales-manager.presence.*') ? 'bg-teal-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
            ...
            <a href="<?php echo e(route('employee.sales-manager.transfer.index')); ?>"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 <?php echo e(request()->routeIs('employee.sales-manager.transfer.*') ? 'bg-teal-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'); ?>"
               @click="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); }">
        ...
        <?php if(!auth()->user()->isSalesEmployee() && !auth()->user()->isSalesManager()): ?>
