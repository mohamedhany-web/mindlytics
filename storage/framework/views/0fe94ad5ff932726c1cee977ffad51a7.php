...
<?php $__env->startSection('title', 'تقرير أداء الموظفين — '.$monthNames[$month].' '.$year); ?>
...
<div class="space-y-8">
    <div class="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
        <form method="get" class="flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div>
                ...
                <input type="number" name="year" value="<?php echo e($year); ?>" min="2000" max="2100" class="w-28 rounded-xl border-slate-300 text-sm shadow-sm focus:ring-violet-500 focus:border-violet-500">
            </div>
            <div>
        ...
        <div class="flex flex-wrap gap-3">
            <a href="<?php echo e($excelUrl); ?>" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold shadow-lg transition-colors">
                <i class="fas fa-file-excel"></i>
                تنزيل Excel (٤ أوراق)
            </a>
            <a href="<?php echo e(route('admin.design-task-cycles.index')); ?>" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border-2 border-slate-200 bg-white text-slate-700 text-sm font-semibold hover:bg-slate-50">
    ...
    <p class="text-sm text-slate-600 flex flex-wrap items-center gap-x-4 gap-y-1">
        <span><i class="fas fa-calendar-alt text-violet-500 ml-1"></i> الفترة: <strong><?php echo e($start->format('Y-m-d')); ?></strong> — <strong><?php echo e($end->format('Y-m-d')); ?></strong></span>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        <div class="rounded-2xl bg-gradient-to-br from-violet-600 to-violet-800 text-white p-5 shadow-lg border border-violet-500/30">
            ...
            <p class="text-3xl font-black mt-1 tabular-nums"><?php echo e(number_format($summary['tasks_completed'])); ?></p>
            <p class="text-xs text-violet-200 mt-2">من <?php echo e(number_format($summary['tasks_assigned'])); ?> مسندة بالشهر</p>
        </div>
        <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
            ...
            <p class="text-3xl font-black text-slate-900 mt-1 tabular-nums">
                <?php echo e($summary['tasks_on_time_rate_pct'] !== null ? $summary['tasks_on_time_rate_pct'].'%' : '—'); ?>

            </p>
            <div class="mt-3 h-2 rounded-full bg-slate-100 overflow-hidden">
                <div class="h-full rounded-full bg-emerald-500 transition-all" style="width: <?php echo e(min(100, $summary['tasks_on_time_rate_pct'] ?? 0)); ?>%"></div>
            </div>
            <p class="text-xs text-slate-500 mt-2"><?php echo e($summary['tasks_on_time']); ?> في الموعد · <?php echo e($summary['tasks_late']); ?> متأخر</p>
        </div>
        <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
            ...
            <p class="text-3xl font-black text-fuchsia-700 mt-1 tabular-nums">
                <?php echo e($summary['designer_on_time_rate_pct'] !== null ? $summary['designer_on_time_rate_pct'].'%' : '—'); ?>

            </p>
            <p class="text-xs text-slate-500 mt-2"><?php echo e($summary['designer_submissions_month']); ?> تسليم بالشهر</p>
        </div>
        <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
            <p class="text-xs font-bold text-slate-500 uppercase">تسليمات ملفات</p>
            <p class="text-3xl font-black text-slate-900 mt-1 tabular-nums"><?php echo e(number_format($summary['deliverables'])); ?></p>
        ...
        <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
            ...
            <p class="text-3xl font-black text-violet-700 mt-1 tabular-nums"><?php echo e(number_format($summary['design_cycles_touched_month'] ?? 0)); ?></p>
        ...
        <div class="rounded-2xl bg-amber-50 border border-amber-200 p-5 shadow-sm">
            ...
            <p class="text-3xl font-black text-amber-900 mt-1 tabular-nums"><?php echo e(number_format($summary['open_overdue_tasks'])); ?></p>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-md overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50/80 flex flex-wrap items-center justify-between gap-2">
            ...
            <span class="text-xs text-slate-500"><?php echo e(count($rows)); ?> موظفاً نشطاً</span>
        </div>
            ...
            <table class="min-w-[1600px] w-full text-xs">
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php $__currentLoopData = $rows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="hover:bg-violet-50/40 <?php echo e($i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'); ?>">
                            <td class="px-3 py-2 font-bold text-slate-900 sticky right-0 z-10 border-l border-slate-100 <?php echo e($i % 2 === 0 ? 'bg-white' : 'bg-slate-50'); ?>"><?php echo e($row['user']->name); ?></td>
                            <td class="px-2 py-2 text-slate-600 border-slate-100 border-l max-w-[120px] truncate" title="<?php echo e($row['user']->employeeJob->name ?? ''); ?>"><?php echo e($row['user']->employeeJob->name ?? '—'); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['tasks_assigned_in_month']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums font-semibold text-violet-800 border-slate-100 border-l"><?php echo e($row['tasks_completed_in_month']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['tasks_completion_rate_pct'] !== null ? $row['tasks_completion_rate_pct'].'%' : '—'); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums text-emerald-700 border-slate-100 border-l"><?php echo e($row['tasks_on_time']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums text-red-600 border-slate-100 border-l"><?php echo e($row['tasks_late']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['tasks_on_time_rate_pct'] !== null ? $row['tasks_on_time_rate_pct'].'%' : '—'); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['tasks_completed_design']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['tasks_completed_video']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['tasks_completed_sales']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['tasks_completed_other']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['design_cycles_as_designer']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['designer_submissions_in_month']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['designer_on_time']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['designer_late']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['designer_on_time_rate_pct'] !== null ? $row['designer_on_time_rate_pct'].'%' : '—'); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['design_cycles_created_as_moderator']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['design_cycles_completed_as_moderator']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['design_cycles_cancelled_as_moderator']); ?></td>
                            <td class="text-center px-1 py-2 tabular-nums border-slate-100 border-l"><?php echo e($row['moderator_avg_cycle_completion_days'] !== null ? $row['moderator_avg_cycle_completion_days'] : '—'); ?></td>
                        </tr>
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php $__currentLoopData = $rows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="hover:bg-slate-50">
                            <td class="px-3 py-2 font-medium sticky right-0 bg-white"><?php echo e($row['user']->name); ?></td>
                            <td class="text-center py-2 tabular-nums"><?php echo e($row['avg_completion_hours'] !== null ? $row['avg_completion_hours'] : '—'); ?></td>
                            <td class="text-center py-2 tabular-nums text-amber-800 font-medium"><?php echo e($row['open_overdue_tasks_end_of_month']); ?></td>
                            <td class="text-center py-2 tabular-nums"><?php echo e($row['deliverables_submitted']); ?></td>
                            <td class="text-center py-2 tabular-nums text-slate-500"><?php echo e($row['tasks_no_deadline_completed']); ?></td>
                        </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </tbody>
            </table>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/admin/design-task-cycles/performance-report.blade.php ENDPATH**/ ?>