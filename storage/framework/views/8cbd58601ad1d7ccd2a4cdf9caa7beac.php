        ...
        <div class="px-4 py-4 bg-slate-50 border-b border-slate-200 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <h2 class="text-lg font-black text-slate-900 tracking-tight"><?php echo e($invoice->invoice_number); ?></h2>
                </div>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <?php if(Route::has('admin.accounting.hub')): ?>
                <a href="<?php echo e(route('admin.accounting.hub')); ?>" class="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-white">
                    <i class="fas fa-calculator text-sky-600"></i>
                    المحاسبة
                </a>
                <?php endif; ?>
                <a href="<?php echo e(route('admin.payments.index')); ?>?search=<?php echo e(urlencode($invoice->invoice_number)); ?>" class="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-white">
                    <i class="fas fa-credit-card text-emerald-600"></i>
                    المدفوعات
                </a>
                <?php if($invoice->order_id): ?>
                <a href="<?php echo e(route('admin.orders.show', $invoice->order_id)); ?>" class="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-white">
                    <i class="fas fa-shopping-cart text-violet-600"></i>
                    الطلب
                </a>
                <?php endif; ?>
                <a href="<?php echo e(route('admin.invoices.edit', $invoice)); ?>" class="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-white">
                ...
                <a href="<?php echo e(route('admin.invoices.index')); ?>" class="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-white">
    ...
    <div id="invoice-print-area" class="w-full rounded-2xl border border-slate-200 bg-white shadow-lg overflow-hidden print:shadow-none print:border-slate-300">
        ...
        <div class="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-sky-900 text-white px-6 py-8 sm:px-10">
            ...
            <div class="relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-[0.2em] text-white/60"><?php echo e($brandName); ?></p>
                        ...
                        <div class="mt-3 space-y-1 text-sm text-white/80">
                            <?php if($supportEmail): ?>
                                <p class="flex items-center gap-2"><i class="fas fa-envelope w-4 text-center text-white/50"></i> <?php echo e($supportEmail); ?></p>
                            <?php endif; ?>
                            <?php if($supportPhone): ?>
                                <p class="flex items-center gap-2 font-mono dir-ltr text-right"><i class="fas fa-phone w-4 text-center text-white/50"></i> <?php echo e($supportPhone); ?></p>
                            <?php endif; ?>
                            <?php if($appUrl): ?>
                                <p class="flex items-center gap-2 text-xs text-white/60 break-all"><i class="fas fa-link w-4 text-center text-white/40"></i> <?php echo e($appUrl); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                <div class="rounded-2xl bg-white/10 backdrop-blur-sm ring-1 ring-white/20 px-5 py-4 text-sm min-w-[240px]">
                    <dl class="space-y-2.5">
                        <div class="flex justify-between gap-6 border-b border-white/10 pb-2">
                            ...
                            <dd class="font-mono font-bold text-white"><?php echo e($invoice->invoice_number); ?></dd>
                        </div>
                        <div class="flex justify-between gap-6 border-b border-white/10 pb-2">
                            <dt class="text-white/60">تاريخ الإصدار</dt>
                            <dd class="font-semibold text-white tabular-nums"><?php echo e($invoice->created_at->format('Y-m-d')); ?></dd>
                        </div>
                        <div class="flex justify-between gap-6 border-b border-white/10 pb-2">
                            <dt class="text-white/60">الاستحقاق</dt>
                            <dd class="font-semibold text-white tabular-nums"><?php echo e($invoice->due_date ? $invoice->due_date->format('Y-m-d') : '—'); ?></dd>
                        </div>
                        <?php if($invoice->paid_at): ?>
                            <div class="flex justify-between gap-6 border-b border-white/10 pb-2">
                                <dt class="text-white/60">تاريخ السداد</dt>
                                <dd class="font-semibold text-emerald-200 tabular-nums"><?php echo e($invoice->paid_at->format('Y-m-d')); ?></dd>
                            </div>
                        <?php endif; ?>
                        <div class="flex justify-between gap-6 items-center pt-1">
                            <dt class="text-white/60">الحالة</dt>
                            <dd>
                                <span class="inline-flex items-center rounded-lg px-2.5 py-1 text-xs font-bold bg-white/20 text-white ring-1 ring-white/30"><?php echo e($st['label']); ?></span>
                            </dd>
                        </div>
                    </dl>
                </div>
        ...
        <div class="p-6 sm:p-8 space-y-8">
            <div class="flex flex-wrap items-center gap-3">
                <span class="inline-flex items-center gap-2 rounded-xl px-3 py-1.5 text-xs font-bold ring-1 <?php echo e($st['class']); ?>">
                    <span class="h-2 w-2 rounded-full bg-current opacity-70"></span>
                    <?php echo e($st['label']); ?>

                </span>
                <span class="inline-flex items-center rounded-xl bg-slate-100 text-slate-800 px-3 py-1.5 text-xs font-bold"><?php echo e($typeLabel); ?></span>
            </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50/60 p-5 sm:p-6">
                    ...
                    <dl class="space-y-3 text-sm">
                        <div class="flex justify-between gap-4 border-b border-slate-200/80 pb-2">
                            ...
                            <dd class="font-bold text-slate-900 text-left"><?php echo e($invoice->user->name ?? '—'); ?></dd>
                        </div>
                        <div class="flex justify-between gap-4 border-b border-slate-200/80 pb-2">
                            <dt class="text-slate-500">البريد</dt>
                            <dd class="font-semibold text-slate-800 text-left break-all"><?php echo e($invoice->user->email ?? '—'); ?></dd>
                        </div>
                        <div class="flex justify-between gap-4 border-b border-slate-200/80 pb-2">
                            <dt class="text-slate-500">الهاتف</dt>
                            <dd class="font-mono font-semibold text-slate-800 dir-ltr text-left"><?php echo e($invoice->user->phone ?? '—'); ?></dd>
                        </div>
                    </dl>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-slate-50/60 p-5 sm:p-6">
                    ...
                    <dl class="space-y-3 text-sm">
                        <div class="flex justify-between gap-4 border-b border-slate-200/80 pb-2">
                            ...
                            <dd class="font-semibold text-slate-800 tabular-nums"><?php echo e($invoice->updated_at->format('Y-m-d H:i')); ?></dd>
                        </div>
                        <?php if($invoice->status === 'partial' || (float) $invoice->paid_amount > 0): ?>
                            <div class="flex justify-between gap-4 border-b border-slate-200/80 pb-2">
                                <dt class="text-slate-500">المدفوع</dt>
                                <dd class="font-bold text-emerald-700 tabular-nums"><?php echo e(number_format((float) $invoice->paid_amount, 2)); ?> ج.م</dd>
                            </div>
                            <div class="flex justify-between gap-4 border-b border-slate-200/80 pb-2">
                                <dt class="text-slate-500">المتبقي</dt>
                                <dd class="font-bold text-amber-800 tabular-nums"><?php echo e(number_format(max(0, (float) $invoice->remaining_amount), 2)); ?> ج.م</dd>
                            </div>
                        <?php endif; ?>
                        <div>
                            <dt class="text-slate-500 mb-1">ملاحظات</dt>
                            <dd class="text-slate-800 leading-relaxed rounded-xl bg-white border border-slate-100 p-3 text-sm"><?php echo e($invoice->notes ? e($invoice->notes) : '—'); ?></dd>
                        </div>
                    </dl>
                </div>
            </div>
            ...
            <?php if($invoice->description && $invoice->description !== '-'): ?>
                <div class="rounded-2xl border border-slate-200 bg-white p-5">
                    ...
                    <p class="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap"><?php echo e($invoice->description); ?></p>
                </div>
            <?php endif; ?>
                    ...
                    <?php if(count($lineItems) > 0): ?>
                        <table class="min-w-full text-sm">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <?php $__currentLoopData = $lineItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $line): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr class="hover:bg-slate-50/80">
                                        <td class="px-4 py-3 text-slate-900 font-medium"><?php echo e($line['description']); ?></td>
                                        <td class="px-4 py-3 text-center tabular-nums text-slate-700"><?php echo e($line['quantity']); ?></td>
                                        <td class="px-4 py-3 text-left tabular-nums text-slate-800"><?php echo e($line['unit'] !== null ? number_format($line['unit'], 2) . ' ج.م' : '—'); ?></td>
                                        <td class="px-4 py-3 text-left font-bold tabular-nums text-slate-900"><?php echo e($line['total'] !== null ? number_format($line['total'], 2) . ' ج.م' : '—'); ?></td>
                                    </tr>
                ...
                <table class="w-full text-sm">
                    <tbody class="divide-y divide-slate-200">
                        <tr>
                            ...
                            <td class="py-3 text-left font-bold tabular-nums text-slate-900"><?php echo e(number_format((float) $invoice->subtotal, 2)); ?> ج.م</td>
                        </tr>
                        <?php if((float) $invoice->tax_amount > 0): ?>
                            <tr>
                                <td class="py-3 text-slate-600">الضريبة</td>
                                <td class="py-3 text-left font-bold tabular-nums text-slate-900"><?php echo e(number_format((float) $invoice->tax_amount, 2)); ?> ج.م</td>
                            </tr>
                        <?php endif; ?>
                        <?php if((float) $invoice->discount_amount > 0): ?>
                            <tr>
                                <td class="py-3 text-slate-600">الخصم</td>
                                <td class="py-3 text-left font-bold tabular-nums text-rose-600">− <?php echo e(number_format((float) $invoice->discount_amount, 2)); ?> ج.م</td>
                            </tr>
                        <?php endif; ?>
                        <tr class="bg-sky-50/80">
                            <td class="py-4 text-base font-black text-slate-900">الإجمالي</td>
                            <td class="py-4 text-left text-xl font-black text-sky-700 tabular-nums"><?php echo e(number_format((float) $invoice->total_amount, 2)); ?> ج.م</td>
                        </tr>
                    </tbody>
                </table>
            </div>
                                ...
                                <?php $__currentLoopData = $invoice->payments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $payment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr class="hover:bg-slate-50/80">
                                        <td class="px-4 py-3 font-mono font-semibold text-slate-900">
                                            <a href="<?php echo e(route('admin.payments.show', $payment)); ?>" class="text-sky-700 hover:underline"><?php echo e($payment->payment_number); ?></a>
                                        </td>
                                        <td class="px-4 py-3 text-center tabular-nums text-slate-700"><?php echo e($payment->paid_at ? $payment->paid_at->format('Y-m-d H:i') : '—'); ?></td>
                                        <td class="px-4 py-3 text-center text-slate-600"><?php echo e($payment->payment_method ?? '—'); ?></td>
                                        <td class="px-4 py-3 text-center">
                                            <span class="text-xs font-semibold rounded-lg px-2 py-0.5 <?php echo e($payment->status === 'completed' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700'); ?>"><?php echo e($payment->status ?? '—'); ?></span>
                                        </td>
                                        <td class="px-4 py-3 text-left font-bold tabular-nums"><?php echo e(number_format((float) $payment->amount, 2)); ?> ج.م</td>
                                    </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </tbody>
                        </table>
            ...
            <p class="text-center text-[11px] text-slate-400 print:mt-6">
                وثيقة مُولَّدة آلياً — <?php echo e($brandName); ?> — <?php echo e(now()->format('Y-m-d H:i')); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/admin/invoices/show.blade.php ENDPATH**/ ?>