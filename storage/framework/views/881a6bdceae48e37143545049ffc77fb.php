...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
        ...
        <p class="text-sm text-gray-500 mb-6">
            رقم المعاملة: <?php echo e($transaction->transaction_number ?? ('#' . $transaction->id)); ?>

            — النوع الحالي: <?php echo e($typeLabels[$transaction->type] ?? $transaction->type); ?>
        ...
        <?php if($wasRefunded): ?>
            <div class="mb-6 rounded-xl border border-orange-200 bg-orange-50 px-4 py-3 text-sm text-orange-900">
                ...
                <?php if(!empty($metadata['refunded_at'])): ?>
                    في <?php echo e(\Illuminate\Support\Carbon::parse($metadata['refunded_at'])->format('Y-m-d H:i')); ?>
        ...
        <form action="<?php echo e(route('admin.transactions.update', $transaction)); ?>" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="user_id" required <?php if($wasRefunded): echo 'disabled'; endif; ?> class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 disabled:bg-gray-100">
                        <?php $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($user->id); ?>" <?php echo e((int) old('user_id', $transaction->user_id) === (int) $user->id ? 'selected' : ''); ?>><?php echo e($user->name); ?> - <?php echo e($user->phone); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="type" required <?php if($wasRefunded): echo 'disabled'; endif; ?> class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 disabled:bg-gray-100">
                        <option value="credit" <?php echo e(old('type', $transaction->type) === 'credit' ? 'selected' : ''); ?>>إيراد</option>
                        <option value="debit" <?php echo e(old('type', $transaction->type) === 'debit' ? 'selected' : ''); ?>>مصروف</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="amount" step="0.01" min="0" required value="<?php echo e(old('amount', $transaction->amount)); ?>"
                           <?php if($wasRefunded): echo 'disabled'; endif; ?>
                ...
                <div>
                    ...
                    <select name="status" required <?php if($wasRefunded): echo 'disabled'; endif; ?> class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 disabled:bg-gray-100">
                        <option value="pending" <?php echo e(old('status', $transaction->status) === 'pending' ? 'selected' : ''); ?>>معلقة</option>
                        <option value="completed" <?php echo e(old('status', $transaction->status) === 'completed' ? 'selected' : ''); ?>>مكتملة</option>
                        <option value="cancelled" <?php echo e(old('status', $transaction->status) === 'cancelled' ? 'selected' : ''); ?>>ملغاة</option>
                        <?php if($canRefund): ?>
                        <option value="reversed" <?php echo e(old('status', $transaction->status) === 'reversed' ? 'selected' : ''); ?>>مستردة (سحب من المحفظة + استرداد كامل)</option>
                        <?php endif; ?>
                    </select>
                    <?php if($canRefund): ?>
            ...
            <div>
                ...
                <textarea name="description" rows="3" <?php if($wasRefunded): echo 'disabled'; endif; ?> class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 disabled:bg-gray-100"><?php echo e(old('description', $transaction->description)); ?></textarea>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 border border-orange-200">
        ...
        <form action="<?php echo e(route('admin.transactions.refund', $transaction)); ?>" method="POST" class="space-y-4" onsubmit="return confirm('هل أنت متأكد من تنفيذ الاسترداد؟ سيتم سحب المبلغ من المحفظة المرتبطة.');">
            <?php echo csrf_field(); ?>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">مبلغ الاسترداد</label>
                    <input type="number" name="amount" step="0.01" min="0.01" max="<?php echo e($transaction->amount); ?>" value="<?php echo e(old('amount', $transaction->amount)); ?>"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                    <p class="text-xs text-gray-500 mt-1">الحد الأقصى: <?php echo e(number_format($transaction->amount, 2)); ?> ج.م</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">سبب / ملاحظات الاسترداد</label>
                    <input type="text" name="description" value="<?php echo e(old('description', 'استرداد معاملة: ' . ($transaction->transaction_number ?? $transaction->id))); ?>"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                </div>
            </div>
    ...
    <div id="wallet-sync-form" class="bg-amber-50 rounded-xl shadow-lg p-6 border-2 border-amber-400">
        ...
        <p class="text-sm text-amber-800 mb-4">
            ...
            <strong><?php echo e(number_format($syncAmount, 2)); ?> ج.م</strong>
        </p>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\mindly tics\Mindlytics\resources\views\admin\transactions\edit.blade.php ENDPATH**/ ?>