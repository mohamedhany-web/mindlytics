...
<head>
    ...
    <title>موعد العمل — <?php echo e(config('app.name')); ?></title>
...
<div class="relative z-10 min-h-screen flex flex-col items-center justify-center px-5 py-12">
    ...
    <div class="w-full max-w-md text-center">
        ...
        <h1 class="text-2xl sm:text-3xl font-black text-slate-900 mb-2"><?php echo e($title); ?></h1>

        <?php if($user): ?>
            <p class="text-slate-500 text-sm mb-1"><?php echo e($user->name); ?></p>
        <?php endif; ?>

        <p class="text-slate-600 text-sm leading-relaxed mb-8 max-w-sm mx-auto">
            <?php echo e($att['message'] ?? 'لا يمكن الوصول للنظام خارج موعد العمل.'); ?>
        ...
        <?php if($unlock): ?>
            <div class="mb-6 rounded-xl border border-teal-200 bg-teal-50 px-4 py-3 text-right text-sm text-teal-900">
                ...
                <p class="text-xs text-teal-800 mt-1.5 leading-relaxed">
                    بواسطة: <strong><?php echo e($unlock['manager_name'] ?? 'مدير المبيعات'); ?></strong>
                    · حتى <strong><?php echo e($unlock['expires_at_human'] ?? '—'); ?></strong>
                    <?php if(!empty($unlock['duration_label'])): ?>
                        (<?php echo e($unlock['duration_label']); ?>)
                    <?php endif; ?>
                </p>
                <?php if(!empty($unlock['reason'])): ?>
                    <p class="text-[11px] text-teal-700/90 mt-1">السبب: <?php echo e($unlock['reason']); ?></p>
                <?php endif; ?>
            </div>
        <?php endif; ?>

        
        <?php if($mode === 'locked_before_shift' && ($att['seconds_until_open'] ?? 0) > 0): ?>
            <div class="mb-8" x-data="countdown(<?php echo e((int) ($att['seconds_until_open'] ?? 0)); ?>)" x-init="start()">
        ...
        <?php if($att['schedule'] ?? null): ?>
            <div class="schedule-line mb-8 pb-8 border-b border-slate-200/80">
                <span><i class="fas fa-calendar-day text-sky-400 text-xs"></i> <strong><?php echo e($att['schedule']->name); ?></strong></span>
                <span><i class="fas fa-clock text-sky-400 text-xs"></i> <strong class="tabular-nums"><?php echo e($att['schedule']->timeRangeLabel()); ?></strong></span>
                <span><i class="fas fa-hourglass-half text-sky-400 text-xs"></i> <strong><?php echo e($att['schedule']->required_hours); ?> س</strong></span>
            </div>
        <?php endif; ?>

        
        <?php if(in_array($mode, ['awaiting_clock_in', 'manager_unlocked'], true) && ($att['can_clock_in'] ?? false)): ?>
            <form method="post" action="<?php echo e(route('employee.attendance.clock-in')); ?>" class="mb-6">
                <?php echo csrf_field(); ?>
        ...
        <?php if($mode === 'completed' && ($att['record'] ?? null)): ?>
            <p class="text-sm text-emerald-600 font-semibold mb-6">
                <i class="fas fa-check-circle ml-1"></i>
                <?php echo e(number_format(($att['record']->worked_minutes ?? 0) / 60, 2)); ?> ساعة عمل
            </p>
        <?php endif; ?>
...
<?php /**PATH C:\xampp\htdocs\mindly tics\Mindlytics\resources\views\employee\attendance\locked.blade.php ENDPATH**/ ?>