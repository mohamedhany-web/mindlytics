...
<?php $__env->startSection('header', 'تعبئة التقرير اليومي — '.$date->format('Y-m-d')); ?>
...
<div class="space-y-4" x-data="dailyReportForm(<?php echo \Illuminate\Support\Js::from($existingContacts)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($leads->map(fn($l) => ['id' => (string)$l->id, 'name' => $l->name, 'phone' => $l->phone, 'stage' => $l->stage])->values())->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($todayLeadsJson)->toHtml() ?>)">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h2 class="text-xl font-bold text-slate-900">تقرير <?php echo e($date->format('Y-m-d')); ?></h2>
            <p class="text-sm text-slate-500">آخر موعد للتسليم: <?php echo e($settings['deadline_time'] ?? '23:59'); ?></p>
        </div>
        <div class="flex flex-wrap gap-2">
            <a href="<?php echo e(route('employee.sales.daily-reports.index', ['date' => $date->toDateString()])); ?>" class="px-4 py-2 text-sm border border-slate-200 rounded-lg text-slate-700">العودة</a>
            <form method="post" action="<?php echo e(route('employee.sales.daily-reports.sync-auto')); ?>">
                <?php echo csrf_field(); ?>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-4 items-start">
        ...
        <aside class="xl:col-span-3 space-y-4 xl:sticky xl:top-4">
            <div class="dr-panel p-4">
                ...
                <p class="text-2xl font-black <?php echo e($kpiClass); ?>"><?php echo e($kpi['overall_pct'] ?? 0); ?>%</p>
                <p class="text-xs font-semibold <?php echo e($kpiClass); ?> mt-1"><?php echo e($kpi['status_label'] ?? ''); ?></p>
                <ul class="mt-4 space-y-2 text-xs">
                    <?php $__currentLoopData = $kpi['lines'] ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $line): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $lc = match($line['status']){ 'met'=>'text-emerald-700','near'=>'text-amber-700',default=>'text-rose-700' }; ?>
                        <li class="flex justify-between gap-2 border-b border-slate-100 pb-1">
                            <span class="text-slate-600"><?php echo e($line['label']); ?></span>
                            <span class="font-bold <?php echo e($lc); ?>"><?php echo e($line['actual']); ?>/<?php echo e($line['target']); ?> (<?php echo e($line['pct']); ?>%)</span>
                        </li>
        ...
        <div class="xl:col-span-9">
            <form method="post" action="<?php echo e(route('employee.sales.daily-reports.store')); ?>" class="space-y-4">
                <?php echo csrf_field(); ?>
                ...
                <section class="dr-panel dr-auto p-5">
                    ...
                    <div class="grid sm:grid-cols-3 gap-3 mt-3">
                        <?php $__currentLoopData = ['messages_replied' => 'ردود رسائل', 'leads_qualified' => 'عملاء مؤهّلون', 'bookings_from_leads' => 'حجوزات']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1"><?php echo e($label); ?></label>
                                <input type="number" min="0" name="<?php echo e($key); ?>" value="<?php echo e(old($key, $r?->$key ?? 0)); ?>" required class="dr-input px-3 py-2 text-sm">
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                    <div class="mt-3 dr-manual rounded-lg border p-3">
                        ...
                        <textarea name="activity_notes" rows="3" class="dr-input px-3 py-2 text-sm" placeholder="مثال: عميل طلب عرض خاص، متابعة مع الإدارة…"><?php echo e(old('activity_notes', $r?->activity_notes)); ?></textarea>
                    </div>
                </section>
                ...
                <section class="dr-panel dr-auto p-5">
                    ...
                    <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-3 mt-3">
                        <?php $__currentLoopData = ['numbers_worked' => 'أرقام', 'followups_done' => 'متابعات', 'calls_made' => 'مكالمات', 'meetings_held' => 'اجتماعات', 'calls_answered' => 'ردود مكالمات']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1"><?php echo e($label); ?></label>
                                <input type="number" min="0" name="<?php echo e($key); ?>" value="<?php echo e(old($key, $r?->$key ?? 0)); ?>" required class="dr-input px-3 py-2 text-sm">
                            </div>
                    ...
                    <div class="mt-3 dr-manual rounded-lg border p-3">
                        ...
                        <textarea name="productivity_notes" rows="2" class="dr-input px-3 py-2 text-sm"><?php echo e(old('productivity_notes', $r?->productivity_notes)); ?></textarea>
                    </div>
                </section>
                    ...
                    <div class="mb-4 rounded-lg border border-sky-200 bg-sky-50/60 p-4">
                        <div class="flex flex-wrap items-center justify-between gap-2 mb-2">
                            <p class="text-sm font-bold text-sky-900">
                                ...
                                عملاء تواصلت معهم اليوم (<?php echo e(count($todayLeads ?? [])); ?>)
                            </p>
...
<?php echo $__env->make('layouts.employee', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\mindly tics\Mindlytics\resources\views\employee\sales\daily-reports\edit.blade.php ENDPATH**/ ?>