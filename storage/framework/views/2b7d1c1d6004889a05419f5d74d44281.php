        ...
        <aside class="lg:col-span-4 space-y-4 order-2 lg:sticky lg:top-4">
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 sm:p-6 space-y-4">
                <div class="flex flex-wrap gap-2 items-center">
                    ...
                    ?>
                    <span class="px-3 py-1 rounded-full text-xs font-bold <?php echo e($prClass); ?>"><?php echo e(\App\Models\SalesLead::priorityLabel($pr)); ?></span>
                    <span class="text-sm text-gray-500"><?php echo e(\App\Models\SalesLead::sourceLabel($lead->source)); ?></span>
                    <?php if($lead->category): ?>
                        <span class="px-3 py-1 rounded-full text-xs font-bold" style="color: <?php echo e($lead->category->color); ?>; background: <?php echo e($lead->category->color); ?>18"><?php echo e($lead->category->name); ?></span>
                    <?php endif; ?>
                    <?php if($lead->import_batch): ?>
                        <span class="text-xs text-gray-400">دفعة: <?php echo e($lead->import_batch); ?></span>
                    <?php endif; ?>
                </div>
                ...
                <dl class="space-y-3 text-sm border-t border-gray-100 pt-4">
                    ...
                    <div>
                        ...
                        <dd class="font-semibold <?php if($lead->isFollowUpOverdue()): ?> text-rose-600 <?php else: ?> text-gray-900 <?php endif; ?>"><?php echo e($lead->next_follow_up_at?->format('Y-m-d H:i') ?? '—'); ?></dd>
                    </div>
                    <div>
                ...
                <?php if($lead->stage === 'won'): ?>
                    <div class="rounded-xl border border-amber-200 bg-amber-50/90 p-4 space-y-3">
                        ...
                        <?php if($lead->csat_rating): ?>
                            <p class="text-sm text-gray-800">التقييم الحالي: <strong><?php echo e($lead->csat_rating); ?>/5</strong><?php if($lead->csat_recorded_at): ?> — <?php echo e($lead->csat_recorded_at->format('Y-m-d')); ?><?php endif; ?></p>
                            <?php if($lead->csat_comment): ?><p class="text-xs text-gray-600 whitespace-pre-wrap"><?php echo e($lead->csat_comment); ?></p><?php endif; ?>
                        <?php endif; ?>
                        <form method="post" action="<?php echo e(route('employee.sales.leads.csat.store', $lead)); ?>" class="space-y-2">
                            <?php echo csrf_field(); ?>
...
unset($__errorArgs, $__bag); ?>
                            <div>
                                ...
                                <select name="csat_rating" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                    <?php for($i = 5; $i >= 1; $i--): ?>
                                        <option value="<?php echo e($i); ?>" <?php if((int) old('csat_rating', $lead->csat_rating ?? 5) === $i): echo 'selected'; endif; ?>><?php echo e($i); ?></option>
                                    <?php endfor; ?>
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 mb-1">ملاحظة</label>
                                <textarea name="csat_comment" rows="2" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"><?php echo e(old('csat_comment', $lead->csat_comment)); ?></textarea>
                            </div>
                ...
                <?php if($lead->isOpen() && ($lead->isFollowUpOverdue() || $lead->isStaleContact())): ?>
                    <div class="rounded-xl border border-rose-200 bg-rose-50/80 p-3 text-sm text-rose-900">
                        ...
                        <?php if($lead->isStaleContact()): ?>
                            <p class="font-semibold mt-1"><i class="fas fa-hourglass-end ml-1"></i> لم يُسجَّل تواصل خلال <?php echo e(\App\Models\SalesLead::STALE_CONTACT_DAYS); ?> أيام — سجّل نشاطاً.</p>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
...
<?php echo $__env->make('layouts.employee', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/cityphone/Documents/all Mindlytics Project/Mindlytics/resources/views/employee/sales/leads/show.blade.php ENDPATH**/ ?>