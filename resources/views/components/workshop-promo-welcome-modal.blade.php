...
>
    <div class="workshop-promo-confetti" aria-hidden="true">
        @foreach(range(1, 24) as $i)
            <span class="workshop-promo-confetti-piece" style="--i: {{ $i }}; --hue: {{ ($i * 37) % 360 }};"></span>
        @endforeach
    </div>
        ...
        <div class="p-6 sm:p-8 text-center">
            ...
            <div class="rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-sky-50 p-5 mb-6 text-right">
                ...
                @if($code)
                    ...
                    <p class="font-mono text-2xl font-black text-violet-700 tracking-wide mb-1">{{ $code }}</p>
                ...
                @if($discount)
                    <div class="inline-flex flex-col items-center gap-1 rounded-xl bg-white border border-slate-200 px-4 py-3 shadow-sm w-full sm:w-auto">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xl font-black text-sky-600">{{ $discount }}</span>
                        </div>
                        <span class="text-[11px] font-bold text-slate-500">{{ $discountQuip }}</span>
                    </div>
                @endif
                @if($workshop)
                    <p class="text-xs text-slate-600 mt-3">
                        <i class="fas fa-chalkboard-teacher text-violet-500 ml-1"></i>
                        الورشة اللي خلّتك كده: <span class="font-semibold text-slate-800">{{ $workshop }}</span>
                    </p>
                @endif
                @if($expires && $expires !== 'بدون انتهاء')
                    <p class="text-[11px] text-slate-500 mt-2">
                        <i class="fas fa-clock text-slate-400 ml-1"></i>
                        الخصم صالح لحد {{ $expires }} — متسيبوش يضيع
                    </p>
                @endif
            </div>
