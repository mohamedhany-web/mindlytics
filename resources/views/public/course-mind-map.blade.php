...
@section('title', __('public.course_mind_map_page_title', ['course' => $course->localized('title') ?? __('public.course_fallback')]) . ' - ' . __('public.site_suffix'))
...
@section('content')
<div class="mind-zigzag-wrap pt-24 pb-20" id="mind-map-scene" data-step-total="{{ $stepCount }}" data-progress-label="{{ e(__('public.course_mind_map_progress')) }}">
    ...
    <svg class="mind-zigzag-svg" viewBox="0 0 100 400" preserveAspectRatio="none" aria-hidden="true">
        <path fill="none" stroke="url(#{{ $gradId }}Bg)" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"
              d="M50 0 L72 45 L28 90 L72 135 L28 180 L72 225 L28 270 L72 315 L50 360" opacity="0.5"/>
        <defs>
            <linearGradient id="{{ $gradId }}Bg" x1="0%" y1="0%" x2="100%" y2="0%">
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <nav class="mb-6 text-slate-600 text-sm flex flex-wrap items-center gap-1">
            ...
            <span class="text-slate-400">/</span>
            <a href="{{ route('public.course.show', $course->id) }}" class="hover:text-blue-600 transition-colors">{{ Str::limit($course->localized('title') ?? __('public.course_fallback'), 40) }}</a>
            <span class="text-slate-400">/</span>
            <span class="text-slate-900 font-semibold">{{ __('public.course_mind_map_short') }}</span>
        </nav>
        ...
        <div class="text-center max-w-2xl mx-auto mb-10 md:mb-12">
            <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/80 border border-emerald-200/80 text-emerald-800 text-xs font-bold shadow-sm mb-4">
                ...
                {{ __('public.course_mind_map_badge') }}
            </div>
            <h1 class="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 mb-3 leading-tight tracking-tight">
                {{ __('public.course_mind_map_heading') }}
            </h1>
            <p class="text-lg sm:text-xl text-slate-700 font-bold">{{ $course->localized('title') ?? __('public.course_title_fallback') }}</p>
            <p class="text-slate-500 text-sm mt-3 leading-relaxed">{{ __('public.course_mind_map_intro') }}</p>
            <p class="text-slate-400 text-xs mt-2 flex items-center justify-center gap-2">
                ...
                {{ __('public.course_mind_map_scroll_hint') }}
            </p>
        </div>

        @php $timetableBody = trim((string) ($course->mind_map_timetable ?? '')); @endphp
        @if($timetableBody !== '')
            <div class="max-w-4xl mx-auto mb-10 md:mb-12">
                <div class="rounded-2xl border border-amber-200/90 bg-gradient-to-br from-amber-50/95 via-white to-orange-50/80 p-5 sm:p-6 shadow-md">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h2 class="text-lg sm:text-xl font-black text-slate-900">{{ __('public.course_mind_map_timetable_title') }}</h2>
                            <p class="text-xs text-amber-900/70 font-medium">{{ __('public.course_mind_map_timetable_sub') }}</p>
                        </div>
                    </div>
                    <div class="rounded-xl bg-white/90 border border-amber-100 px-4 py-4 text-slate-800 text-sm sm:text-base leading-relaxed whitespace-pre-line">{{ $timetableBody }}</div>
                </div>
            </div>
        @endif
        ...
        <div class="relative max-w-4xl mx-auto" id="mind-track">
            ...
            <svg class="mind-path-svg hidden md:block" viewBox="0 0 100 1000" preserveAspectRatio="none" aria-hidden="true">
                ...
                <path id="{{ $pathId }}" d="M50 0 C58 80 42 160 50 240 C58 320 42 400 50 480 C58 560 42 640 50 720 C58 800 42 880 50 960 L50 1000"/>
            </svg>

            <div class="mind-progress-wrap hidden sm:block max-w-xl mx-auto px-2">
                <div class="mind-progress-glow" role="progressbar" aria-valuemin="0" aria-valuemax="{{ $stepCount }}" aria-valuenow="0" id="mind-progress-bar-wrap">
            ...
            <ol class="relative z-10 list-none p-0 m-0 space-y-2 md:space-y-0" id="mind-steps-list">
                @foreach($steps as $index => $step)
                    ...
                    @endphp
                    <li class="mind-row" data-mind-index="{{ $index }}">
                        <div class="md:hidden flex gap-4 items-start ps-1 border-s-4 {{ $onStartSide ? 'border-s-emerald-400' : 'border-s-indigo-400' }} rounded-s-xl ps-3 -ms-1">
                            <div class="flex-shrink-0 pt-1">
                                ...
                                @else
                                    <div class="mind-node bg-white text-sky-700 border-2 border-sky-200 ring-4 ring-sky-50">{{ $index }}</div>
                                @endif
                            </div>
                            <div class="mind-card flex-1 p-4 sm:p-5 {{ $onStartSide ? 'mind-card--tilt-r' : 'mind-card--tilt-l' }}">
                                @include('public.partials.course-mind-map-card-inner', [
                                    'isFirst' => $isFirst,
                                    'isLast' => $isLast,
                                    'index' => $index,
                                    'step' => $step,
                        ...
                        <div class="hidden md:grid md:grid-cols-[1fr_auto_1fr] md:gap-x-6 md:gap-y-0 md:items-center md:min-h-[9rem] relative">
                            @if($onStartSide)
                                ...
                                <div class="flex justify-center py-2">
                                    ...
                                    @else
                                        <div class="mind-node w-14 h-14 text-lg bg-white text-sky-700 border-2 border-sky-200 ring-4 ring-sky-50">{{ $index }}</div>
                                    @endif
                                </div>
                                <div></div>
                            @else
