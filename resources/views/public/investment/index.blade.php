    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="stat-card p-5 text-center">
                <div class="text-3xl font-black text-blue-600 mb-1 tabular-nums">{{ $plans->count() }}</div>
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-14">
            <article class="content-panel h-full">
                ...
                <div class="p-6">
                    <ul class="req-list text-sm">
                        @foreach($policyLines($policy->eligibility_rules) as $line)
                            <li>{{ $line }}</li>
                        @endforeach
                    </ul>
                </div>
            ...
            <article class="content-panel h-full">
                ...
                <div class="p-6 space-y-3">
                    @foreach($policyLines($policy->process_description) as $i => $line)
                        <div class="step-item">
                            <span class="step-num">{{ $i + 1 }}</span>
                            <p class="text-sm text-slate-700 leading-relaxed pt-0.5">{{ preg_replace('/^\d+\.\s*/', '', $line) }}</p>
                        </div>
                    @endforeach
                </div>
            </article>
            ...
            <article class="content-panel h-full md:col-span-2 lg:col-span-1">
                ...
                <div class="p-6">
                    <ul class="req-list text-sm">
                        @foreach($policyLines($policy->legal_framework) as $line)
                            <li>{{ $line }}</li>
                        @endforeach
                    </ul>
                </div>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            @forelse($plans as $plan)
                <a href="{{ route('investment.show', $plan->slug) }}" class="job-card p-6 group">
                    <div class="flex items-start justify-between gap-4 mb-4">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-xs font-bold text-sky-600 mt-1">{{ $plan->planTypeLabel() }}</p>
                        </div>
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        <span class="job-meta-chip blue"><i class="fas fa-coins text-[10px]"></i>{{ $plan->formattedMinInvestment() }}</span>
                        <span class="job-meta-chip green"><i class="fas fa-shield-alt text-[10px]"></i>مخاطر {{ $plan->riskLevelLabel() }}</span>
                        @if($plan->duration_months)
                            <span class="job-meta-chip violet"><i class="fas fa-calendar text-[10px]"></i>{{ $plan->duration_months }} شهر</span>
                        @endif
                    </div>

                    @if($plan->short_description)
                        <p class="text-sm text-slate-600 leading-relaxed line-clamp-3 flex-1">{{ $plan->short_description }}</p>
                    @endif

                    @if($plan->expected_return_min)
                        <p class="text-xs text-slate-500 mt-3 font-semibold">
                            عائد متوقع: {{ $plan->expected_return_min }}% — {{ $plan->expected_return_max }}%
                        </p>
                    @endif
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
        ...
        @if($email || $phone)
            <div class="mt-8 grid sm:grid-cols-2 gap-4">
                @if($email)
                    <a href="mailto:{{ $email }}" class="info-tile group no-underline text-inherit">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-extrabold text-slate-900 dir-ltr">{{ $email }}</p>
                        </div>
                    </a>
                @endif
                @if($phone)
                    <a href="tel:{{ $phone }}" class="info-tile group no-underline text-inherit">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-extrabold text-slate-900 dir-ltr">{{ $phone }}</p>
                        </div>
                    </a>
                @endif
            </div>
