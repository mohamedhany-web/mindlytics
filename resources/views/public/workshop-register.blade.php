...
<head>
    ...
    <title>التسجيل في الورشة - {{ $workshop->title }} | Mindlytics</title>
                ...
                <div class="space-y-3 text-sm text-slate-200">
                    @if($workshop->starts_at)
                        <div class="flex items-center gap-2">
                            ...
                            <span>تاريخ البداية: {{ $workshop->starts_at->format('Y-m-d H:i') }}</span>
                        </div>
                    @endif
                    @if($workshop->ends_at)
                        <div class="flex items-center gap-2">
                            <i class="fas fa-clock text-blue-400"></i>
                            <span>تاريخ النهاية: {{ $workshop->ends_at->format('Y-m-d H:i') }}</span>
                        </div>
                    @endif
                    @php
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if($total)
                            <span>المقاعد: {{ $registeredCount }} / {{ $total }} @if(!is_null($remaining)) (متبقي {{ $remaining }}) @endif</span>
                        @else
                    ...
                    <form method="POST" action="{{ route('public.workshops.register', $workshop->slug) }}" class="space-y-4">
                        ...
                        <div class="space-y-1">
                            ...
                            <textarea id="notes" name="notes" rows="3"
                                      class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500"
                                      placeholder="مثلاً: مستواك الحالي، الأمور التي تود التركيز عليها في الورشة، أو أي متطلبات خاصة.">{{ old('notes') }}</textarea>
                        </div>
                        <button type="submit"
