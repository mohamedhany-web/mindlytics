...
<body class="bg-gradient-to-br from-slate-50 via-emerald-50/30 to-sky-50 min-h-screen text-slate-900">
    <div class="max-w-3xl mx-auto px-4 py-8 sm:py-12">
        <header class="text-center mb-8">
            <p class="text-sm font-semibold text-emerald-700 tracking-wide uppercase mb-2">Mindlytics Academy — توصيف دبلومة</p>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50 overflow-hidden">
            <div class="grid sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x sm:divide-x-reverse divide-slate-100">
                @foreach([
                    ...
                    ['icon' => 'fa-calendar-day', 'label' => 'البداية', 'value' => $entry->startDisplay()],
            ...
            @if($highlights !== [])
                <div class="px-6 py-6 border-t border-slate-100">
                    <h2 class="font-black text-lg mb-4">تفاصيل الدبلومة</h2>
                    <ul class="space-y-2">
            ...
            @php $bookingMethods = $entry->bookingMethodList(); @endphp
            @if($bookingMethods !== [])
                <div class="px-6 py-6 border-t border-slate-100 bg-sky-50/40">
                    <h2 class="font-black text-lg mb-4">طرق الحجز</h2>
                    <ul class="space-y-3">
                        @foreach($bookingMethods as $method)
                            <li class="flex gap-3 items-start text-slate-800">
                                <span class="w-8 h-8 rounded-lg bg-sky-100 text-sky-700 flex items-center justify-center shrink-0">
                                    <i class="fas fa-credit-card text-sm"></i>
                                </span>
                                <span class="font-semibold leading-relaxed pt-1">{{ $method }}</span>
                            </li>
                        @endforeach
                    </ul>
                </div>
            ...
            @php $freeLectures = $entry->freeLectureLinkList(); @endphp
            @if($freeLectures !== [])
                <div class="px-6 py-6 border-t border-slate-100">
                    <h2 class="font-black text-lg mb-4">محاضرات مجانية</h2>
                    <div class="space-y-3">
                        @foreach($freeLectures as $lecture)
                            <a href="{{ $lecture['url'] }}" target="_blank" rel="noopener noreferrer"
                               class="flex items-center justify-between gap-3 p-4 rounded-2xl border border-emerald-200 bg-emerald-50/70 hover:bg-emerald-100 transition-colors text-emerald-900 no-underline">
                                <span class="flex items-center gap-3 min-w-0">
                                    <span class="w-10 h-10 rounded-xl bg-white text-emerald-700 flex items-center justify-center shrink-0 shadow-sm">
                                        <i class="fas fa-play"></i>
                                    </span>
                                    <span class="font-bold truncate">{{ $lecture['title'] }}</span>
                                </span>
                                <i class="fas fa-external-link-alt text-emerald-600 shrink-0"></i>
                            </a>
                        @endforeach
                    </div>
                </div>
