...
<div class="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6" style="background: #f8fafc; min-height: 100vh;">
        ...
        <div class="p-5 sm:p-6">
            <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <p class="font-bold text-slate-900 mt-1">{{ $displayPhone }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <p class="text-xs text-slate-500">الاسم المعتمد</p>
                    <p class="font-bold text-slate-900 mt-1">{{ $displayName }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <p class="text-xs text-slate-500">Phone Number ID</p>
                    <p class="font-mono text-xs text-slate-800 break-all mt-1">{{ $savedPhoneId !== '' ? $savedPhoneId : '—' }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <p class="text-xs text-slate-500">WABA ID</p>
                    <p class="font-mono text-xs text-slate-800 break-all mt-1">{{ $savedWaba !== '' ? $savedWaba : '—' }}</p>
                </div>
            </div>
            ...
            @if($hasSecret || $hasToken || $savedAppId !== '' || $savedPhoneId !== '')
    ...
    <section class="{{ $waSectionClass }}">
        ...
        <div class="p-5 sm:p-6">
            <form method="POST" action="{{ route('admin.whatsapp.settings.update') }}" class="space-y-5" id="wa-settings-form">
                @csrf
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="password" name="access_token" value=""
                                   autocomplete="new-password" class="{{ $waInputClass }} dir-ltr font-mono text-sm"
                                   placeholder="{{ $hasToken ? 'محفوظ — اتركه فارغاً للإبقاء' : 'System User Token من Meta' }}">
                        </div>
                        <div>
                            <label class="{{ $waLabelClass }}">Phone Number ID</label>
                            <input type="text" name="phone_number_id" value="{{ $savedPhoneId }}"
                                   class="{{ $waInputClass }} dir-ltr font-mono text-sm" placeholder="من WhatsApp → API Setup">
                        </div>
                        <div>
                            <label class="{{ $waLabelClass }}">WhatsApp Business Account ID (WABA)</label>
                            <input type="text" name="business_account_id" value="{{ $savedWaba }}"
                                   class="{{ $waInputClass }} dir-ltr font-mono text-sm" placeholder="اختياري — للـ Webhook">
                        </div>
                        <div class="flex items-end">
                            <label class="inline-flex items-center gap-2 text-sm font-semibold text-slate-700 cursor-pointer px-4 py-2.5 rounded-xl border-2 {{ $savedEnabled ? 'border-emerald-300 bg-emerald-50' : 'border-slate-200 bg-white' }} w-full">
                                <input type="checkbox" name="enable_service" value="1" @checked($savedEnabled) class="rounded text-emerald-600">
    ...
    <section class="{{ $waSectionClass }}">
        ...
        <div class="p-5 sm:p-6 space-y-3">
            <div class="grid sm:grid-cols-2 gap-3">
                <div>
                    <label class="{{ $waLabelClass }}">رقم الاختبار</label>
                    <input type="text" id="test-phone" placeholder="2010xxxxxxx" class="{{ $waInputClass }} text-sm dir-ltr">
                </div>
                <div>
                    <label class="{{ $waLabelClass }}">نص الرسالة (اختياري)</label>
                    <input type="text" id="test-message" placeholder="رسالة الاختبار" class="{{ $waInputClass }} text-sm">
                </div>
            </div>
