...
<div class="space-y-6">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            ...
            <select name="workshop_id" class="w-full rounded-xl border border-slate-200 px-4 py-3 focus:ring-2 focus:ring-violet-500">
                ...
                @foreach($workshops as $ws)
                    <option value="{{ $ws->id }}" @selected(old('workshop_id', $workshopPromoCode->workshop_id ?? '') == $ws->id)>
                        {{ $ws->title }} @if($ws->starts_at) ({{ $ws->starts_at->format('Y-m-d') }}) @endif
                    </option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <input type="text" name="code" required value="{{ old('code', $workshopPromoCode->code ?? \App\Models\WorkshopPromoCode::generateCode()) }}"
        ...
        <div class="md:col-span-2">
            ...
            <input type="text" name="title" required value="{{ old('title', $workshopPromoCode->title ?? '') }}"
        ...
        <div>
            ...
            <select name="discount_type" class="w-full rounded-xl border border-slate-200 px-4 py-3 focus:ring-2 focus:ring-violet-500">
                <option value="percentage" @selected(old('discount_type', $workshopPromoCode->discount_type ?? 'percentage') === 'percentage')>نسبة مئوية</option>
                <option value="fixed" @selected(old('discount_type', $workshopPromoCode->discount_type ?? '') === 'fixed')>مبلغ ثابت</option>
            </select>
        </div>
        <div>
            ...
            <input type="number" name="discount_value" step="0.01" min="0" required
                   value="{{ old('discount_value', $workshopPromoCode->discount_value ?? '') }}"
                   class="w-full rounded-xl border border-slate-200 px-4 py-3 focus:ring-2 focus:ring-violet-500">
        </div>
        <div>
            ...
            <input type="number" name="maximum_discount" step="0.01" min="0"
                   value="{{ old('maximum_discount', $workshopPromoCode->maximum_discount ?? '') }}"
                   class="w-full rounded-xl border border-slate-200 px-4 py-3 focus:ring-2 focus:ring-violet-500">
        </div>
        <div>
            ...
            <input type="number" name="minimum_order_amount" step="0.01" min="0"
                   value="{{ old('minimum_order_amount', $workshopPromoCode->minimum_order_amount ?? '') }}"
                   class="w-full rounded-xl border border-slate-200 px-4 py-3 focus:ring-2 focus:ring-violet-500">
        </div>
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-2">تاريخ البداية</label>
            <input type="date" name="starts_at" value="{{ old('starts_at', isset($workshopPromoCode) && $workshopPromoCode->starts_at ? $workshopPromoCode->starts_at->format('Y-m-d') : '') }}"
                   class="w-full rounded-xl border border-slate-200 px-4 py-3 focus:ring-2 focus:ring-violet-500">
        </div>
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-2">تاريخ الانتهاء *</label>
            <input type="date" name="expires_at" value="{{ old('expires_at', isset($workshopPromoCode) && $workshopPromoCode->expires_at ? $workshopPromoCode->expires_at->format('Y-m-d') : '') }}"
                   class="w-full rounded-xl border border-slate-200 px-4 py-3 focus:ring-2 focus:ring-violet-500">
        ...
        <div>
            ...
            <input type="number" name="max_activations" min="1"
                   value="{{ old('max_activations', $workshopPromoCode->max_activations ?? '') }}"
    ...
    <div>
        ...
        <textarea name="description" rows="2" class="w-full rounded-xl border border-slate-200 px-4 py-3 focus:ring-2 focus:ring-violet-500">{{ old('description', $workshopPromoCode->description ?? '') }}</textarea>
    </div>
    ...
    <div class="rounded-xl border border-violet-100 bg-violet-50/50 p-4">
        ...
        <div class="flex flex-wrap gap-4">
            <label class="inline-flex items-center gap-2">
                <input type="checkbox" name="applies_to_online" value="1" @checked(old('applies_to_online', $workshopPromoCode->applies_to_online ?? true)) class="rounded border-violet-300 text-violet-600">
                <span class="text-sm">أونلاين (لايف)</span>
            </label>
            <label class="inline-flex items-center gap-2">
                <input type="checkbox" name="applies_to_offline" value="1" @checked(old('applies_to_offline', $workshopPromoCode->applies_to_offline ?? true)) class="rounded border-violet-300 text-violet-600">
                <span class="text-sm">أوفلاين (حضور)</span>
            </label>
            <label class="inline-flex items-center gap-2">
                <input type="checkbox" name="applies_to_recorded" value="1" @checked(old('applies_to_recorded', $workshopPromoCode->applies_to_recorded ?? true)) class="rounded border-violet-300 text-violet-600">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
            ...
            <select name="applicable_advanced_course_ids[]" multiple size="6"
                ...
                @php $selAdv = old('applicable_advanced_course_ids', $workshopPromoCode->applicable_advanced_course_ids ?? []); @endphp
                @foreach($advancedCourses as $c)
                    <option value="{{ $c->id }}" @selected(in_array($c->id, $selAdv ?? []))>{{ $c->title }} — {{ number_format($c->price, 0) }} ج.م</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="applicable_offline_course_ids[]" multiple size="6"
                    class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-violet-500">
                @php $selOff = old('applicable_offline_course_ids', $workshopPromoCode->applicable_offline_course_ids ?? []); @endphp
                @foreach($offlineCourses as $c)
                    <option value="{{ $c->id }}" @selected(in_array($c->id, $selOff ?? []))>{{ $c->title }} — {{ number_format($c->price, 0) }} ج.م</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <label class="inline-flex items-center gap-2">
        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $workshopPromoCode->is_active ?? true)) class="rounded border-slate-300 text-violet-600">
