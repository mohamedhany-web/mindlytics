@php
    ...
    $bookingMethodsText = old('booking_methods_text', isset($entry) && is_array($entry->booking_methods) ? implode("\n", $entry->booking_methods) : '');
    $freeLinksText = old('free_lecture_links_text', '');
    if ($freeLinksText === '' && isset($entry) && is_array($entry->free_lecture_links)) {
        $freeLinksText = collect($entry->free_lecture_links)->map(function ($item) {
            if (is_array($item)) {
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    <div>
        <label class="block text-xs font-semibold text-slate-500 mb-1">اسم الدبلومة *</label>
    ...
    <div>
        ...
        <input type="text" name="slug" value="{{ old('slug', $entry->slug) }}" dir="ltr"
               class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/30"
               placeholder="data-science-diploma">
        <p class="text-xs text-slate-400 mt-1">اتركه فارغاً للتوليد التلقائي — الرابط: /diploma-info/{slug}</p>
    </div>
    <div>
    ...
    <div>
        <label class="block text-xs font-semibold text-slate-500 mb-1">تاريخ بداية الدبلومة</label>
        <input type="date" name="starts_at" value="{{ old('starts_at', optional($entry->starts_at)->format('Y-m-d')) }}"
               class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm">
    </div>
    <div>
        <label class="block text-xs font-semibold text-slate-500 mb-1">موعد البداية (نص بديل)</label>
        <input type="text" name="start_label" value="{{ old('start_label', $entry->start_label) }}"
               class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm" placeholder="بداية أكتوبر / Start of October">
    </div>
    <div>
    ...
    <div>
        ...
        <input type="text" name="format" value="{{ old('format', $entry->format) }}"
               class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm" placeholder="Online / Offline / Hybrid">
    </div>
    <div>
    ...
    <div class="md:col-span-2">
        <label class="block text-xs font-semibold text-slate-500 mb-1">ربط بمسار تعليمي / دبلوم LMS (اختياري)</label>
        <select name="academic_year_id" class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm">
            <option value="">— بدون ربط —</option>
            @foreach($paths as $path)
                <option value="{{ $path->id }}" @selected((string) old('academic_year_id', $entry->academic_year_id) === (string) $path->id)>
                    {{ $path->name }}@if($path->code) ({{ $path->code }})@endif
                </option>
            @endforeach
        </select>
    </div>
    ...
    <div class="md:col-span-2">
        <label class="block text-xs font-semibold text-slate-500 mb-1">طرق الحجز (سطر لكل طريقة)</label>
        <textarea name="booking_methods_text" rows="4" class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm" placeholder="Instapay: 01xxxxxxxxx&#10;واتساب المبيعات: 01xxxxxxxxx&#10;تحويل بنكي: حساب …">{{ $bookingMethodsText }}</textarea>
        <p class="text-xs text-slate-400 mt-1">تظهر في صفحة التوصيف تحت قسم «طرق الحجز»</p>
    </div>
    <div class="md:col-span-2">
        <label class="block text-xs font-semibold text-slate-500 mb-1">محاضرات مجانية (روابط خارجية)</label>
        <textarea name="free_lecture_links_text" rows="4" dir="ltr" class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm font-mono" placeholder="مقدمة مجانية | https://youtube.com/...&#10;https://drive.google.com/...">{{ $freeLinksText }}</textarea>
        <p class="text-xs text-slate-400 mt-1">سطر لكل رابط: العنوان | الرابط — أو الرابط فقط. يفتح في تبويب جديد.</p>
    </div>
    <div class="md:col-span-2">
