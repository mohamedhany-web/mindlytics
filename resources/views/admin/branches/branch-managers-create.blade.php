...
<div class="space-y-6 pb-16">
    ...
    @if($branches->isEmpty())
        <div class="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-5 text-amber-900 text-sm">
            لا يوجد فرع في النظام بعد. <a href="{{ route('admin.branches.create') }}" class="font-bold text-blue-700 underline">أنشئ فرعاً أولاً</a>.
        </div>
    @else
        <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
            <form action="{{ route('admin.branch-managers.store') }}" method="POST" class="p-6 sm:p-8 space-y-6">
                @csrf
                        ...
                        <select name="branch_id" id="branch_id" required
                            ...
                            @foreach($branches as $b)
                                <option value="{{ $b->id }}" {{ (string) old('branch_id', '') === (string) $b->id ? 'selected' : '' }}>
                                    {{ $b->name }} ({{ $b->slug }}){{ $b->is_active ? '' : ' — موقوف' }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-6 space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email') }}" required dir="ltr" autocomplete="off"
                                   class="w-full rounded-xl border-2 border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all">
                ...
                <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 pt-2">
                    <a href="{{ route('admin.branches.index') }}" class="inline-flex items-center justify-center rounded-xl border-2 border-slate-300 px-6 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
                        إلغاء
                    </a>
