...
@section('title', 'إضافة خصم موظف - Mindlytics')
@section('header', 'إضافة خصم موظف')
...
<div class="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6" style="background: #f8fafc; min-height: 100vh;">
    <section class="rounded-2xl bg-white/95 backdrop-blur border-2 border-slate-200/50 shadow-xl overflow-hidden">
        ...
        @endif
        <form method="POST" action="{{ route('admin.employee-deductions.store') }}" class="px-5 py-6 sm:px-8 lg:px-12">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="employee_id" required class="w-full rounded-xl border-2 border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-rose-500 focus:border-rose-400">
                        ...
                        @foreach($employees as $emp)
                            <option value="{{ $emp->id }}" {{ old('employee_id') == $emp->id ? 'selected' : '' }}>{{ $emp->name }} @if($emp->email)({{ $emp->email }})@endif</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="agreement_id" class="w-full rounded-xl border-2 border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-rose-500 focus:border-rose-400">
                        ...
                        @foreach($agreements as $agr)
                            <option value="{{ $agr->id }}" {{ old('agreement_id') == $agr->id ? 'selected' : '' }}>{{ $agr->agreement_number }} — {{ $agr->employee->name ?? '' }}</option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required class="w-full rounded-xl border-2 border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-rose-500 focus:border-rose-400" placeholder="مثال: خصم تأمين اجتماعي" />
                    @error('title')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">الوصف (اختياري)</label>
                    <textarea name="description" rows="2" class="w-full rounded-xl border-2 border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-rose-500 focus:border-rose-400" placeholder="تفاصيل إضافية...">{{ old('description') }}</textarea>
                    @error('description')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">المبلغ (ج.م) <span class="text-red-500">*</span></label>
                    <input type="number" name="amount" step="0.01" min="0.01" value="{{ old('amount') }}" required class="w-full rounded-xl border-2 border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-rose-500 focus:border-rose-400" placeholder="0.00" />
                ...
                <div>
                    ...
                    <select name="type" required class="w-full rounded-xl border-2 border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-rose-500 focus:border-rose-400">
                        <option value="tax" {{ old('type') == 'tax' ? 'selected' : '' }}>ضريبة</option>
                        <option value="insurance" {{ old('type', 'other') == 'insurance' ? 'selected' : '' }}>تأمين</option>
                        <option value="loan" {{ old('type') == 'loan' ? 'selected' : '' }}>قرض</option>
                ...
                <div>
                    ...
                    <input type="date" name="deduction_date" value="{{ old('deduction_date', date('Y-m-d')) }}" required class="w-full rounded-xl border-2 border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-rose-500 focus:border-rose-400" />
                ...
                <div>
                    ...
                    <select name="status" required class="w-full rounded-xl border-2 border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-rose-500 focus:border-rose-400">
                        ...
                        <option value="applied" {{ old('status') == 'applied' ? 'selected' : '' }}>مطبقة</option>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" rows="2" class="w-full rounded-xl border-2 border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-rose-500 focus:border-rose-400">{{ old('notes') }}</textarea>
