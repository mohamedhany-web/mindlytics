...
<div class="p-4 md:p-6 space-y-6" style="background:#f8fafc;min-height:100vh;">
    ...
    <form method="get" class="flex flex-wrap gap-3 items-end bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
        ...
        <div class="flex-1 min-w-[180px]">
            ...
            <input type="search" name="search" value="{{ request('search') }}" class="w-full border rounded-lg px-3 py-2 text-sm">
        </div>
        ...
        <table class="w-full min-w-[900px] text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50/80 align-top">
                    <td class="py-2 px-3 text-gray-600 whitespace-nowrap">{{ $log->created_at->format('Y-m-d H:i:s') }}</td>
                    <td class="py-2 px-3 text-gray-900">{{ $log->user->name ?? '—' }}</td>
                    <td class="py-2 px-3"><span class="text-xs font-semibold text-emerald-800">{{ $actionLabels[$log->action] ?? $log->action }}</span></td>
                    <td class="py-2 px-3 text-gray-700 max-w-md">{{ \Illuminate\Support\Str::limit($log->description, 200) }}</td>
                    <td class="py-2 px-3 text-xs text-gray-500 max-w-[200px] truncate" title="{{ $log->url }}">{{ \Illuminate\Support\Str::limit($log->url, 40) }}</td>
                </tr>
                @empty
