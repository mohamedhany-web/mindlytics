...
@section('title', 'تصنيفات العملاء')
@section('header', 'المبيعات — تصنيفات العملاء')
...
<div class="space-y-6" x-data="{ editingId: null }">
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-4 py-4 bg-slate-50 border-b border-slate-200 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('admin.sales.leads.import') }}" class="inline-flex items-center gap-2 px-3 py-2 text-sm font-semibold text-slate-700 rounded-xl border border-slate-300 hover:bg-white">
                    <i class="fas fa-file-upload text-violet-600"></i>
                    استيراد دفعة
                </a>
                <a href="{{ route('admin.sales.kpi.index') }}" class="inline-flex items-center gap-2 px-3 py-2 text-sm font-semibold text-white rounded-xl bg-emerald-600 hover:bg-emerald-700">
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        ...
        <div class="p-4">
            <form method="post" action="{{ route('admin.sales.categories.store') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4 items-end">
                @csrf
                <div class="lg:col-span-3">
                    <label class="block text-xs font-semibold text-slate-700 mb-1">اسم التصنيف *</label>
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="مثال: دورات أونلاين" class="{{ $inputClass }}">
                </div>
                <div class="lg:col-span-3">
                    <label class="block text-xs font-semibold text-slate-700 mb-1">وصف مختصر</label>
                    <input type="text" name="description" value="{{ old('description') }}" placeholder="اختياري" class="{{ $inputClass }}">
                </div>
                <div class="lg:col-span-2">
                    <label class="block text-xs font-semibold text-slate-700 mb-1">اللون</label>
                    <div class="flex items-center gap-2">
                        <input type="color" name="color" value="{{ old('color', '#059669') }}" class="h-10 w-14 rounded-xl border border-slate-300 cursor-pointer">
                ...
                <div class="lg:col-span-2">
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" max="9999" class="{{ $inputClass }}">
                </div>
                <div class="lg:col-span-2">
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-4 py-3 border-b border-slate-200 bg-slate-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            ...
            <span class="text-xs font-semibold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200">{{ $categories->count() }} تصنيف</span>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach($categories as $cat)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                <td class="px-4 py-3 text-slate-400 tabular-nums text-xs">{{ $loop->iteration }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex items-start gap-3">
                                        <span class="mt-1 w-4 h-4 rounded-full flex-shrink-0 ring-2 ring-white shadow-sm" style="background: {{ $cat->color }}"></span>
                                        <div class="min-w-0">
                                            <p class="font-bold text-slate-900">{{ $cat->name }}</p>
                                            @if($cat->description)
                                                <p class="text-xs text-slate-500 mt-0.5 truncate max-w-xs">{{ $cat->description }}</p>
                                            @endif
                                            <p class="text-[10px] text-slate-400 mt-0.5 font-mono">{{ $cat->slug }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <a href="{{ route('admin.sales.leads.index', ['category_id' => $cat->id]) }}"
                                       ...
                                       {{ $cat->leads_count > 0 ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100' : 'bg-slate-50 text-slate-400 border border-slate-200' }}">
                                        {{ number_format($cat->leads_count) }}
                                    </a>
                                </td>
                                <td class="px-4 py-3 text-center tabular-nums text-slate-600">{{ $cat->sort_order }}</td>
                                <td class="px-4 py-3 text-center">
                                    @if($cat->is_active)
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1.5 flex-wrap">
                                        ...
                                        <button type="button"
                                                @click="editingId = editingId === {{ $cat->id }} ? null : {{ $cat->id }}"
                                        ...
                                        @if(!$cat->leads_count)
                                            <form action="{{ route('admin.sales.categories.destroy', $cat) }}" method="post" class="inline" onsubmit="return confirm('حذف التصنيف «{{ $cat->name }}»؟');">
                                                @csrf
                                                @method('DELETE')
                            ...
                            {{-- صف التعديل --}}
                            <tr x-show="editingId === {{ $cat->id }}" x-cloak class="bg-violet-50/40 border-b border-violet-100">
                                <td colspan="6" class="px-4 py-4">
                                    <form method="post" action="{{ route('admin.sales.categories.update', $cat) }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-end">
                                        @csrf
                                        @method('PUT')
                                        <div class="lg:col-span-3">
                                            <label class="block text-xs font-semibold text-slate-700 mb-1">الاسم</label>
                                            <input type="text" name="name" value="{{ $cat->name }}" required class="{{ $inputClass }}">
                                        </div>
                                        <div class="lg:col-span-3">
                                        ...
                                        <div class="lg:col-span-2">
                                            <label class="block text-xs font-semibold text-slate-700 mb-1">اللون</label>
                                            <input type="color" name="color" value="{{ $cat->color }}" class="h-10 w-full rounded-xl border border-slate-300 cursor-pointer">
                                        </div>
                                        ...
                                        <div class="lg:col-span-1 flex items-end pb-1">
                                            <label class="inline-flex items-center gap-2 text-xs font-semibold text-slate-700 cursor-pointer">
                                                <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500" @checked($cat->is_active)>
                                                نشط
                                            </label>
                                        </div>
        ...
        <div class="p-4 flex flex-wrap gap-2">
            @foreach($categories->where('is_active', true) as $cat)
                <span class="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-sm font-semibold border shadow-sm"
                      style="color: {{ $cat->color }}; border-color: {{ $cat->color }}44; background: {{ $cat->color }}12">
                    <span class="w-2.5 h-2.5 rounded-full" style="background: {{ $cat->color }}"></span>
                    {{ $cat->name }}
                    <span class="text-xs opacity-70 tabular-nums">({{ $cat->leads_count }})</span>
                </span>
            @endforeach
            @if($categories->where('is_active', false)->isNotEmpty())
                <span class="text-xs text-slate-400 self-center mr-2">معطّلة:</span>
                @foreach($categories->where('is_active', false) as $cat)
