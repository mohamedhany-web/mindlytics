...
@section('content')
@php
    $activeTemplate = $certificate->template ?: ($branding->default_template ?? 'achievement');
    if ($activeTemplate === 'econev') { $activeTemplate = 'achievement'; }
@endphp
<div class="space-y-6" x-data="{ selectedTemplate: '{{ $activeTemplate }}' }">
    <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200 no-print">
        <div class="flex justify-between items-start mb-6 gap-4 flex-wrap">
            ...
            <div class="flex gap-3">
                <a href="{{ route('admin.certificates.branding') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
                    <i class="fas fa-image ml-2"></i>هوية الشهادات
                </a>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="bg-gray-50 rounded-xl p-4">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    @endif
                    <div><span class="text-gray-600">السيريال:</span> <span class="font-medium text-gray-900 mr-2 font-mono">{{ $certificate->serial_number ?? '-' }}</span></div>
                    <div><span class="text-gray-600">الحالة:</span>
                        @php $status = $certificate->status ?? ($certificate->is_verified ? 'issued' : 'pending'); @endphp
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200 no-print">
        <h3 class="text-lg font-bold text-gray-900 mb-4">قوالب Certificate of Achievement</h3>
        <div class="template-selector">
            @foreach($templates as $key => $tpl)
                <div class="template-option" :class="{ 'active': selectedTemplate === '{{ $key }}' }" @click="selectedTemplate = '{{ $key }}'">
                    <div class="template-preview preview-{{ $key }}"></div>
    ...
    <div class="certificate-container">
        @foreach($templates as $key => $tpl)
            <div x-show="selectedTemplate === '{{ $key }}'" x-cloak>
                @include('components.certificate-templates', [
                    'certificate' => $certificate,
                    'branding' => $branding,
                    'template' => $key,
                    'templateDomId' => 'certificate-template-' . $key,
    ...
    <div class="text-center space-x-4 no-print">
        <button onclick="window.print()" class="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-8 py-3 rounded-xl font-semibold shadow-lg">
            <i class="fas fa-print"></i>
            <span>طباعة الشهادة</span>
        </button>
        <button onclick="downloadCertificate()" class="inline-flex items-center gap-2 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white px-8 py-3 rounded-xl font-semibold shadow-lg">
...
<script>
    function downloadCertificate() {
        const visible = Array.from(document.querySelectorAll('.certificate-container > div')).find(el => el.offsetParent !== null);
        const element = (visible && visible.querySelector('.certificate-template')) || document.querySelector('.certificate-template');
        html2pdf().set({
            margin: 0,
