                ...
                <div>
                    ...
                    <select name="template" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                        @foreach($templates as $key => $tpl)
                            <option value="{{ $key }}" @selected(old('template', $branding->default_template) === $key)>
                                {{ $tpl['name'] }}
                            </option>
                        @endforeach
                    </select>
                    <p class="text-xs text-gray-500 mt-1">
                        اللوجو والإمضاء والختم من
                        <a href="{{ route('admin.certificates.branding') }}" class="text-sky-600 underline">هوية الشهادات</a>
                        — ويُنشأ سيريال تلقائي عند الإصدار.
                    </p>
                </div>
            </div>
