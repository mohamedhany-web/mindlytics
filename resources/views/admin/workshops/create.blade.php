...
@section('title', 'إنشاء ورشة جديدة')
...
<div class="p-6 lg:p-8">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <form method="POST" action="{{ route('admin.workshops.store') }}" class="px-5 py-6 sm:px-8 lg:px-10 space-y-6">
            @csrf
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select id="mode" name="mode"
                            class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500">
                        <option value="online" {{ old('mode', 'online') === 'online' ? 'selected' : '' }}>أونلاين (عن بُعد)</option>
                        <option value="offline" {{ old('mode') === 'offline' ? 'selected' : '' }}>في المكان (أوفلاين)</option>
                        <option value="both" {{ old('mode') === 'both' ? 'selected' : '' }}>إتاحة الاختيار (أونلاين أو أوفلاين)</option>
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" id="max_seats" name="max_seats" value="{{ old('max_seats', 0) }}" min="0"
                           class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500">
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" id="seats_online" name="seats_online" value="{{ old('seats_online', 0) }}" min="0"
                           class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500">
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" id="seats_offline" name="seats_offline" value="{{ old('seats_offline', 0) }}" min="0"
                           class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500">
            ...
            <div class="space-y-2">
                ...
                <textarea id="description" name="description" rows="5"
                          ...
                          placeholder="اكتب هنا محاور الورشة، المتطلبات، الأدوات التي سيحتاجها الطالب، وسطر عن المدرب.">{{ old('description') }}</textarea>
            </div>
