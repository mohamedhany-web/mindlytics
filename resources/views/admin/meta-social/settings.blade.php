...
@section('title', 'إعدادات Meta Social')
@section('header', 'ربط Meta — Facebook & Instagram')
...
<div class="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6" style="background: #f8fafc; min-height: 100vh;">
        ...
        <div class="p-5 sm:p-6">
            <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <p class="font-bold text-slate-900 mt-1">{{ $connectionMeta['label'] ?? '—' }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <p class="text-xs text-slate-500 font-semibold">حساب Meta</p>
                    <p class="font-bold text-slate-900 mt-1">{{ $connectionMeta['meta_user_name'] ?? '—' }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <p class="text-xs text-slate-500 font-semibold">App ID</p>
                    <p class="font-mono text-xs text-slate-800 break-all mt-1">{{ $savedAppId !== '' ? $savedAppId : '—' }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <p class="text-xs text-slate-500 font-semibold">Webhook Verify Token</p>
                    <p class="font-mono text-xs text-slate-800 break-all mt-1">{{ $savedWebhookToken !== '' ? '••••••••' : '—' }}</p>
                </div>
            </div>
            ...
            @if($webhookStatus['last_received_at'] ?? null)
                <p class="mt-4 text-xs text-emerald-700 flex items-center gap-2">
                    <i class="fas fa-check-circle"></i> آخر Webhook: {{ $webhookStatus['last_received_at'] }}
                </p>
            @endif

            @if($isConnected || $hasSecret || $savedAppId !== '')
                <form method="post" action="{{ route('admin.meta-social.oauth.disconnect') }}" class="mt-4"
    ...
    <div class="grid lg:grid-cols-2 gap-4 sm:gap-6">
        {{-- نموذج الإعدادات --}}
        <section class="{{ $smSectionClass }} lg:col-span-1">
            <div class="px-5 py-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white">
                <h3 class="text-lg font-bold text-slate-900 flex items-center gap-2">
            ...
            <div class="p-5 sm:p-6">
                <form method="post" action="{{ route('admin.meta-social.settings.update') }}" class="space-y-5" id="sm-settings-form">
                    @csrf

                    <label class="flex items-center gap-3 text-sm font-semibold text-slate-800 cursor-pointer">
                        <input type="checkbox" name="enabled" value="1" @checked($savedEnabled) class="rounded border-slate-300 text-sky-600 focus:ring-sky-500 w-4 h-4">
                    ...
                    <div>
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            <div>
                                <label class="{{ $smLabelClass }}">Meta App ID *</label>
                                <input type="text" name="app_id" value="{{ $savedAppId }}" required
                                       class="{{ $smInputClass }} dir-ltr font-mono text-sm" placeholder="123456789012345">
                            </div>
                            <div>
                                <label class="{{ $smLabelClass }}">
                                    Meta App Secret {{ $hasSecret ? '' : '*' }}
                                    @if($hasSecret)
                                ...
                                <input type="password" name="app_secret" value=""
                                       autocomplete="new-password" class="{{ $smInputClass }} dir-ltr font-mono text-sm"
                                       placeholder="{{ $hasSecret ? 'محفوظ — اتركه فارغاً للإبقاء' : 'App Secret من Meta Developers' }}"
                                       @if(!$hasSecret) required @endif>
                            </div>
                            <div class="sm:col-span-2">
                                <label class="{{ $smLabelClass }}">Graph API URL *</label>
                                <input type="url" name="api_url" value="{{ $savedApiUrl }}" required
                                       class="{{ $smInputClass }} dir-ltr font-mono text-sm">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-slate-100 pt-5">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="{{ $smLabelClass }}">Webhook Verify Token *</label>
                                <input type="text" name="webhook_verify_token" value="{{ $savedWebhookToken }}" required
                                       class="{{ $smInputClass }} dir-ltr font-mono text-sm">
                            </div>
                            <div>
                                <label class="{{ $smLabelClass }}">OAuth Scopes (مفصولة بفاصلة)</label>
                                <textarea name="oauth_scopes" rows="4" class="{{ $smTextareaClass }} dir-ltr font-mono text-xs">{{ $savedScopes }}</textarea>
                            </div>
                        </div>
                    </div>

                    <button type="submit" class="{{ $smBtnPrimary }} w-full sm:w-auto">
                ...
                <div class="p-5 sm:p-6 space-y-4">
                    <div class="grid sm:grid-cols-2 gap-3">
                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <p class="font-bold text-slate-900 mt-0.5">{{ $connectionMeta['label'] ?? '—' }}</p>
                        </div>
                        @if($connectionMeta['meta_user_name'] ?? null)
                            <div class="rounded-xl border border-sky-200 bg-sky-50/50 p-4">
                                <p class="text-xs text-sky-700 font-semibold">الحساب</p>
                                <p class="font-bold text-sky-900 mt-0.5">{{ $connectionMeta['meta_user_name'] }}</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        @if($hasSecret)
                            <a href="{{ route('admin.meta-social.oauth.redirect') }}" class="{{ $smBtnMeta }}">
                        ...
                        @endif
                        <form method="post" action="{{ route('admin.meta-social.settings.webhook-sync') }}" class="inline">@csrf
                            <button type="submit" class="{{ $smBtnSecondary }} text-sm"><i class="fas fa-sync"></i> مزامنة Webhook</button>
                        </form>
                    </div>
                </div>
            </section>
...
document.getElementById('btn-test-connection')?.addEventListener('click', async function () {
    ...
    try {
        const res = await fetch(@json(route('admin.meta-social.settings.test')), { headers: { 'Accept': 'application/json' } });
