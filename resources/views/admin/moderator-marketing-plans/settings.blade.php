...
@section('title', 'إعدادات خطط التسويق')
@section('header', 'إعدادات الأتمتة — خطط التسويق')
...
<div class="p-3 sm:p-4 md:p-6 space-y-6" style="background: #f8fafc; min-height: 100vh;">
    ...
    <section class="rounded-2xl bg-white border-2 border-slate-200 shadow-xl overflow-hidden">
        <div class="px-6 py-5 border-b bg-gradient-to-r from-pink-50 to-violet-50 flex flex-wrap items-center justify-between gap-3">
            ...
            <a href="{{ route('admin.moderator-marketing-plans.index') }}" class="text-sm text-slate-600 hover:text-pink-700 font-semibold">
                <i class="fas fa-arrow-right ml-1"></i> خطط التسويق
            </a>
        </div>

        <div class="p-6">
            <form method="post" action="{{ route('admin.moderator-marketing-plans.settings.update') }}" class="space-y-6 max-w-2xl">
                @csrf @method('PUT')

                <div class="grid sm:grid-cols-2 gap-4">
                    <label class="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/50 p-4 cursor-pointer">
                        <input type="checkbox" name="automation_enabled" value="1" @checked($settings['automation_enabled'] ?? true) class="mt-1 rounded border-slate-300 text-pink-600">
                        <span>
                    ...
                    <label class="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/50 p-4 cursor-pointer">
                        <input type="checkbox" name="auto_create_tasks" value="1" @checked($settings['auto_create_tasks'] ?? true) class="mt-1 rounded border-slate-300 text-violet-600">
                        <span>
                    ...
                    <label class="flex items-start gap-3 rounded-xl border border-rose-200 bg-rose-50/40 p-4 cursor-pointer sm:col-span-2">
                        <input type="checkbox" name="penalty_enabled" value="1" @checked($settings['penalty_enabled'] ?? true) class="mt-1 rounded border-rose-300 text-rose-600">
                        <span>
                ...
                <div class="rounded-xl border border-slate-200 p-4 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" name="penalty_amount" value="{{ old('penalty_amount', $settings['penalty_amount'] ?? 50) }}" class="w-full max-w-xs rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-pink-500">
                        @error('penalty_amount')<p class="text-rose-600 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div class="grid sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">وقت التذكير اليومي</label>
                            <input type="time" name="reminder_time" value="{{ old('reminder_time', $reminderTime) }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">موعد تطبيق الغرامة</label>
                            <input type="time" name="confirmation_deadline_time" value="{{ old('confirmation_deadline_time', $deadlineTime) }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm">
                        </div>
                    </div>
                    <p class="text-xs text-slate-500 leading-relaxed">
                        التذكير يُرسل يومياً الساعة <strong>{{ $reminderTime }}</strong> —
                        الغرامات تُطبَّق الساعة <strong>{{ $deadlineTime }}</strong> على أحداث اليوم غير المؤكدة.
                    </p>
                </div>
                ...
                <div class="flex flex-wrap gap-3 pt-2">
                    ...
                    <a href="{{ route('admin.moderator-marketing-plans.index') }}" class="px-5 py-2.5 rounded-xl border border-slate-300 text-sm font-semibold text-slate-700 hover:bg-slate-50">إلغاء</a>
                </div>
            </form>
        </div>
    </section>
