...
@section('title', 'إشعارات التطبيق — Mindlytics Community')
@section('page_title', 'Mindlytics Community · إشعارات التطبيق')
...
<div class="w-full min-h-screen p-3 sm:p-4 md:p-6 lg:p-8 space-y-4 sm:space-y-6" style="background: #f8fafc;">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
        ...
        <div class="rounded-2xl border-2 border-slate-200/80 bg-white p-6 sm:p-8 shadow-lg">
            ...
            <form action="{{ route('admin.mobile-app.notifications.store') }}" method="POST" class="space-y-5">
                @csrf
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1">العنوان</label>
                    <input type="text" name="title" id="field-title" value="{{ old('title') }}" required maxlength="255"
                ...
                <div>
                    ...
                    <textarea name="message" id="field-message" rows="4" required maxlength="8000"
                              class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 text-slate-900 focus:border-violet-400 focus:ring-0"
                              placeholder="نص يظهر في قائمة الإشعارات والمعاينة">{{ old('message') }}</textarea>
                </div>
                    ...
                    <div>
                        ...
                        <select name="type" class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 bg-white">
                            @foreach($types as $k => $label)
                                <option value="{{ $k }}" @selected(old('type', 'general') === $k)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <select name="scope" id="scope" onchange="window.mobileNotifScope && window.mobileNotifScope()"
                        ...
                        <option value="all_students" @selected(old('scope') === 'all_students')>جميع الطلاب النشطين</option>
                        <option value="course" @selected(old('scope') === 'course')>طلاب كورس محدد</option>
                        <option value="user" @selected(old('scope') === 'user')>طالب واحد</option>
                    </select>
                </div>
                <div id="wrap-course" class="{{ old('scope') === 'course' ? '' : 'hidden' }}">
                    ...
                    <select name="advanced_course_id" class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 bg-white max-h-48 overflow-auto">
                        ...
                        @foreach($courses as $c)
                            <option value="{{ $c->id }}" @selected((string)old('advanced_course_id') === (string)$c->id)>
                                #{{ $c->id }} — {{ \Illuminate\Support\Str::limit($c->title, 80) }}
                            </option>
                        @endforeach
                    </select>
                </div>
                <div id="wrap-user" class="{{ old('scope') === 'user' ? '' : 'hidden' }}">
                    ...
                    <select name="user_id" class="w-full rounded-xl border-2 border-slate-200 px-4 py-3 bg-white max-h-48 overflow-auto">
                        ...
                        @foreach($students as $s)
                            <option value="{{ $s->id }}" @selected((string)old('user_id') === (string)$s->id)>
                                {{ $s->name }} · {{ $s->email }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="action_text" value="{{ old('action_text') }}" maxlength="255"
                        ...
                        <div id="mock-toast" class="rounded-2xl bg-white/95 text-slate-900 p-3 shadow-lg border border-white/20">
                            <div class="flex gap-3">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p id="pv-title" class="text-sm font-black text-slate-900 leading-tight truncate">{{ old('title', 'عنوان الإشعار') }}</p>
                                    <p id="pv-body" class="text-xs text-slate-600 mt-1 line-clamp-4">{{ old('message', 'نص الرسالة كما يقرأه الطالب في التطبيق.') }}</p>
                                </div>
                            </div>
                        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($recentBatches as $batch)
                        <tr class="hover:bg-violet-50/40">
                            <td class="px-4 py-3 whitespace-nowrap text-slate-600">{{ $batch['sent_at']->format('Y-m-d H:i') }}</td>
                            <td class="px-4 py-3 font-bold text-slate-900 max-w-[200px]">{{ \Illuminate\Support\Str::limit($batch['title'], 48) }}</td>
                            <td class="px-4 py-3 text-slate-600 hidden md:table-cell max-w-md">{{ \Illuminate\Support\Str::limit($batch['message'], 90) }}</td>
                            <td class="px-4 py-3 font-black text-violet-700">{{ $batch['recipients'] }}</td>
                            <td class="px-4 py-3 hidden lg:table-cell text-xs">
                                <span class="inline-block rounded-lg bg-slate-100 px-2 py-0.5">{{ $types[$batch['type']] ?? $batch['type'] }}</span>
                                ·
                                <span class="inline-block rounded-lg bg-slate-100 px-2 py-0.5">{{ $priorities[$batch['priority']] ?? $batch['priority'] }}</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    <div class="rounded-2xl border-2 border-slate-200/80 bg-white p-5 sm:p-6 shadow-lg w-full">
        ...
        <div class="flex flex-wrap gap-2 sm:gap-3">
            <a href="{{ route('admin.mobile-app.notifications') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-violet-100 border-violet-300 px-4 py-2 text-sm font-semibold text-violet-900"><i class="fas fa-bell text-violet-600"></i> إشعارات</a>
            <a href="{{ route('admin.mobile-app.maintenance') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-violet-50 hover:border-violet-200"><i class="fas fa-tools text-slate-600"></i> الصيانة</a>
            <a href="{{ route('admin.mobile-app.links') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-violet-50 hover:border-violet-200"><i class="fas fa-link text-indigo-600"></i> الروابط</a>
            <a href="{{ route('admin.mobile-app.appearance') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-violet-50 hover:border-violet-200"><i class="fas fa-palette text-pink-600"></i> المظهر</a>
            <a href="{{ route('admin.mobile-app.edit') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-violet-50 hover:border-violet-200"><i class="fas fa-home text-blue-600"></i> الصفحة الرئيسية</a>
        </div>
    </div>
</div>
