...
@section('title', __('admin.course_community_title') . ' — Mindlytics Community')
@section('page_title', __('admin.course_community_title'))
...
<div class="w-full min-h-screen p-3 sm:p-4 md:p-6 lg:p-8 space-y-4 sm:space-y-6" style="background: #f8fafc;">
    ...
    {{-- هيدر كامل العرض — نفس أسلوب صفحة النشر ومحتوى التطبيق --}}
        ...
        <div class="relative z-10 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
            <div class="flex items-start gap-5 min-w-0 flex-1">
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-sm font-bold text-violet-700/90 mb-1">Mindlytics Community · {{ __('admin.course_community_title') }}</p>
                    ...
                    <p class="mt-3 text-slate-600 text-sm sm:text-base leading-relaxed max-w-4xl">
                        {{ __('admin.course_community_subtitle') }}
                    </p>
                </div>
            </div>
            <div class="flex flex-col sm:flex-row flex-wrap gap-3 shrink-0 w-full xl:w-auto">
                ...
                <a href="{{ route('admin.mobile-app.course-community.posts.create') }}"
                   class="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 via-indigo-600 to-violet-700 px-5 py-2.5 text-white font-black shadow-xl shadow-violet-600/35 hover:from-violet-500 hover:via-indigo-500 hover:to-violet-600 transition-all duration-300">
                    ...
                    {{ __('admin.course_community_new_post') }}
                </a>
            </div>
        </div>
    </div>
        ...
        <div class="rounded-2xl p-5 sm:p-6 border-2 border-blue-200/50 shadow-lg transition-all duration-300"
            ...
            <div class="flex items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-3xl font-black text-blue-700">{{ $posts->total() }}</p>
                </div>
        ...
        <div class="rounded-2xl p-5 sm:p-6 border-2 border-emerald-200/50 shadow-lg transition-all duration-300"
            ...
            <div class="flex items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-3xl font-black text-emerald-800">{{ $posts->currentPage() }} <span class="text-lg font-bold text-emerald-600">/ {{ $posts->lastPage() }}</span></p>
                </div>
            ...
            <p class="text-xs text-emerald-800/75 mt-3 leading-relaxed">{{ $posts->perPage() }} عنصر في الصفحة</p>
        </div>
        <div class="rounded-2xl p-5 sm:p-6 border-2 border-amber-200/50 shadow-lg transition-all duration-300"
            ...
            <div class="flex items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-3xl font-black text-amber-900">{{ $courses->count() }}</p>
                </div>
    ...
    <div class="rounded-2xl border-2 border-slate-200/70 bg-white p-5 sm:p-8 shadow-lg w-full">
        ...
        <form method="get" action="{{ route('admin.mobile-app.course-community.index') }}" class="flex flex-col lg:flex-row flex-wrap gap-4 lg:items-end">
            <div class="flex-1 min-w-[200px] space-y-2">
                <label class="block text-sm font-bold text-slate-700">{{ __('admin.course_community_all_courses') }}</label>
            ...
            <div class="flex-[2] min-w-[240px] space-y-2">
                <label class="block text-sm font-bold text-slate-700">{{ __('admin.course_community_search_placeholder') }}</label>
                <input type="text" name="q" value="{{ request('q') }}" placeholder="{{ __('admin.course_community_search_placeholder') }}"
                       class="w-full rounded-xl border-2 border-slate-200/90 bg-slate-50/50 px-4 py-3 text-sm focus:ring-2 focus:ring-violet-500/30 focus:border-violet-400 transition-shadow"/>
            </div>
    ...
    <div class="rounded-2xl border-2 border-slate-200/70 bg-white shadow-xl overflow-hidden w-full">
        <div class="px-5 sm:px-8 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-gradient-to-r from-slate-50/80 to-white">
            ...
            <span class="text-sm font-semibold text-slate-500">{{ $posts->firstItem() ?? 0 }}–{{ $posts->lastItem() ?? 0 }} من {{ $posts->total() }}</span>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($posts as $post)
                        <tr class="hover:bg-violet-50/40 transition-colors">
                            <td class="px-4 sm:px-6 py-4 text-slate-500 font-mono text-xs">{{ $post->id }}</td>
                            <td class="px-4 sm:px-6 py-4 font-bold text-slate-800 max-w-[200px]">
                                <span class="line-clamp-2">{{ Str::limit($post->course?->title ?? '—', 48) }}</span>
                            </td>
                            <td class="px-4 sm:px-6 py-4 text-slate-700 font-semibold">{{ $post->user->name ?? '—' }}</td>
                            <td class="px-4 sm:px-6 py-4 text-slate-600 max-w-xl">
                                <span class="line-clamp-2">{{ Str::limit($post->body, 120) }}</span>
                            </td>
                            <td class="px-4 py-4 text-center">
                                <span class="inline-flex min-w-[2rem] justify-center rounded-lg bg-slate-100 px-2 py-1 font-bold text-slate-700">{{ $post->images->count() }}</span>
                            </td>
                            <td class="px-4 py-4 text-center">
                                <span class="inline-flex min-w-[2rem] justify-center rounded-lg bg-emerald-50 px-2 py-1 font-bold text-emerald-800">{{ $post->comments_count }}</span>
                            </td>
                            <td class="px-4 py-4 text-center">
                                @if($post->is_pinned)
                            ...
                            <td class="px-4 sm:px-6 py-4 text-slate-500 whitespace-nowrap text-xs font-medium">{{ $post->created_at?->format('Y-m-d H:i') }}</td>
                            <td class="px-4 sm:px-6 py-4 text-center whitespace-nowrap">
                                <a href="{{ route('admin.mobile-app.course-community.posts.show', $post) }}"
                        ...
                        <tr>
                            <td colspan="9" class="px-6 py-16 text-center">
                                <div class="max-w-md mx-auto rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/80 p-8">
                                    ...
                                    <p class="font-black text-slate-800 text-lg mb-2">{{ __('admin.course_community_no_posts') }}</p>
