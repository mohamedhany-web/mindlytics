...
@section('title', __('admin.course_community_new_post') . ' — Mindlytics Community')
@section('page_title', __('admin.course_community_new_post'))
...
<div class="w-full min-h-screen p-3 sm:p-4 md:p-6 lg:p-8 space-y-4 sm:space-y-6" style="background: #f8fafc;">
    ...
    {{-- شريط بطول الصفحة — نفس لوحة التحكم / صفحة محتوى التطبيق --}}
            ...
            <div class="flex flex-wrap gap-3 shrink-0">
                <a href="{{ route('admin.mobile-app.course-community.index') }}"
                   class="inline-flex items-center gap-2 rounded-xl border-2 border-slate-200/90 bg-white/95 px-4 py-2.5 text-sm font-bold text-slate-800 shadow-sm hover:bg-slate-50 transition-colors">
                    <i class="fas fa-arrow-{{ app()->getLocale() === 'ar' ? 'right' : 'left' }} text-violet-600"></i>
                    رجوع للمراقبة
                </a>
    ...
    <form method="post" action="{{ route('admin.mobile-app.course-community.posts.store') }}" enctype="multipart/form-data" class="space-y-6 w-full">
        @csrf
            ...
            <div class="w-full">
                ...
                <select name="course_id" required
                    ...
                    @foreach($courses as $c)
                        <option value="{{ $c->id }}" @selected(old('course_id') == $c->id)>{{ $c->title }} @if($c->title_en) ({{ $c->title_en }}) @endif</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="rounded-2xl border-2 border-slate-200/70 bg-white p-5 sm:p-8 shadow-lg w-full">
            ...
            <div class="space-y-2">
                ...
                <textarea name="body" rows="10"
                          ...
                          class="w-full rounded-xl border-2 border-slate-200/90 bg-slate-50/50 px-4 py-3 text-sm leading-relaxed focus:ring-2 focus:ring-violet-500/30 focus:border-violet-400 transition-shadow min-h-[200px]">{{ old('body') }}</textarea>
        ...
        <div class="rounded-2xl border-2 border-slate-200/70 bg-white p-5 sm:p-8 shadow-lg w-full">
            ...
            <label class="flex items-start gap-4 cursor-pointer rounded-xl border-2 border-amber-100 bg-amber-50/40 p-4 hover:bg-amber-50/70 transition-colors">
                <input type="checkbox" name="is_pinned" value="1" class="mt-1 rounded border-slate-300 text-violet-600 focus:ring-violet-500 w-5 h-5" @checked(old('is_pinned'))/>
                <span>
