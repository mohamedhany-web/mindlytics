...
<div class="space-y-6">
    ...
    <div class="rounded-2xl p-5 sm:p-6 bg-white border border-slate-200 shadow-sm">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-slate-800 mb-1">{{ 'بناء المنهج' }}</h1>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div id="sections-container">
                @forelse($sections as $section)
                    @include('admin.curriculum.partials.section', ['section' => $section, 'depth' => 0, 'isScholarshipCurriculum' => !empty($isScholarshipCurriculum), 'course' => $course])
                @empty
...
<div id="accessVisibilityModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl p-6 max-w-lg w-full shadow-xl border border-slate-200 max-h-[90vh] overflow-y-auto">
        ...
        <p class="text-sm text-slate-600 mb-4" id="accessVisibilitySubtitle">اختر من يظهر له هذا المحتوى من طلبة المنحة.</p>
...
<div id="lectureModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-5xl">
        <div class="flex items-center justify-between px-5 py-3 border-b border-slate-100">
            <div>
                <h3 class="text-lg font-bold text-slate-800">إضافة محاضرة جديدة</h3>
                <p class="text-xs text-slate-500">أدخل البيانات ثم احفظ لإضافة المحاضرة للمنهج</p>
            </div>
            <button type="button" onclick="closeLectureModal()" class="p-2 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-700">
                <i class="fas fa-times"></i>
            </button>
        </div>
        ...
        <form id="lectureForm" onsubmit="saveLecture(event)">
            ...
            <div class="px-5 py-4">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div class="rounded-xl border border-slate-200 bg-slate-50/70 p-4 space-y-3">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">عنوان المحاضرة <span class="text-red-500">*</span></label>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 mb-1">التاريخ والوقت <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="lectureDescription" rows="3" placeholder="وصف مختصر للمحاضرة..."
                                      class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 text-slate-800 bg-white text-sm"></textarea>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">الملاحظات</label>
                            <textarea name="notes" id="lectureNotes" rows="2" placeholder="ملاحظات إضافية..."
                                      class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 text-slate-800 bg-white text-sm"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="rounded-xl border border-slate-200 bg-white p-4 space-y-3">
                            <h4 class="text-sm font-bold text-slate-700 flex items-center gap-2">
                                <i class="fas fa-video text-sky-500"></i>
                                رابط تسجيل المحاضرة
                                <span class="text-xs font-normal text-slate-400">(اختياري)</span>
                            </h4>
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 mb-1.5">اختر المشغل</label>
                                <div class="grid grid-cols-5 gap-2">
                                    <button type="button" onclick="selectVideoPlatform('youtube', this)" class="platform-btn p-2 border-2 border-slate-200 rounded-xl text-center hover:border-sky-400 transition-colors" data-platform="youtube">
                                        <i class="fab fa-youtube text-red-600 text-base mb-0.5 block"></i>
                                        <span class="text-[10px] font-semibold text-slate-700">YouTube</span>
                                    </button>
                                    <button type="button" onclick="selectVideoPlatform('vimeo', this)" class="platform-btn p-2 border-2 border-slate-200 rounded-xl text-center hover:border-sky-400 transition-colors" data-platform="vimeo">
                                        <i class="fab fa-vimeo text-blue-500 text-base mb-0.5 block"></i>
                                        <span class="text-[10px] font-semibold text-slate-700">Vimeo</span>
                                    </button>
                                    <button type="button" onclick="selectVideoPlatform('google_drive', this)" class="platform-btn p-2 border-2 border-slate-200 rounded-xl text-center hover:border-sky-400 transition-colors" data-platform="google_drive">
                                        <i class="fab fa-google-drive text-green-600 text-base mb-0.5 block"></i>
                                        <span class="text-[10px] font-semibold text-slate-700">Drive</span>
                                    </button>
                                    <button type="button" onclick="selectVideoPlatform('direct', this)" class="platform-btn p-2 border-2 border-slate-200 rounded-xl text-center hover:border-sky-400 transition-colors" data-platform="direct">
                                        <i class="fas fa-file-video text-purple-600 text-base mb-0.5 block"></i>
                                        <span class="text-[10px] font-semibold text-slate-700">مباشر</span>
                                    </button>
                                    <button type="button" onclick="selectVideoPlatform('bunny', this)" class="platform-btn p-2 border-2 border-slate-200 rounded-xl text-center hover:border-sky-400 transition-colors" data-platform="bunny">
                                        <i class="fas fa-cloud text-orange-600 text-base mb-0.5 block"></i>
                                        <span class="text-[10px] font-semibold text-slate-700">Bunny</span>
                                    </button>
                                </div>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-[11px] text-slate-500" id="lectureVideoPlaceholder"></p>
                            </div>
                            <div id="lectureVideoPreview" class="hidden bg-black rounded-xl overflow-hidden" style="aspect-ratio: 16/9; max-height: 110px;">
                        ...
                        <div class="rounded-xl border border-slate-200 bg-slate-50/70 p-4 space-y-2">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <button type="button" id="curriculum-add-material" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-sky-100 text-sky-700 rounded-lg font-semibold text-xs hover:bg-sky-200 transition-colors">
                                    <i class="fas fa-plus"></i>
                                    إضافة مادة
                                </button>
                            </div>
                            <div id="curriculum-materials-container" class="space-y-2">
                                <div class="curriculum-material-row flex flex-wrap items-end gap-2 p-2.5 bg-white rounded-xl border border-slate-200">
                                    <div class="flex-1 min-w-[120px]">
                                        <label class="block text-[11px] font-semibold text-slate-600 mb-1">الملف</label>
                                        <input type="file" name="material_files[]" class="w-full text-xs text-slate-700 file:mr-2 file:py-1 file:px-2.5 file:rounded-lg file:border-0 file:bg-sky-100 file:text-sky-700 file:text-xs file:font-semibold" accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt,.zip,.rar,.png,.jpg,.jpeg">
                                    </div>
                                    <div class="w-32">
                                        <label class="block text-[11px] font-semibold text-slate-600 mb-1">عنوان (اختياري)</label>
                                        <input type="text" name="material_titles[]" placeholder="ملخص المحاضرة" class="w-full px-2.5 py-1.5 border border-slate-200 rounded-lg text-xs">
                                    </div>
                                    <label class="flex items-center gap-1.5 pb-1.5">
                                        <input type="hidden" name="material_visible[]" value="0">
                                        <input type="checkbox" name="material_visible[]" value="1" checked class="w-3.5 h-3.5 text-sky-600 rounded">
                                        <span class="text-xs font-medium text-slate-700">ظاهر للطالب</span>
                                    </label>
                                    <button type="button" class="curriculum-remove-material px-2.5 py-1.5 bg-rose-100 text-rose-700 rounded-lg text-xs font-medium hover:bg-rose-200" style="display:none;"><i class="fas fa-times ml-1"></i> حذف</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="flex gap-3 px-5 py-3 border-t border-slate-100">
...
function saveAssignment(e) {
    ...
    fetch('{{ route("admin.advanced-courses.curriculum.assignments.store", $course) }}', {
        method: 'POST',
        headers: {
...
function saveExam(e) {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin ml-1"></i> جاري الحفظ...';
    fetch('{{ route("admin.advanced-courses.curriculum.exams.store", $course) }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
function getSelectedOptions(selectId) {
    ...
    if (!el) return [];
    return Array.from(el.selectedOptions).map(o => parseInt(o.value, 10)).filter(Boolean);
}
...
function openAccessVisibilityModal(type, id, label) {
    if (!isScholarshipCurriculum) return;
    document.getElementById('accessTargetType').value = type;
    document.getElementById('accessTargetId').value = id;
    const titleEl = document.getElementById('accessVisibilityTitle');
    const subtitleEl = document.getElementById('accessVisibilitySubtitle');
    if (titleEl) titleEl.textContent = type === 'section' ? 'إعداد وصول القسم' : 'إعداد وصول العنصر';
    if (subtitleEl) subtitleEl.textContent = label
        ? ('اختر من يظهر له: ' + label)
...
function closeAccessVisibilityModal() {
    const modal = document.getElementById('accessVisibilityModal');
    if (!modal) return;
    modal.classList.add('hidden');
    modal.classList.remove('flex');
}

function saveAccessVisibility(e) {
    e.preventDefault();
    ...
    const url = type === 'section'
        ? `/admin/sections/${id}/visibility`
        : `/admin/curriculum-items/${id}/visibility`;
...
function addItem(e) {
    ...
    fetch(`/admin/sections/${sectionId}/items`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function removeItem(id) {
    ...
    fetch(`/admin/curriculum-items/${id}`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
        }
...
async function editLectureFromCurriculum(lectureId, sectionId) {
    try {
        const response = await fetch(`/admin/lectures/${lectureId}`, {
            headers: {
                'Accept': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
            }
        ...
        if (!response.ok) {
            ...
            window.location.href = `/admin/lectures/${lectureId}/edit`;
            return;
        }
...
async function loadVideoQuestionsData(lectureId) {
    ...
    try {
        var res = await fetch('{{ url('/admin/lectures') }}/' + lectureId + '/video-questions', {
...
async function submitVideoQuestion(e) {
    ...
    try {
        var res = await fetch('{{ url('/admin/lectures') }}/' + videoQuestionsLectureId + '/video-questions', {
            method: 'POST',
            credentials: 'same-origin',
...
async function deleteVideoQuestion(lectureId, vqId) {
    ...
    try {
        var res = await fetch('{{ url('/admin/lectures') }}/' + lectureId + '/video-questions/' + vqId, {
            method: 'DELETE',
            credentials: 'same-origin',
...
function saveLecture(e) {
    ...
    let url = '{{ route("admin.lectures.store") }}';
    let method = 'POST';
    
    if (lectureId) {
        url = `/admin/lectures/${lectureId}`;
    ...
    .catch(err => {
        ...
        if (err.message && err.message.includes('422')) {
            fetch('{{ route("admin.lectures.store") }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'X-Requested-With': 'XMLHttpRequest',
...
(function curriculumSortable() {
    ...
    var sectionsOrderUrl = '{{ route("admin.advanced-courses.sections.order", $course) }}';
    var moveItemUrlBase = '{{ url("admin/curriculum-items") }}';
    ...
    function saveItemsOrder(sectionId, itemIds) {
        ...
        var url = '/admin/sections/' + sectionId + '/items/order';
        fetch(url, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': token, 'Accept': 'application/json' },
