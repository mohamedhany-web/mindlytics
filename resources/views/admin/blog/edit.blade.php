...
@section('title', 'تعديل المقال - Mindlytics')
...
<div class="min-h-screen bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-gray-600">{{ $blog->title }}</p>
                </div>
                <div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form action="{{ route('admin.blog.update', $blog) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow rounded-lg p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="excerpt" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900">{{ old('excerpt', $blog->excerpt) }}</textarea>
                    @error('excerpt')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="15" required
                              class="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900">{{ old('content', $blog->content) }}</textarea>
                    @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="meta_description" rows="2"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900">{{ old('meta_description', $blog->meta_description) }}</textarea>
                </div>
            </div>
