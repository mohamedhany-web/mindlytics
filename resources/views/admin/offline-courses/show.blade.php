...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white rounded-2xl p-5 border-2 border-emerald-200/50 shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-semibold text-gray-600 mb-1">إجمالي الإيرادات</p>
                    <p class="text-2xl font-black text-emerald-700">{{ number_format($stats['total_revenue'] ?? 0, 2) }}</p>
                    <p class="text-xs text-gray-500 mt-1">ج.م</p>
                </div>
                <div class="w-14 h-14 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl flex items-center justify-center text-white shadow-lg">
                    <i class="fas fa-money-bill-wave text-xl"></i>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-2xl p-5 border-2 border-red-200/50 shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-semibold text-gray-600 mb-1">المبالغ المتبقية</p>
                    <p class="text-2xl font-black text-red-700">{{ number_format($stats['total_remaining'] ?? 0, 2) }}</p>
                    <p class="text-xs text-gray-500 mt-1">ج.م</p>
                </div>
        ...
        <div class="bg-white rounded-2xl p-5 border-2 border-indigo-200/50 shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-semibold text-gray-600 mb-1">إجمالي الجلسات</p>
                    <p class="text-2xl font-black text-indigo-700">{{ $stats['total_sessions'] ?? 0 }}</p>
                    <p class="text-xs text-gray-500 mt-1">جلسة</p>
                </div>
        ...
        <a href="{{ route('admin.offline-courses.enrollments.index', [$offlineCourse, 'channel' => 'offline']) }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-all duration-300 border border-gray-200 border-r-4 border-r-emerald-500 group">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600">{{ $stats['students_offline_channel'] ?? 0 }} تسجيل · نشط {{ $stats['active_offline_channel'] ?? 0 }}</p>
                </div>
            </div>
        </a>
        <a href="{{ route('admin.offline-courses.enrollments.index', [$offlineCourse, 'channel' => 'online']) }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-all duration-300 border border-gray-200 border-r-4 border-r-indigo-500 group">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600">{{ $stats['students_online_channel'] ?? 0 }} تسجيل · نشط {{ $stats['active_online_channel'] ?? 0 }}</p>
                </div>
            </div>
        </a>
        <a href="{{ route('admin.offline-course-bookings.index', ['offline_course_id' => $offlineCourse->id]) }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-all duration-300 border border-gray-200 border-r-4 border-r-amber-400 group">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600">معلّقة: {{ $stats['pending_offline_bookings'] ?? 0 }}</p>
                </div>
            </div>
        </a>
        <a href="{{ route('admin.online-course-bookings.index', ['offline_course_id' => $offlineCourse->id]) }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-all duration-300 border border-gray-200 border-r-4 border-r-violet-400 group">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600">معلّقة: {{ $stats['pending_online_bookings'] ?? 0 }}</p>
                </div>
            </div>
        </a>
