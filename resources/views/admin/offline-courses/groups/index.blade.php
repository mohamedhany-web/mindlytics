...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
        ...
        <form action="{{ route('admin.offline-courses.groups.store', $offlineCourse) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="datetime-local" name="class_time" id="class_time" value="{{ old('class_time') }}"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                @error('class_time') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-bold text-gray-900">المجموعات ({{ $groups->count() }})</h2>
        </div>
        @if($groups->count() > 0)
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @endphp
                                <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $statusClass[$group->status] ?? 'bg-gray-100 text-gray-800' }}">
                                    {{ $group->status === 'active' ? 'نشط' : ($group->status === 'completed' ? 'منتهي' : 'ملغي') }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <button type="button" onclick="openEditModal({{ json_encode($group) }})" class="text-yellow-600 hover:text-yellow-800 font-medium ml-2">تعديل</button>
