    ...
    <section class="rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-sky-900 text-white shadow-xl overflow-hidden">
        <div class="px-6 py-8 sm:px-10 border-b border-white/10">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('admin.accounting.gateway-operations') }}" class="inline-flex items-center gap-2 rounded-2xl bg-indigo-500 px-4 py-2.5 text-sm font-semibold text-white shadow-lg hover:bg-indigo-400">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-px bg-white/10 border-t border-white/10">
            <div class="bg-slate-900/50 p-4 lg:col-span-2">
                ...
                <p class="text-lg font-bold text-indigo-200">{{ number_format($snapshot['gateway_online_month_gross'] ?? 0, 2) }} <span class="text-xs text-white/45">ج.م</span></p>
            </div>
            <div class="bg-slate-900/50 p-4 lg:col-span-2">
                <p class="text-[11px] text-white/55">عمولات بوابات الدفع (الشهر)</p>
                <p class="text-lg font-bold text-amber-200">{{ number_format($snapshot['gateway_fees_month'] ?? 0, 2) }} <span class="text-xs text-white/45">ج.م</span></p>
                <a href="{{ route('admin.accounting.gateway-operations') }}" class="mt-2 inline-flex text-[11px] font-semibold text-white/80 hover:text-white underline">سجل العمليات والربط المحاسبي</a>
            </div>
        </div>
        @endif
    </section>
