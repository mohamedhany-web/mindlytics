...
<div class="space-y-6">
        ...
        <div class="px-4 py-4 bg-slate-50 border-b border-slate-200 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-slate-500 mt-1">{{ $snapshot['month_label'] ?? '' }}</p>
                </div>
            </div>
    ...
    <section class="rounded-2xl border border-amber-200 bg-amber-50 shadow-lg overflow-hidden">
        <div class="px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            ...
            <p class="text-xs font-semibold text-amber-800">{{ implode(' · ', $pendingAlerts) }}</p>
        </div>
    </section>
    @endif
    ...
    {{-- مؤشرات ثانوية --}}
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        @foreach($secondaryStats as $card)
            @php $theme = $cardThemes[$card['theme']] ?? $cardThemes['sky']; @endphp
            <div class="dashboard-stat-card rounded-2xl border-2 {{ $theme['border'] }} bg-gradient-to-br {{ $theme['bg'] }} p-5 shadow-lg">
                <div class="flex items-center justify-between gap-3 mb-3">
                    <div class="min-w-0 flex-1">
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-4 py-3 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            ...
            <a href="{{ route('admin.accounting.gateway-operations') }}" class="inline-flex items-center gap-1 text-xs font-bold text-indigo-700 hover:text-indigo-900">
                سجل العمليات والربط المحاسبي
                <i class="fas fa-arrow-left text-[10px]"></i>
            </a>
        </div>
        <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            @foreach($gatewayStats as $card)
                @php $theme = $cardThemes[$card['theme']] ?? $cardThemes['indigo']; @endphp
                <div class="dashboard-stat-card rounded-2xl border-2 {{ $theme['border'] }} bg-gradient-to-br {{ $theme['bg'] }} p-5 shadow-md">
                    <div class="flex items-center justify-between gap-3">
                        <div class="min-w-0 flex-1">
                            <p class="text-sm font-bold {{ $theme['label'] }} mb-1">{{ $card['label'] }}</p>
                            <p class="text-2xl font-black bg-gradient-to-r {{ $theme['value'] }} bg-clip-text text-transparent tabular-nums">{{ $card['value'] }}</p>
                            <p class="text-xs font-medium {{ $theme['desc'] }} mt-1">{{ $card['desc'] }}</p>
                        </div>
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        ...
        <div class="p-4 flex flex-wrap gap-2">
            ...
            <a href="{{ route('admin.accounting.installments') }}" class="inline-flex items-center gap-2 px-3 py-2 text-xs font-bold text-violet-800 rounded-xl border border-violet-200 bg-violet-50 hover:bg-violet-100">
                <i class="fas fa-percentage"></i>
                لوحة التقسيط
            </a>
            @endif
            @if(Route::has('admin.accounting.gateway-operations'))
            <a href="{{ route('admin.accounting.gateway-operations') }}" class="inline-flex items-center gap-2 px-3 py-2 text-xs font-bold text-indigo-800 rounded-xl border border-indigo-200 bg-indigo-50 hover:bg-indigo-100">
                <i class="fas fa-plug"></i>
                بوابات الدفع
            </a>
            @endif
            <a href="{{ route('admin.accounting.chart') }}" class="inline-flex items-center gap-2 px-3 py-2 text-xs font-bold text-slate-800 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100">
                <i class="fas fa-sitemap"></i>
                شجرة الحسابات
            </a>
            <a href="{{ route('admin.invoices.index') }}" class="inline-flex items-center gap-2 px-3 py-2 text-xs font-bold text-amber-800 rounded-xl border border-amber-200 bg-amber-50 hover:bg-amber-100">
                <i class="fas fa-file-invoice"></i>
                الفواتير
            </a>
            <a href="{{ route('admin.payments.index') }}" class="inline-flex items-center gap-2 px-3 py-2 text-xs font-bold text-emerald-800 rounded-xl border border-emerald-200 bg-emerald-50 hover:bg-emerald-100">
                <i class="fas fa-credit-card"></i>
                المدفوعات
            </a>
            <a href="{{ route('admin.expenses.index') }}" class="inline-flex items-center gap-2 px-3 py-2 text-xs font-bold text-rose-800 rounded-xl border border-rose-200 bg-rose-50 hover:bg-rose-100">
                <i class="fas fa-receipt"></i>
                المصروفات
            </a>
            @if(Route::has('admin.withdrawals.index'))
            <a href="{{ route('admin.withdrawals.index') }}" class="inline-flex items-center gap-2 px-3 py-2 text-xs font-bold text-sky-800 rounded-xl border border-sky-200 bg-sky-50 hover:bg-sky-100">
    ...
    @foreach($visibleSections as $section)
        ...
        <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
            <div class="px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center gap-3">
                <div class="w-9 h-9 rounded-xl bg-gradient-to-br {{ $sectionTheme['header'] }} flex items-center justify-center text-white shadow-sm">
                    <i class="fas {{ $section['icon'] ?? 'fa-folder-open' }} text-sm"></i>
                </div>
                <div>
                    <h2 class="text-base font-black text-slate-900">{{ $section['title'] }}</h2>
                    <p class="text-[11px] text-slate-500">{{ count($section['items']) }} رابط</p>
                </div>
            </div>
            <div class="p-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                    @foreach($section['items'] as $item)
                        <a href="{{ route($item['route']) }}"
                           class="group flex items-start gap-3 rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50/40 p-4 transition-all {{ $sectionTheme['hover'] }} hover:shadow-md">
                            <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br {{ $sectionTheme['link'] }} text-white shadow-sm">
                                <i class="fas {{ $item['icon'] }} text-sm"></i>
                            </span>
                            <span class="min-w-0 flex-1">
                                <span class="block text-sm font-bold text-slate-900 group-hover:text-slate-800">{{ $item['label'] }}</span>
                                @if(!empty($item['hint']))
