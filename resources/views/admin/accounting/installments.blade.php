    ...
    <section class="rounded-3xl bg-gradient-to-br from-violet-900 via-slate-900 to-sky-900 text-white shadow-xl overflow-hidden">
        <div class="px-6 py-8 sm:px-10 border-b border-white/10">
            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                ...
                <div class="flex flex-wrap gap-2.5">
                    ...
                    <a href="{{ route('admin.accounting.chart') }}" class="inline-flex items-center gap-2 rounded-2xl bg-white/10 px-4 py-2.5 text-sm font-semibold border border-white/20 hover:bg-white/20">
                        <i class="fas fa-sitemap"></i>
                        شجرة الحسابات
                    </a>
                    <a href="{{ route('admin.installments.plans.index') }}" class="inline-flex items-center gap-2 rounded-2xl bg-white/10 px-4 py-2.5 text-sm font-semibold border border-white/20 hover:bg-white/20">
                        <i class="fas fa-layer-group"></i>
                        خطط التقسيط
                    </a>
                    <a href="{{ route('admin.installments.agreements.index') }}" class="inline-flex items-center gap-2 rounded-2xl bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-white shadow-lg hover:bg-emerald-400">
                        <i class="fas fa-handshake"></i>
                        اتفاقيات التقسيط
                    </a>
                    <a href="{{ route('admin.installments.agreements.manual-booking') }}" class="inline-flex items-center gap-2 rounded-2xl bg-amber-400 px-4 py-2.5 text-sm font-semibold text-slate-900 shadow-lg hover:bg-amber-300">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-px bg-white/10">
            <div class="bg-slate-900/80 p-4 sm:p-5">
                ...
                <p class="mt-1 text-2xl font-black text-white">{{ number_format($stats['agreements_total']) }}</p>
            </div>
            <div class="bg-slate-900/80 p-4 sm:p-5">
                <p class="text-[11px] font-semibold text-white/55">نشطة</p>
                <p class="mt-1 text-2xl font-black text-emerald-300">{{ number_format($stats['agreements_active']) }}</p>
            </div>
            <div class="bg-slate-900/80 p-4 sm:p-5">
                <p class="text-[11px] font-semibold text-white/55">متأخرة (اتفاقية)</p>
                <p class="mt-1 text-2xl font-black text-amber-300">{{ number_format($stats['agreements_overdue']) }}</p>
            </div>
            <div class="bg-slate-900/80 p-4 sm:p-5">
                <p class="text-[11px] font-semibold text-white/55">مكتملة</p>
                <p class="mt-1 text-2xl font-black text-violet-200">{{ number_format($stats['agreements_completed']) }}</p>
            </div>
            <div class="bg-slate-900/80 p-4 sm:p-5">
                <p class="text-[11px] font-semibold text-white/55">أونلاين</p>
                <p class="mt-1 text-2xl font-black text-sky-200">{{ number_format($stats['online_linked']) }}</p>
            </div>
            <div class="bg-slate-900/80 p-4 sm:p-5">
                <p class="text-[11px] font-semibold text-white/55">أوفلاين</p>
                <p class="mt-1 text-2xl font-black text-teal-200">{{ number_format($stats['offline_linked']) }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-px bg-white/10 border-t border-white/10">
            <div class="bg-slate-900/60 p-4">
                ...
                <p class="text-lg font-bold text-white">{{ number_format($stats['total_financed'], 2) }} <span class="text-xs text-white/50">ج.م</span></p>
            </div>
            <div class="bg-slate-900/60 p-4">
                <p class="text-[11px] text-white/55">أقساط معلقة (مجموع)</p>
                <p class="text-lg font-bold text-violet-200">{{ number_format($stats['pending_installments_sum'], 2) }}</p>
                <p class="text-[10px] text-white/45">{{ number_format($stats['pending_count']) }} قسط</p>
            </div>
            <div class="bg-slate-900/60 p-4">
                <p class="text-[11px] text-white/55">أقساط متأخرة (مجموع)</p>
                <p class="text-lg font-bold text-rose-200">{{ number_format($stats['overdue_installments_sum'], 2) }}</p>
                <p class="text-[10px] text-white/45">{{ number_format($stats['overdue_payments_count']) }} قسط</p>
            </div>
            <div class="bg-slate-900/60 p-4">
                <p class="text-[11px] text-white/55">محصّل أقساط الشهر</p>
                <p class="text-lg font-bold text-emerald-200">{{ number_format($stats['paid_installments_month'], 2) }}</p>
                <p class="text-[10px] text-white/45">{{ $stats['month_label'] }} — {{ number_format($stats['paid_count_month']) }} عملية</p>
            </div>
        </div>
    </section>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <section class="rounded-3xl bg-white border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 bg-gradient-to-l from-violet-50 to-white flex flex-wrap items-center justify-between gap-3">
                ...
                <a href="{{ route('admin.installments.agreements.index') }}" class="text-sm font-semibold text-violet-600 hover:text-violet-800">كل الاتفاقيات ←</a>
            </div>
                ...
                @forelse($upcomingDues as $p)
                    ...
                    <div class="px-4 py-3 flex flex-wrap items-center gap-3 hover:bg-slate-50/80">
                        ...
                        <div class="text-left sm:text-right">
                            <p class="text-sm font-bold text-slate-800">{{ number_format((float) $p->amount, 2) }} ج.م</p>
        ...
        <section class="rounded-3xl bg-white border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 bg-gradient-to-l from-emerald-50 to-white flex flex-wrap items-center justify-between gap-3">
                ...
                <a href="{{ route('admin.payments.index') }}" class="text-sm font-semibold text-emerald-600 hover:text-emerald-800">المدفوعات ←</a>
            </div>
            <div class="divide-y divide-slate-100 max-h-[520px] overflow-y-auto">
                @forelse($recentPayments as $p)
                    ...
                    <div class="px-4 py-3 flex flex-wrap items-center gap-3 hover:bg-slate-50/80">
                        ...
                        <div class="text-left sm:text-right">
                            <p class="text-sm font-bold text-emerald-700">+{{ number_format((float) $p->amount, 2) }} ج.م</p>
