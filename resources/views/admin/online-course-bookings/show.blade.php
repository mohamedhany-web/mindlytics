...
@section('title', 'طلب حجز أونلاين #' . $offlineCourseBooking->id)
@section('header', 'طلب حجز أونلاين #' . $offlineCourseBooking->id)
...
<div class="space-y-6 w-full">
    ...
    <div class="flex flex-wrap gap-3">
        <a href="{{ route('admin.online-course-bookings.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-800 rounded-lg text-sm hover:bg-gray-200"><i class="fas fa-arrow-right ml-2"></i> القائمة</a>
        <a href="{{ route('admin.offline-courses.show', $offlineCourseBooking->course) }}" class="inline-flex items-center px-4 py-2 bg-purple-50 text-purple-800 rounded-lg text-sm hover:bg-purple-100">صفحة الكورس</a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 space-y-4">
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
            <div><dt class="text-gray-500">رقم الطلب</dt><dd class="font-semibold text-gray-900">#{{ $offlineCourseBooking->id }}</dd></div>
            ...
            <div class="lg:col-span-2"><dt class="text-gray-500">الكورس</dt><dd class="font-semibold text-gray-900">{{ $offlineCourseBooking->course->title }}</dd></div>
            ...
            @if($offlineCourseBooking->requestedGroup)
                <div class="lg:col-span-2"><dt class="text-gray-500">المجموعة المطلوبة (أونلاين)</dt><dd class="font-semibold text-purple-800">{{ $offlineCourseBooking->requestedGroup->name }}</dd></div>
            @endif
            ...
            <div><dt class="text-gray-500">الاسم المحوِّل</dt><dd class="text-gray-800">{{ $offlineCourseBooking->transfer_name ?: '—' }}</dd></div>
            ...
            @if($offlineCourseBooking->reviewed_at)
                <div><dt class="text-gray-500">تاريخ المراجعة</dt><dd class="text-gray-800">{{ $offlineCourseBooking->reviewed_at?->format('Y-m-d H:i') }}</dd></div>
            @endif
            @if($offlineCourseBooking->assignedGroup)
                <div><dt class="text-gray-500">المجموعة المعتمدة</dt><dd class="font-semibold text-emerald-700">{{ $offlineCourseBooking->assignedGroup->name }}</dd></div>
            @endif
        </dl>
        ...
        @if($offlineCourseBooking->payment_proof)
            <div class="border-t border-gray-100 pt-4">
                ...
                <div class="space-y-3">
                    ...
                    <img src="{{ asset('storage/' . $offlineCourseBooking->payment_proof) }}" alt="إيصال التحويل" class="max-w-full lg:max-w-3xl rounded-lg border border-gray-200 shadow-sm">
                </div>
            </div>
        @endif
    ...
    @if($offlineCourseBooking->isPending())
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 space-y-4">
            ...
            <form action="{{ route('admin.online-course-bookings.approve', $offlineCourseBooking) }}" method="post" class="space-y-4">
                @csrf
                <div>
                    ...
                    <select name="group_id" required class="w-full max-w-md rounded-lg border-gray-300">
                        ...
                        @foreach($groups as $g)
                            <option value="{{ $g->id }}" @disabled(! $g->canEnroll('online'))>
                                {{ $g->name }} — متاح أونلاين {{ $g->availableSeats('online') }} / {{ $g->max_students_online }}
                                @if(! $g->canEnroll('online')) (غير متاحة) @endif
                            </option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-red-100 p-6">
            ...
            <form action="{{ route('admin.online-course-bookings.reject', $offlineCourseBooking) }}" method="post" class="space-y-3">
                @csrf
