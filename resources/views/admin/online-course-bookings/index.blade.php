...
@section('title', 'طلبات حجز الكورسات الأونلاين')
@section('header', 'طلبات حجز الكورسات الأونلاين')
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
        <form method="get" class="flex flex-wrap gap-4 items-end">
            ...
            <a href="{{ route('admin.online-course-bookings.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm">إعادة تعيين</a>
        </form>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($bookings as $b)
                        <tr class="hover:bg-slate-50/80">
                            ...
                            <td class="px-4 py-3 text-gray-600 text-xs">{{ $b->requestedGroup->name ?? '—' }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $b->payment_method === 'wallet' ? 'محفظة' : 'تحويل' }}</td>
                            <td class="px-4 py-3">
                                @if($b->status === 'pending')
                            ...
                            <td class="px-4 py-3">
                                <a href="{{ route('admin.online-course-bookings.show', $b) }}" class="text-blue-600 hover:text-blue-800 font-medium">تفاصيل</a>
                            </td>
                        </tr>
                    @empty
