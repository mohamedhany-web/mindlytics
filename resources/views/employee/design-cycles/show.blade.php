...
@section('title', 'دورة تصميم #'.$designCycle->id)
@section('header', 'دورة تصميم #'.$designCycle->id)
...
<div class="w-full max-w-none space-y-8">
    ...
    <div class="flex flex-wrap items-center gap-2">
        <a href="{{ route('employee.design-cycles.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-sm font-semibold text-slate-800">
            <i class="fas fa-arrow-right"></i> القائمة
        </a>
        @if($designCycle->designerTask)
            <a href="{{ route('employee.tasks.show', $designCycle->designerTask) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-sky-100 hover:bg-sky-200 text-sm font-semibold text-sky-900">
                <i class="fas fa-paint-brush"></i> مهمة المصمم
            </a>
        @endif
        @if($designCycle->moderatorDeliveryTask)
            <a href="{{ route('employee.tasks.show', $designCycle->moderatorDeliveryTask) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-100 hover:bg-emerald-200 text-sm font-semibold text-emerald-900">
                <i class="fas fa-upload"></i> مهمة تسليمك النهائي
            </a>
        @endif
        <a href="{{ route('employee.tasks.index', ['task_type' => 'design_moderator_delivery']) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-sm font-semibold text-indigo-900">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                @foreach([
                    ...
                    <div class="relative rounded-xl border-2 px-3 py-3 text-center transition
                        {{ $step === $n ? 'border-fuchsia-500 bg-fuchsia-50 shadow-md' : ($step > $n ? 'border-emerald-200 bg-emerald-50/50' : 'border-gray-100 bg-gray-50/80 opacity-80') }}">
                        <div class="text-lg mb-1 {{ $step >= $n ? 'text-fuchsia-700' : 'text-gray-400' }}">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm space-y-4 w-full">
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
            <div class="rounded-xl bg-slate-50 px-4 py-3 border border-slate-100">
                ...
                <dd class="font-bold text-gray-900">{{ $designCycle->designer->name ?? '—' }}</dd>
            </div>
            <div class="rounded-xl bg-slate-50 px-4 py-3 border border-slate-100">
                <dt class="text-gray-500 text-xs font-semibold mb-1">حد تسليم المصمم</dt>
                <dd class="font-bold text-gray-900">{{ $designCycle->deadline_at?->format('Y-m-d H:i') }}</dd>
            </div>
            <div class="rounded-xl bg-slate-50 px-4 py-3 border border-slate-100">
                <dt class="text-gray-500 text-xs font-semibold mb-1">الأولوية</dt>
                <dd class="font-bold text-gray-900">{{ $designCycle->priority }}</dd>
            </div>
            <div class="rounded-xl bg-slate-50 px-4 py-3 border border-slate-100">
                <dt class="text-gray-500 text-xs font-semibold mb-1">تسليم المصمم (أول مرة)</dt>
                <dd class="font-bold text-gray-900">{{ $designCycle->designer_submitted_at?->format('Y-m-d H:i') ?? '—' }}</dd>
            </div>
        </dl>
        @if($designCycle->description)
            <div>
                <p class="text-xs font-semibold text-gray-500 mb-1">الوصف</p>
                <p class="text-gray-800 whitespace-pre-wrap text-sm">{{ $designCycle->description }}</p>
            </div>
        @endif
        <div>
            <p class="text-xs font-semibold text-gray-500 mb-1">مواصفات التصميم</p>
            <p class="text-gray-800 whitespace-pre-wrap text-sm leading-relaxed">{{ $designCycle->specifications }}</p>
        </div>
    </div>

    {{-- جدول التسليمات الموحّد (مصمم + مشرف) --}}
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden w-full">
        <div class="px-5 py-4 border-b border-gray-100 flex flex-wrap items-center justify-between gap-2 bg-gradient-to-l from-violet-50 to-fuchsia-50">
            ...
            <span class="text-xs font-bold px-3 py-1 rounded-full bg-white/80 border border-fuchsia-100 text-fuchsia-800">{{ $deliverablesTimeline->count() }} تسليم</span>
        </div>
                        ...
                        @foreach($deliverablesTimeline as $row)
                            ...
                            <tr class="hover:bg-fuchsia-50/30 align-top">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    ...
                                    @endphp
                                    <span class="inline-flex px-2 py-0.5 rounded-md text-xs font-bold border {{ $typeClass }}">
                                        {{ EmployeeTaskDeliverable::deliveryTypeLabel($type) }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    @if($type === 'link' && $d->link_url)
                                        <a href="{{ $d->link_url }}" target="_blank" rel="noopener" class="inline-flex items-center gap-1 text-blue-600 hover:text-blue-800 font-semibold text-xs break-all">
                                            <i class="fas fa-external-link-alt shrink-0"></i> فتح الرابط
                                        </a>
                                    @elseif(in_array($type, ['file', 'image'], true) && $d->publicFileUrl())
                                        @if($type === 'image')
                                            <a href="{{ $d->publicFileUrl() }}" target="_blank" rel="noopener" class="block">
                                                <img src="{{ $d->publicFileUrl() }}" alt="" class="max-h-28 rounded-lg border border-gray-200 shadow-sm object-contain bg-gray-50">
                                            </a>
                                        @endif
                                        <a href="{{ $d->publicFileUrl() }}" target="_blank" rel="noopener" class="inline-flex items-center gap-1 mt-2 text-violet-700 hover:text-violet-900 font-semibold text-xs">
                                            <i class="fas fa-download"></i> تحميل @if($d->file_name) ({{ $d->file_name }}) @endif
                                        </a>
                                    @else
                                        <span class="text-gray-400 text-xs">—</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-xs font-mono text-gray-700 whitespace-nowrap">{{ $fmtBytes($d->file_size) }}</td>
                                <td class="px-4 py-3 text-xs text-gray-600 whitespace-nowrap">{{ $d->created_at?->format('Y-m-d H:i') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    <div class="rounded-2xl border border-indigo-200 bg-white shadow-sm overflow-hidden w-full">
        ...
        <div class="p-4 sm:p-5 space-y-4">
            @if($designCycle->moderatorPlannerItems->isEmpty())
            ...
            @else
                <div class="space-y-4">
                    @foreach($designCycle->moderatorPlannerItems as $item)
                        <form method="post" action="{{ route('employee.design-cycles.planner-items.update', [$designCycle, $item]) }}" class="rounded-xl border border-gray-200 bg-gray-50/50 p-4 space-y-3">
                            @csrf
                            @method('PATCH')
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-start">
                                <div class="md:col-span-3">
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">المهمة</label>
                                    <input type="text" name="title" value="{{ old('title', $item->title) }}" required class="w-full rounded-lg border-gray-200 text-sm py-2 px-3">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">القسم / الجهة</label>
                                    <input type="text" name="department" value="{{ old('department', $item->department) }}" placeholder="مثال: سوشيال، طباعة، QA"
                                           class="w-full rounded-lg border-gray-200 text-sm py-2 px-3" list="dept-suggestions-{{ $designCycle->id }}">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">فترة اليوم</label>
                                    <input type="text" name="time_slot" value="{{ old('time_slot', $item->time_slot) }}" placeholder="صباحاً، ظهراً، مساءً"
                                           class="w-full rounded-lg border-gray-200 text-sm py-2 px-3" list="slot-suggestions-{{ $designCycle->id }}">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">موعد</label>
                                    <input type="date" name="due_date" value="{{ old('due_date', $item->due_date?->format('Y-m-d')) }}"
                            ...
                            <div>
                                ...
                                <input type="text" name="notes" value="{{ old('notes', $item->notes) }}" placeholder="تفاصيل إضافية، رابط، اسم مسؤول..."
                                       class="w-full rounded-lg border-gray-200 text-sm py-2 px-3">
                            </div>
                        </form>
                        <div class="flex justify-end -mt-2 mb-2">
                            <form method="post" action="{{ route('employee.design-cycles.planner-items.destroy', [$designCycle, $item]) }}" onsubmit="return confirm('حذف هذا البند؟');" class="inline">
                                @csrf
                                @method('DELETE')
            ...
            <datalist id="dept-suggestions-{{ $designCycle->id }}">
            ...
            <datalist id="slot-suggestions-{{ $designCycle->id }}">
            ...
            <div class="rounded-xl border-2 border-dashed border-indigo-200 bg-indigo-50/30 p-4">
                ...
                <form method="post" action="{{ route('employee.design-cycles.planner-items.store', $designCycle) }}" class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                    @csrf
                    <div class="md:col-span-4">
                        <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">المهمة <span class="text-red-500">*</span></label>
                        <input type="text" name="title" value="{{ old('title') }}" required placeholder="مثال: مراجعة الملف قبل الإرسال"
                               class="w-full rounded-lg border-gray-200 text-sm py-2 px-3">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">القسم</label>
                        <input type="text" name="department" value="{{ old('department') }}" placeholder="قسم" class="w-full rounded-lg border-gray-200 text-sm py-2 px-3" list="dept-suggestions-{{ $designCycle->id }}">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">فترة اليوم</label>
                        <input type="text" name="time_slot" value="{{ old('time_slot') }}" placeholder="فترة" class="w-full rounded-lg border-gray-200 text-sm py-2 px-3" list="slot-suggestions-{{ $designCycle->id }}">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">موعد</label>
                        <input type="date" name="due_date" value="{{ old('due_date') }}" class="w-full rounded-lg border-gray-200 text-sm py-2 px-3">
                    </div>
                    <div class="md:col-span-2">
                    ...
                    <div class="md:col-span-12">
                        <input type="text" name="notes" value="{{ old('notes') }}" placeholder="ملاحظات (اختياري)" class="w-full rounded-lg border-gray-200 text-sm py-2 px-3">
                    </div>
                </form>
            </div>
        </div>
    ...
    @if($designCycle->status === DesignTaskCycle::STATUS_DESIGN_SUBMITTED && ! $designCycle->moderator_delivery_task_id)
        <div class="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-6 shadow-sm space-y-4 w-full">
            ...
            <form method="post" action="{{ route('employee.design-cycles.moderator-delivery.store', $designCycle) }}" class="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl">
                @csrf
                <div class="md:col-span-2">
    ...
    @if(! in_array($designCycle->status, [DesignTaskCycle::STATUS_COMPLETED, DesignTaskCycle::STATUS_CANCELLED], true))
        <form method="post" action="{{ route('employee.design-cycles.cancel', $designCycle) }}" onsubmit="return confirm('إلغاء هذا الطلب؟');" class="inline">
            @csrf
