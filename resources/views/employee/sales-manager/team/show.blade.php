...
<div class="space-y-5 pb-10">
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-4 sm:px-5 py-4 bg-gradient-to-l from-slate-50 via-white to-indigo-50/40 border-b border-slate-200 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="min-w-0">
                    ...
                    <div class="flex flex-wrap items-center gap-2 mt-2">
                        ...
                        <span class="inline-flex items-center rounded-md px-2 py-0.5 text-xs font-bold border
                            {{ !empty($attendanceState['unlock']) ? 'bg-teal-50 border-teal-200 text-teal-800' : 'bg-slate-50 border-slate-200 text-slate-600' }}">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('employee.sales-manager.leads.index', ['assignee' => $employee->id]) }}"
                   class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-slate-700 rounded-lg border border-slate-300 bg-white hover:bg-slate-50">
        ...
        <div class="grid grid-cols-2 xl:grid-cols-5 gap-3 p-4">
            @foreach([
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm">
                    ...
                    <p class="text-xl font-black tabular-nums {{ $card['text'] }}">{{ $card['value'] }}</p>
                </div>
            @endforeach
        </div>
    </section>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-5 items-start">
        ...
        <div class="xl:col-span-7 space-y-5">
                    ...
                    @if($schedule)
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div class="rounded-xl border border-indigo-100 bg-indigo-50/50 px-4 py-3">
                                ...
                                <p class="text-base font-black text-slate-900">{{ $schedule->name }}</p>
                            </div>
                            <div class="rounded-xl border border-slate-200 bg-slate-50/60 px-4 py-3">
                                ...
                                <p class="text-base font-black text-slate-900 tabular-nums">{{ $schedule->timeRangeLabel() }}</p>
                            </div>
                            <div class="rounded-xl border border-slate-200 bg-slate-50/60 px-4 py-3">
                                <p class="text-[11px] font-bold text-slate-500 mb-1">الساعات المطلوبة</p>
                                <p class="text-base font-black text-slate-900 tabular-nums">{{ $schedule->required_hours ?? '—' }} ساعة</p>
                            </div>
                            <div class="rounded-xl border border-slate-200 bg-slate-50/60 px-4 py-3">
                                <p class="text-[11px] font-bold text-slate-500 mb-1">سماح التأخير</p>
                                <p class="text-base font-black text-slate-900 tabular-nums">{{ $schedule->grace_minutes ?? 0 }} دقيقة</p>
                            </div>
                            <div class="rounded-xl border border-amber-100 bg-amber-50/60 px-4 py-3 sm:col-span-2">
                                ...
                                <p class="text-base font-black text-slate-900">
                                    {{ $employee->weeklyOffDayLabel() ?? 'غير محدد (سبت/أحد افتراضياً)' }}
                                </p>
                            </div>
                        </div>
                        @if($schedule->description)
                            <p class="text-xs text-slate-600 mt-3 leading-relaxed">{{ $schedule->description }}</p>
                        @endif
                    @else
                        <div class="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/50 px-4 py-8 text-center">
                            ...
                            <p class="text-xs text-slate-500 mt-1">يوم الراحة: {{ $employee->weeklyOffDayLabel() ?? 'غير محدد' }}</p>
                        </div>
                    @endif
                </div>
            </section>
            ...
            <section class="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-4 sm:px-5 py-3 border-b border-slate-100 bg-slate-50/80 flex flex-wrap items-center justify-between gap-2">
                    ...
                    <span class="text-[11px] font-semibold text-slate-500">{{ $leaves->count() }} سجل أخير</span>
                </div>
                ...
                <div class="p-4 sm:p-5 space-y-4">
                    @if($activeLeave)
                        <div class="rounded-xl border border-violet-200 bg-violet-50 px-4 py-3">
                            ...
                            <p class="text-xs text-violet-800 mt-1">
                                {{ $activeLeave->type_label }} ·
                                {{ $activeLeave->start_date->format('Y-m-d') }} → {{ $activeLeave->end_date->format('Y-m-d') }}
                                ({{ $activeLeave->days }} يوم)
                            </p>
                            @if($activeLeave->reason)
                                <p class="text-xs text-slate-600 mt-1">{{ $activeLeave->reason }}</p>
                            @endif
                        </div>
                    @endif
                    ...
                    @if($pendingLeaves->isNotEmpty())
                        <div>
                            <p class="text-[11px] font-bold text-amber-800 mb-2">طلبات قيد المراجعة ({{ $pendingLeaves->count() }})</p>
                            <ul class="space-y-2">
                                @foreach($pendingLeaves as $leave)
                                    <li class="rounded-lg border border-amber-200 bg-amber-50/50 px-3 py-2 text-sm flex flex-wrap justify-between gap-2">
                                        <span>
                                            <strong>{{ $leave->type_label }}</strong>
                                            · {{ $leave->start_date->format('Y-m-d') }} → {{ $leave->end_date->format('Y-m-d') }}
                                        </span>
                                        <span class="text-[10px] font-bold px-2 py-0.5 rounded-full border {{ $statusColors['pending'] }}">قيد المراجعة</span>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                                ...
                                @forelse($leaves as $leave)
                                    <tr>
                                        <td class="px-3 py-2.5">
                                            <p class="font-semibold text-slate-900">{{ $leave->type_label }}</p>
                                            @if($leave->reason)
                                                <p class="text-[11px] text-slate-500 max-w-[14rem] truncate" title="{{ $leave->reason }}">{{ $leave->reason }}</p>
                                            @endif
                                        </td>
                                        <td class="px-3 py-2.5 tabular-nums whitespace-nowrap">{{ $leave->start_date->format('Y-m-d') }}</td>
                                        <td class="px-3 py-2.5 tabular-nums whitespace-nowrap">{{ $leave->end_date->format('Y-m-d') }}</td>
                                        <td class="px-3 py-2.5 tabular-nums">{{ $leave->days }}</td>
                                        <td class="px-3 py-2.5">
                                            <span class="text-[10px] font-bold px-2 py-0.5 rounded-full border {{ $statusColors[$leave->status] ?? $statusColors['cancelled'] }}">
                                                {{ $leave->status_label }}
                                            </span>
                                        </td>
                                    </tr>
        ...
        {{-- Sidebar: profile + recent attendance --}}
        <aside class="xl:col-span-5 space-y-4 xl:sticky xl:top-4">
            <section class="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                ...
                <dl class="p-4 space-y-2.5 text-sm">
                    <div class="rounded-lg border border-slate-200 bg-slate-50/50 px-3 py-2.5 flex justify-between gap-2">
                        ...
                        <dd class="font-semibold text-slate-900 text-left">{{ $employee->name }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/50 px-3 py-2.5 flex justify-between gap-2">
                        <dt class="text-slate-500 shrink-0">البريد</dt>
                        <dd class="font-semibold text-slate-900 text-left break-all">{{ $employee->email ?? '—' }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/50 px-3 py-2.5 flex justify-between gap-2">
                        <dt class="text-slate-500">الهاتف</dt>
                        <dd class="font-semibold text-slate-900">{{ $employee->phone ?? '—' }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/50 px-3 py-2.5 flex justify-between gap-2">
                        <dt class="text-slate-500">الوظيفة</dt>
                        <dd class="font-semibold text-slate-900">{{ $employee->employeeJob->name ?? '—' }}</dd>
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-slate-50/50 px-3 py-2.5 flex justify-between gap-2">
                        <dt class="text-slate-500">يوم الراحة</dt>
                        <dd class="font-semibold text-slate-900">{{ $employee->weeklyOffDayLabel() ?? '—' }}</dd>
                    </div>
                </dl>
            </section>
            ...
            <section class="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-4 py-3 border-b border-slate-100 bg-slate-50/80 flex items-center justify-between gap-2">
                    ...
                    <a href="{{ route('employee.sales-manager.attendance.employee', $employee) }}" class="text-[11px] font-bold text-violet-700 hover:underline">التفاصيل</a>
                </div>
                <ul class="divide-y divide-slate-100">
                    @forelse($recentAttendance as $rec)
                        <li class="px-4 py-2.5 flex items-center justify-between gap-2 text-sm">
                            <span class="font-medium text-slate-800 tabular-nums">{{ $rec->work_date?->format('Y-m-d') }}</span>
                            ...
                            <span class="text-[10px] font-bold text-slate-600">
                                {{ \App\Models\EmployeeAttendanceRecord::statusLabels()[$rec->status] ?? $rec->status }}
                            </span>
                        </li>
                    @empty
