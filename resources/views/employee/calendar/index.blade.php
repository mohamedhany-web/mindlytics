...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div class="bg-white rounded-xl p-5 border border-blue-100 shadow-sm">
            ...
            <p class="text-2xl font-black text-blue-700">{{ $stats['tasks'] }}</p>
        </div>

        <div class="bg-white rounded-xl p-5 border border-teal-100 shadow-sm">
            <p class="text-xs font-semibold text-slate-500 mb-1">
                {{ !empty($stats['is_sales_manager']) ? 'متابعات الفريق' : 'المتابعات' }}
            </p>
            <p class="text-2xl font-black text-teal-700">{{ $stats['followups'] ?? 0 }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl p-5 border border-emerald-100 shadow-sm">
            ...
            <p class="text-2xl font-black text-emerald-700">{{ $stats['leaves'] }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl p-5 border border-violet-100 shadow-sm">
            ...
            <p class="text-2xl font-black text-violet-700">{{ $stats['meetings'] }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl p-5 border border-rose-100 shadow-sm">
            ...
            <p class="text-2xl font-black text-rose-700">{{ $stats['upcoming'] }}</p>
        </div>
    </div>

    @if(($stats['followups'] ?? 0) > 0 || auth()->user()->isSalesStaff())
    <div class="flex flex-wrap items-center gap-3 text-xs text-slate-600">
        ...
        @if(!empty($stats['is_sales_manager']))
