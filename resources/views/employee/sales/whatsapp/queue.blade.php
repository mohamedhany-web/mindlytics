...
<div class="w-full space-y-4">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="shrink-0 flex items-center gap-3">
                <div class="text-center rounded-lg bg-white/80 border border-emerald-100 px-3 py-1.5">
                    <p class="text-xl sm:text-2xl font-black text-emerald-700 tabular-nums leading-none" id="wa-queue-total">{{ $conversations->total() }}</p>
                ...
                <div class="flex items-start gap-2.5 min-w-0">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-1.5 min-w-0">
                            <h3 class="font-bold text-sm text-slate-900 truncate" title="{{ $conversation->displayName() }}">
                        ...
                        <p class="text-[11px] text-slate-500 tabular-nums dir-ltr truncate mt-0.5">{{ $conversation->formattedPhone() }}</p>
                    </div>
                </div>

                <p class="text-xs text-slate-600 line-clamp-2 leading-relaxed min-h-[2.25rem]" title="{{ $conversation->last_message_preview ?: '' }}">
    ...
    @if($conversations->hasPages())
        <div class="pt-1 flex justify-center">{{ $conversations->links() }}</div>
    @endif
</div>
@endsection
