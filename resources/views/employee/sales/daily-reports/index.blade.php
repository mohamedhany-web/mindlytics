...
<div class="space-y-6">
    @if(session('success'))
        <div class="rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 text-sm font-semibold">{{ session('success') }}</div>
    @endif
    @if(session('error'))
        <div class="rounded-xl bg-rose-50 border border-rose-200 text-rose-800 px-4 py-3 text-sm font-semibold">{{ session('error') }}</div>
    @endif
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <a href="{{ route('employee.sales.daily-reports.edit', ['date' => $date->toDateString()]) }}"
           class="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-sm">
            ...
            {{ $report?->isSubmitted() ? 'عرض التقرير' : 'تعبئة / تعديل التقرير' }}
        </a>
    </div>
