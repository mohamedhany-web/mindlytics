        ...
        <aside class="lg:col-span-4 space-y-4 order-2 lg:sticky lg:top-4">
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 sm:p-6 space-y-4">
                ...
                <dl class="space-y-3 text-sm border-t border-gray-100 pt-4">
                    ...
                    <div>
                        <dt class="text-gray-500 mb-1">الإجراء التالي (Next Action)</dt>
                        <dd class="font-semibold text-gray-900">{{ $lead->follow_up_channel ? (\App\Models\SalesLead::FOLLOW_UP_CHANNELS[$lead->follow_up_channel] ?? $lead->follow_up_channel) : '—' }}</dd>
                    </div>
                    <div>
                ...
                @if($lead->isOpen())
                    <div class="rounded-xl border border-teal-200 bg-teal-50/70 p-4 space-y-3">
                        ...
                        <form method="post" action="{{ route('employee.sales.leads.next-follow', $lead) }}" class="space-y-2">
                            @csrf
                            @error('next_follow_up_at')<p class="text-xs text-rose-600">{{ $message }}</p>@enderror
                            <div>
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 mb-1">الإجراء التالي (Next Action)</label>
                                <select name="follow_up_channel" required class="w-full border border-teal-200 rounded-lg px-3 py-2 text-sm bg-white">
                                ...
                                @error('follow_up_channel')<p class="text-xs text-rose-600">{{ $message }}</p>@enderror
                            </div>
                            <div>
                ...
                @endif
                @if($lead->isOpen() && app(\App\Services\SalesLeadMovementPolicy::class)->requiresActiveMovement($lead->stage))
                    @php
                        $missingFollow = ! $lead->next_follow_up_at || $lead->next_follow_up_at->isPast();
                        $missingAction = blank($lead->follow_up_channel);
                    @endphp
                    @if($missingFollow || $missingAction || $lead->isStaleContact())
                        <div class="rounded-xl border border-rose-200 bg-rose-50/80 p-3 text-sm text-rose-900 space-y-1">
                            @if($missingFollow || $missingAction)
                                <p class="font-bold"><i class="fas fa-ban ml-1"></i> Lead سايب — ممنوع بدون Status + Next Action + موعد متابعة.</p>
                            @endif
                            @if($lead->isFollowUpOverdue())
                                <p class="font-semibold"><i class="fas fa-clock ml-1"></i> موعد المتابعة متأخر — حدّد موعداً جديداً الآن.</p>
                            @endif
                            @if($missingAction)
                                <p class="font-semibold"><i class="fas fa-bolt ml-1"></i> Next Action ناقص — اختر اتصال / واتساب / اجتماع.</p>
                            @endif
                            @if($lead->isStaleContact())
                                <p class="font-semibold"><i class="fas fa-hourglass-end ml-1"></i> لم يُسجَّل تواصل خلال {{ \App\Models\SalesLead::STALE_CONTACT_DAYS }} أيام — سجّل نشاطاً.</p>
                            @endif
                        </div>
                    @endif
                @endif
