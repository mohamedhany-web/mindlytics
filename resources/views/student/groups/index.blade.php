    ...
    <header class="sg-chrome sg-reveal">
        <div>
            <nav class="sg-crumb" aria-label="مسار التنقل">
                ...
                <span style="color:var(--ml-ink);font-weight:700">{{ __('student.my_groups_title') }}</span>
            </nav>
            <h1>{{ __('student.my_groups_title') }}</h1>
            <p class="sub">{{ __('student.my_groups_subtitle') }}</p>
        </div>
        <div class="sg-signals" aria-label="ملخص">
            <span class="sg-signal sg-signal-live">{{ $groupsCount }} {{ __('student.my_groups') }}</span>
        </div>
    </header>
    ...
    @if($groupsCount > 0)
        <section class="sg-stage sg-reveal" style="--reveal-delay:50ms" aria-label="مجموعتك">
            <div>
                ...
                <p class="sg-copy">
                    {{ $firstGroup->course->title ?? 'مجموعة تعلّم' }}
                    · {{ $firstGroup->members->count() }} {{ __('student.member_singular') }}
                    @if($firstGroup->leader)
                        · {{ __('student.leader_label') }}: {{ $firstGroup->leader->name }}
                    @endif
                </p>
            </div>
            <div class="sg-stage-actions">
                <a class="sg-btn" href="{{ route('student.groups.show', $firstGroup) }}">
                    <i class="fas fa-comments text-xs"></i>
                    فتح المجموعة
                </a>
                <a class="sg-btn sg-btn-quiet" href="{{ route('dashboard') }}">العودة لمساحة التعلّم</a>
            </div>
        </section>

        <div class="sg-list sg-reveal" style="--reveal-delay:120ms" role="list">
            @foreach($groups as $group)
                <a href="{{ route('student.groups.show', $group) }}" class="sg-row" role="listitem">
                    <div class="sg-ico" aria-hidden="true"><i class="fas fa-user-friends"></i></div>
                    <div class="sg-body">
                        <h3>{{ $group->name }}</h3>
                        <p class="meta">{{ $group->course->title ?? '—' }}</p>
                        <div class="sg-tags">
                            <span class="sg-tag">
    ...
    @else
        <section class="sg-stage sg-reveal" style="--reveal-delay:50ms">
            <div>
                ...
                <h2>{{ __('student.no_groups') }}</h2>
                <p class="sg-copy">{{ __('student.no_groups_desc') }}</p>
            </div>
            <div class="sg-stage-actions">
                <a class="sg-btn" href="{{ route('dashboard') }}">مساحة التعلّم</a>
            </div>
        </section>
    @endif
</div>
