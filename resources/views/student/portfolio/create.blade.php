...
@section('title', __('student.pf_add_project_btn'))
@section('header', __('student.pf_add_project_btn'))
...
<div class="space-y-5 max-w-5xl">
    <nav class="flex flex-wrap items-center gap-2 text-sm font-bold text-[var(--sp-muted)]">
        ...
        <span class="text-[var(--sp-text)]">{{ __('student.pf_add_project_btn') }}</span>
    </nav>
    ...
    <section class="sp-pf-hero">
        <div class="relative z-[1]">
            ...
            <h2 class="text-xl sm:text-2xl font-extrabold m-0">{{ __('student.pf_create_title') }}</h2>
            <p class="text-sm text-white/70 m-0 mt-2 max-w-2xl leading-relaxed">{{ __('student.pf_create_subtitle') }}</p>
        </div>
    </section>

    <section class="sp-card overflow-hidden">
        <form action="{{ route('student.portfolio.store') }}" method="POST" enctype="multipart/form-data" class="p-5 sm:p-6 md:p-8">
            @csrf
