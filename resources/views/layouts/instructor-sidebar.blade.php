<div class="flex flex-col h-full bg-white">
    ...
    <nav class="flex-1 overflow-y-auto sidebar-scroll p-2 md:p-3 space-y-1">
        ...
        @if($isInstructor || $user->hasAnyPermission('instructor.view.courses', 'instructor.manage.lectures', 'instructor.manage.groups', 'instructor.manage.assignments', 'instructor.manage.exams', 'instructor.manage.attendance', 'instructor.view.tasks'))
            ...
            <a href="{{ route('instructor.learn-discussions.index') }}"
               @click="if (window.innerWidth < 1024) sidebarOpen = false"
               class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors {{ request()->routeIs('instructor.learn-discussions.*') ? 'bg-teal-50 border border-teal-200' : 'hover:bg-slate-50 border border-transparent' }}">
            ...
            <a href="{{ route('instructor.offline-courses.index') }}" 
               @click="if (window.innerWidth < 1024) sidebarOpen = false"
               class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors {{ request()->routeIs('instructor.offline-courses.*') && request('channel', 'offline') === 'offline' ? 'bg-amber-50 border border-amber-200' : 'hover:bg-slate-50 border border-transparent' }}">
            ...
            <a href="{{ route('instructor.online-group-courses.index') }}"
               @click="if (window.innerWidth < 1024) sidebarOpen = false"
               class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors {{ request()->routeIs('instructor.online-group-courses.*') ? 'bg-indigo-50 border border-indigo-200' : 'hover:bg-slate-50 border border-transparent' }}">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs text-slate-500 mt-0.5">{{ $myOnlineStudentsCount }} طالب نشط</div>
                </div>
                <i class="fas fa-chevron-left text-slate-400 text-xs"></i>
            </a>
            @endif
            ...
            <a href="{{ route('instructor.calendar') }}" @click="if (window.innerWidth < 1024) sidebarOpen = false"
               class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors {{ request()->routeIs('instructor.calendar*') ? 'bg-sky-50 border border-sky-200' : 'hover:bg-slate-50 border border-transparent' }}">
