...
@section('title', 'كورسات أونلاين — الفرع')
@section('header', 'كورسات أونلاين — ' . $branch->name)
...
<div class="p-6 lg:p-8 max-w-[1400px] mx-auto space-y-6">
    <form method="GET" class="flex flex-wrap gap-3 items-end">
        <div>
            ...
            <input type="text" name="q" value="{{ request('q') }}" placeholder="عنوان الكورس"
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($courses as $c)
                        <tr>
                            ...
                            <td class="px-4 py-3 font-medium">{{ $c->localized('title') }}</td>
                            <td class="px-4 py-3">{{ $c->instructor->name ?? '—' }}</td>
