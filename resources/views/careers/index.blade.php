...
<section class="py-12 md:py-16 bg-gradient-to-b from-slate-50 to-white">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
        ...
        <div class="flex items-center gap-3 mb-8">
            ...
            @if($jobs->count() > 0)
                <span class="text-sm font-bold text-blue-600 bg-blue-50 border border-blue-100 px-3 py-1 rounded-full">{{ $jobs->count() }} وظيفة</span>
            @endif
        </div>

        <div class="grid sm:grid-cols-2 gap-6">
            @forelse($jobs as $job)
                <a href="{{ route('careers.show', $job) }}" class="careers-card bg-white rounded-2xl shadow-md p-6 block no-underline text-inherit group">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0 flex-1">
                            <h3 class="text-lg font-black text-slate-900 group-hover:text-blue-700 transition-colors truncate">{{ $job->title }}</h3>
                            <div class="flex flex-wrap gap-2 mt-3">
                                @if($job->department)
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 text-xs font-bold border border-blue-100">
                                        <i class="fas fa-building text-[10px]"></i>{{ $job->department }}
                                    </span>
                                @endif
                                @if($job->location)
                    ...
                    @if($job->description)
                        <p class="text-sm text-slate-600 mt-4 line-clamp-3 leading-relaxed">{{ \Illuminate\Support\Str::limit(strip_tags((string) $job->description), 160) }}</p>
                    @endif
            ...
            @empty
                <div class="sm:col-span-2 rounded-2xl border-2 border-dashed border-slate-200 bg-white p-10 text-center">
                    ...
                    <a href="{{ route('public.contact') }}" class="inline-flex items-center gap-2 mt-5 px-5 py-2.5 rounded-xl border-2 border-blue-200 bg-blue-50 text-blue-700 text-sm font-bold hover:bg-blue-100 transition-colors">
