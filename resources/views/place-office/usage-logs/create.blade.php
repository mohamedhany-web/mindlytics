...
@section('title', 'تسجيل يومي')
@section('header', 'تسجيل يومي — ' . $location->name)
...
<div class="space-y-6"
     x-data="placeDailyForm({
        usageType: @json($defaultUsageType),
        hours: @json(old('hours', '')),
        hourlyRate: {{ $hourlyRate }},
        expenses: @json($oldExpenses)
     })">
        ...
        <div class="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-gray-600 mt-1 font-medium">{{ $location->name }} — فترة {{ $period }}</p>
            ...
            <a href="{{ route('place.office.usage-logs.index') }}"
    ...
    @if(!$settlementOpen)
        <div class="rounded-xl border-2 border-amber-300 bg-amber-50 p-4 text-amber-900">
            ...
            <p class="text-sm mt-1">مخالصة شهر {{ $period }} غير مفتوحة حالياً. قد لا يُقبل الحفظ حتى تفتح الإدارة الفترة.</p>
        </div>
    @endif
        ...
        <div class="dashboard-card rounded-2xl p-5 border-2 border-blue-200/50 shadow-xl">
            <div class="relative z-10 flex items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-2xl sm:text-3xl font-black text-blue-700 tabular-nums">{{ number_format((float) $approvedHoursThisMonth, 2) }}</p>
                </div>
        ...
        <div class="dashboard-card rounded-2xl p-5 border-2 border-yellow-200/50 shadow-xl">
            <div class="relative z-10 flex items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-2xl sm:text-3xl font-black text-yellow-700 tabular-nums">{{ $pendingLogs + $pendingExpenses }}</p>
                </div>
            ...
            <div class="relative z-10 flex items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-xl sm:text-2xl font-black text-green-700 tabular-nums">
                        @if($hourlyRate > 0){{ number_format($hourlyRate, 2) }} ج.م@else — @endif
                    </p>
                </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div class="xl:col-span-8 space-y-6">
            <form action="{{ route('place.office.usage-logs.store') }}" method="POST" class="space-y-6">
                @csrf

                {{-- نوع النشاط — دائماً ظاهر --}}
                <div class="dashboard-card rounded-2xl p-6 shadow-xl">
                    <div class="relative z-10 space-y-5">
                        ...
                        {{-- يُرسل دائماً مع النموذج حتى بدون Alpine --}}
                        <input type="hidden" name="usage_type" :value="usageType" value="{{ $defaultUsageType }}">

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach($usageTypes as $value => $label)
                                <button type="button"
                                        @click="usageType = '{{ $value }}'"
                                        class="type-card text-right w-full"
                                        :class="usageType === '{{ $value }}' ? 'active' : ''">
                                    <div class="flex items-center gap-3">
                                        <div class="w-11 h-11 rounded-xl flex items-center justify-center shrink-0"
                                             :class="usageType === '{{ $value }}' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'">
                                            <i class="fas {{ $value === 'course' ? 'fa-chalkboard-teacher' : 'fa-tasks' }} text-lg"></i>
                                        </div>
                                        ...
                                        <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0"
                                             :class="usageType === '{{ $value }}' ? 'border-blue-600 bg-blue-600' : 'border-slate-300'">
                                            <i class="fas fa-check text-white text-[9px]" x-show="usageType === '{{ $value }}'"></i>
                                        </div>
                                    </div>
                                </button>
                            @endforeach
                        </div>
                        @error('usage_type')<p class="text-red-500 text-sm font-medium">{{ $message }}</p>@enderror
                        ...
                        <div x-show="usageType === 'course'" x-transition class="pt-4 border-t border-slate-200/80">
                            ...
                            <select name="offline_course_id"
                                ...
                                @foreach($courses as $course)
                                    <option value="{{ $course->id }}" @selected(old('offline_course_id') == $course->id)>{{ $course->title }}</option>
                                @endforeach
                            </select>
                            @if($courses->isEmpty())
                            ...
                            @endif
                            @error('offline_course_id')<p class="text-red-500 text-sm mt-1 font-medium">{{ $message }}</p>@enderror
                        </div>
                    </div>
                </div>
                    ...
                    <div class="relative z-10 space-y-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <input type="date" name="usage_date" value="{{ old('usage_date', now()->toDateString()) }}" max="{{ now()->toDateString() }}" required
                                ...
                                @error('usage_date')<p class="text-red-500 text-sm mt-1 font-medium">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                ...
                                @error('hours')<p class="text-red-500 text-sm mt-1 font-medium">{{ $message }}</p>@enderror
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3" placeholder="مثال: محاضرة الوحدة الثالثة — قاعة A"
                                      class="form-input w-full rounded-xl border-2 border-gray-200 px-4 py-3 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20">{{ old('description') }}</textarea>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-start p-3 sm:p-4 rounded-xl bg-white/80 border-2 border-slate-100">
                                ...
                                <div class="sm:col-span-3">
                                    ...
                                    <select :name="'expenses['+index+'][category]'" x-model="row.category"
                                            class="form-input w-full rounded-lg border-2 border-gray-200 px-3 py-2.5 text-sm">
                                        @foreach($expenseCategories as $key => $catLabel)
        ...
        <div class="xl:col-span-4 space-y-4">
            ...
            @if($currentSettlement)
                <div class="bg-white rounded-2xl border-2 border-blue-200/60 p-5 shadow-lg">
                    <h4 class="text-sm font-black text-slate-800 mb-2">مخالصة {{ $period }}</h4>
                    <p class="text-lg font-black text-blue-700">{{ $currentSettlement->status_label }}</p>
                    <a href="{{ route('place.office.settlements.index') }}" class="inline-flex items-center gap-1 text-sm font-semibold text-blue-600 hover:underline mt-3">
            ...
            <div class="rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 p-5 text-white shadow-lg">
                ...
                <p class="text-lg font-black mt-1">{{ $user->name }}</p>
                <p class="text-xs opacity-80 mt-2" dir="ltr">{{ $user->email }}</p>
            </div>
        </div>
    </div>
</div>
