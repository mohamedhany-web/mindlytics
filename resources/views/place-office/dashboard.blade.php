...
@section('title', 'لوحة المكان')
@section('header', 'لوحة المكان')
...
<div class="space-y-6">
    @if($mustCloseSoon)
        <div class="rounded-xl border-2 border-amber-300 bg-amber-50 p-4 text-amber-900">
            <p class="font-bold"><i class="fas fa-exclamation-triangle ml-2"></i>تذكير: يتبقى {{ $daysLeftInMonth }} يوم على نهاية الشهر</p>
            <p class="text-sm mt-1">يجب إرسال المخالصة الشهرية وإقفال حساب {{ $period }} قبل نهاية الشهر.</p>
        </div>
    @endif
        ...
        <div class="relative z-10">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-gray-600 text-base sm:text-lg font-medium">لوحة إدارة المكان — {{ $location->name }}</p>
                    ...
                    @if($location->hourly_rate)
                        <p class="text-sm text-blue-700 font-semibold mt-2">سعر الساعة: {{ number_format((float) $location->hourly_rate, 2) }} ج.م</p>
                    @endif
                </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
        <div class="dashboard-card rounded-2xl p-5 sm:p-6 border-2 border-blue-200/50 shadow-xl">
            <div class="relative z-10 flex items-center justify-between">
                <div>
                    <p class="text-sm font-semibold text-gray-600 mb-1">ساعات معتمدة — {{ $period }}</p>
                    <p class="text-3xl font-black text-blue-700">{{ number_format((float) $approvedHoursThisMonth, 2) }}</p>
                </div>
        ...
        <div class="dashboard-card rounded-2xl p-5 sm:p-6 border-2 border-yellow-200/50 shadow-xl">
            <div class="relative z-10 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-black text-yellow-700">{{ $pendingLogs }}</p>
                </div>
        ...
        <div class="dashboard-card rounded-2xl p-5 sm:p-6 border-2 border-green-200/50 shadow-xl">
            <div class="relative z-10 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-black text-green-700">{{ $currentSettlement?->status_label ?? 'لم تُفتح' }}</p>
                </div>
    ...
    <div class="flex flex-wrap gap-3">
        <a href="{{ route('place.office.usage-logs.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold transition-colors">
            <i class="fas fa-plus"></i> تسجيل ساعات
        </a>
        <a href="{{ route('place.office.settlements.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white border-2 border-blue-200 text-blue-800 text-sm font-semibold hover:bg-blue-50 transition-colors">
            <i class="fas fa-file-invoice"></i> المخالصة الشهرية
        </a>
        <a href="{{ route('place.office.invoices.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white border-2 border-slate-200 text-slate-700 text-sm font-semibold hover:bg-slate-50 transition-colors">
