    ...
    <div class="p-4 sm:p-5 space-y-3">
        <div class="flex flex-wrap items-center justify-between gap-2">
            ...
            <div class="flex flex-wrap gap-1.5 text-[11px]">
                <button type="button"
                        onclick="openCurriculumLectureModal('{{ $section->id }}')"
                ...
                @if(isset($resources) && $resources->isNotEmpty())
                    <button type="button"
                            ...
                            onclick="toggleCurriculumAttach('{{ $section->id }}','resource')">
                ...
                @if(isset($activities) && $activities->isNotEmpty())
                    <button type="button"
                            ...
                            onclick="toggleCurriculumAttach('{{ $section->id }}','activity')">
                ...
                @if(isset($exams) && $exams->isNotEmpty())
                    <button type="button"
                            ...
                            onclick="toggleCurriculumAttach('{{ $section->id }}','exam')">
        ...
        {{-- فورمات الربط داخل القسم --}}
        @php $attachUrl = $attachUrl ?? route('instructor.offline-courses.curriculum.attach-item', $offlineCourse); @endphp

        @if(isset($lectures) && $lectures->isNotEmpty())
            <div id="attach-lecture-{{ $section->id }}" class="hidden rounded-xl border border-violet-100 bg-violet-50/40 p-3 space-y-2">
                <form action="{{ $attachUrl }}" method="POST" class="space-y-2">
                    @csrf
                    <input type="hidden" name="offline_course_section_id" value="{{ $section->id }}">
        ...
        @if(isset($resources) && $resources->isNotEmpty())
            <div id="attach-resource-{{ $section->id }}" class="hidden rounded-xl border border-sky-100 bg-sky-50/40 p-3 space-y-2">
                <form action="{{ $attachUrl }}" method="POST" class="space-y-2">
                    @csrf
                    <input type="hidden" name="offline_course_section_id" value="{{ $section->id }}">
        ...
        @if(isset($activities) && $activities->isNotEmpty())
            <div id="attach-activity-{{ $section->id }}" class="hidden rounded-xl border border-amber-100 bg-amber-50/40 p-3 space-y-2">
                <form action="{{ $attachUrl }}" method="POST" class="space-y-2">
                    @csrf
                    <input type="hidden" name="offline_course_section_id" value="{{ $section->id }}">
        ...
        @if(isset($exams) && $exams->isNotEmpty())
            <div id="attach-exam-{{ $section->id }}" class="hidden rounded-xl border border-emerald-100 bg-emerald-50/40 p-3 space-y-2">
                <form action="{{ $attachUrl }}" method="POST" class="space-y-2">
                    @csrf
                    <input type="hidden" name="offline_course_section_id" value="{{ $section->id }}">
        ...
        {{-- نافذة إنشاء محاضرة — نفس نمط بطاقات المنصة (حدود slate، رأس رمادي فاتح) --}}
        <div id="lecture-modal-{{ $section->id }}" class="fixed inset-0 z-[80] hidden lecture-curriculum-modal" role="dialog" aria-modal="true" aria-labelledby="lecture-modal-title-{{ $section->id }}">
            <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" onclick="closeCurriculumLectureModal('{{ $section->id }}')"></div>
            <div class="absolute inset-0 flex items-center justify-center p-4 pointer-events-none">
                <div class="pointer-events-auto w-full max-w-3xl max-h-[90vh] flex flex-col rounded-2xl shadow-2xl border border-slate-200 bg-white overflow-hidden">
                    <div class="shrink-0 flex items-start justify-between gap-4 px-5 py-4 border-b border-slate-200 bg-slate-50">
                        <div class="min-w-0">
                            <h4 id="lecture-modal-title-{{ $section->id }}" class="text-lg font-bold text-slate-800">محاضرة جديدة</h4>
                            <p class="text-sm text-slate-500 mt-1">القسم: <span class="font-semibold text-slate-700">{{ $section->title }}</span> — اربط الجلسة ثم املأ التفاصيل.</p>
                        </div>
                        <button type="button" onclick="closeCurriculumLectureModal('{{ $section->id }}')" class="p-2 rounded-xl text-slate-500 hover:bg-slate-200 hover:text-slate-800 shrink-0 transition-colors" aria-label="إغلاق">
                            <i class="fas fa-times text-xl"></i>
                        </button>
                    </div>
                    <div class="flex-1 min-h-0 overflow-y-auto p-5 sm:p-6 bg-white">
                        <form action="{{ route('instructor.offline-courses.lectures.store', ['offlineCourse' => $offlineCourse, 'channel' => $curriculumChannel]) }}"
                              method="post"
                              enctype="multipart/form-data"
                              class="space-y-5 text-sm">
                            @csrf
                            <input type="hidden" name="curriculum_section_id" value="{{ $section->id }}">
                            ...
                            @include('instructor.offline-courses.lectures.partials.offline-mindmap-field', ['variant' => 'modal', 'value' => null])
                            <div class="rounded-xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm space-y-4">
                            ...
                            <div class="flex flex-wrap justify-end gap-2 pt-2 border-t border-slate-100">
                                <button type="button" onclick="closeCurriculumLectureModal('{{ $section->id }}')" class="px-4 py-2.5 rounded-xl border border-slate-200 text-slate-700 text-sm font-semibold hover:bg-slate-50">إلغاء</button>
        ...
        @forelse($section->items as $cItem)
            @php
                ...
                $agendaLines = $isOfflineLecture && filled($m->session_agenda)
                    ...
                    : [];
            @endphp
            <div class="flex flex-wrap items-start gap-2 p-3 rounded-xl border {{ $isOfflineLecture ? 'border-violet-100 bg-violet-50/20' : 'border-slate-100 bg-slate-50/50' }}">
                <span class="text-[10px] font-bold uppercase px-2 py-0.5 rounded-md shrink-0 {{ $badgeClass }}">{{ $badgeLabel }}</span>
                <div class="flex-1 min-w-0 space-y-1">
                    <div class="flex flex-wrap items-center gap-2">
                        <span class="text-sm font-semibold text-slate-800">{{ $label }}</span>
                        @if($isOfflineLecture)
                            <a href="{{ route('instructor.offline-courses.lectures.edit', ['offlineCourse' => $offlineCourse, 'lecture' => $m, 'channel' => $curriculumChannel]) }}"
                    ...
                    @if($isOfflineLecture)
                        @if($m->relationLoaded('groupSession') && $m->groupSession)
                            <p class="text-[11px] font-semibold text-violet-800">
                                ...
                                جلسة: {{ $m->groupSession->session_date->format('Y/m/d') }}
                                @php $gst = $m->groupSession->start_time; @endphp
                                {{ is_string($gst) ? substr($gst, 0, 5) : $gst }}
                                @if($m->groupSession->group) — {{ $m->groupSession->group->name }} @endif
                            </p>
                        @endif
                        @if($m->scheduled_at)
                            <p class="text-[11px] text-slate-500"><i class="far fa-clock ml-1"></i>{{ $m->scheduled_at->translatedFormat('l j F Y — H:i') }}</p>
                        @endif
                        @if(count($agendaLines))
                            <ul class="text-[11px] text-slate-600 list-disc list-inside space-y-0.5">
                                @foreach(array_slice($agendaLines, 0, 6) as $line)
                                    <li>{{ \Illuminate\Support\Str::limit($line, 140) }}</li>
                                @endforeach
                            </ul>
                            @if(count($agendaLines) > 6)
                                <p class="text-[10px] text-slate-400">+ {{ count($agendaLines) - 6 }} نقطة أخرى</p>
                            @endif
                        @endif
                        @if($m->description)
                            <p class="text-[11px] text-slate-600 line-clamp-2 whitespace-pre-line">{{ $m->description }}</p>
                        @endif
                        <div class="flex flex-wrap gap-1.5 pt-0.5 items-center">
                            @if($m->hasPlayableRecording())
                                <button type="button"
                                        class="js-open-recording-modal inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-bold bg-rose-600 text-white hover:bg-rose-700 shadow-sm transition"
                                        data-watch-url="{{ route('instructor.offline-courses.lectures.watch', [$offlineCourse, $m]) }}"
                                        data-title="{{ $m->title }}">
                            ...
                            @if(!empty($m->attachments))
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-sky-50 text-sky-700 border border-sky-100"><i class="fas fa-paperclip ml-1"></i> {{ count($m->attachments) }} مرفق</span>
                            @endif
                            @if(!empty($m->download_links))
                            ...
                            @endif
                            @if(filled($m->offline_attendee_mindmap))
    ...
    @if($section->children && $section->children->isNotEmpty())
        <div class="border-t border-slate-100 p-3 space-y-3 bg-white">
            @foreach($section->children as $child)
                @include('instructor.offline-curriculum.partials.section-block', ['section' => $child, 'depth' => ($depth ?? 0) + 1, 'offlineCourse' => $offlineCourse, 'curriculumChannel' => $curriculumChannel, 'groupSessions' => $groupSessions])
            @endforeach
        </div>
    @endif
</div>
