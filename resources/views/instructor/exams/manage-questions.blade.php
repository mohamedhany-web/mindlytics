    ...
    <div class="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col" @click.stop>
        ...
        <div class="p-6 overflow-y-auto flex-1">
            ...
            <div class="space-y-3 max-h-80 overflow-y-auto">
                @foreach($availableQuestions as $question)
                    <form action="{{ route('instructor.exams.questions.add-from-bank', $exam) }}" method="POST" class="flex items-center gap-3 p-4 rounded-xl border border-slate-200 hover:border-sky-200 hover:bg-sky-50/30 transition-colors">
                        @csrf
                        <input type="hidden" name="question_id" value="{{ $question->id }}">
                        ...
                        <input type="number" name="marks" value="{{ $question->points ?? 1 }}" min="0.5" step="0.5" class="w-16 px-2 py-1.5 border border-slate-200 rounded-lg text-sm text-center">
