...
<div class="w-full max-w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-5 sm:p-6">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <a href="{{ route('instructor.offline-courses.activities.create', ['offlineCourse' => $offlineCourse, 'channel' => $channel]) }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-amber-600 text-white rounded-xl font-semibold hover:bg-amber-700">
    ...
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        @if($activities->isEmpty())
            <div class="p-12 text-center text-slate-500">
                ...
                <a href="{{ route('instructor.offline-courses.activities.create', ['offlineCourse' => $offlineCourse, 'channel' => $channel]) }}" class="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-amber-600 text-white rounded-xl font-semibold hover:bg-amber-700">إضافة نشاط</a>
            </div>
        @else
                ...
                @foreach($activities as $activity)
                    <li class="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-50/50">
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0">
                            <a href="{{ route('instructor.offline-courses.activities.show', ['offlineCourse' => $offlineCourse, 'activity' => $activity, 'channel' => $channel]) }}" class="px-3 py-1.5 text-sm bg-amber-100 text-amber-700 rounded-lg hover:bg-amber-200">عرض والتقديمات</a>
                            <a href="{{ route('instructor.offline-courses.activities.edit', ['offlineCourse' => $offlineCourse, 'activity' => $activity, 'channel' => $channel]) }}" class="px-3 py-1.5 text-sm bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200">تعديل</a>
                            <form action="{{ route('instructor.offline-courses.activities.destroy', ['offlineCourse' => $offlineCourse, 'activity' => $activity, 'channel' => $channel]) }}" method="post" class="inline" onsubmit="return confirm('حذف هذا النشاط؟');">
                                @csrf
                                @method('DELETE')
