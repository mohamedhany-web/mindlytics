...
<div class="w-full max-w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-6">
        <form action="{{ route('instructor.offline-courses.activities.store', ['offlineCourse' => $offlineCourse, 'channel' => $channel]) }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="space-y-4">
                <div>
            ...
            <div class="mt-6 flex gap-3">
                ...
                <a href="{{ route('instructor.offline-courses.activities.index', ['offlineCourse' => $offlineCourse, 'channel' => $channel]) }}" class="px-4 py-2.5 bg-slate-100 text-slate-700 rounded-xl font-semibold hover:bg-slate-200">إلغاء</a>
            </div>
        </form>
    </div>
</div>
