...
@section('title', 'موارد الكورس ' . (($channel ?? 'offline') === 'online' ? 'الأونلاين' : 'الأوفلاين') . ' - ' . $offlineCourse->title)
@section('header', 'موارد الكورس ' . (($channel ?? 'offline') === 'online' ? 'الأونلاين' : 'الأوفلاين'))
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="min-w-0 flex-1">
                <h1 class="text-xl sm:text-2xl font-bold text-slate-800 flex flex-wrap items-center gap-2">
                    ...
                    موارد الكورس ({{ ($channel ?? 'offline') === 'online' ? 'أونلاين' : 'أوفلاين' }})
                </h1>
                <p class="mt-1 text-sm font-semibold text-slate-600 truncate" title="{{ $offlineCourse->title }}">الكورس: {{ $offlineCourse->title }}</p>
                <p class="mt-0.5 text-xs text-slate-500">كل الموارد هنا مرتبطة بهذا الكورس فقط.</p>
            </div>
            <a href="{{ route('instructor.offline-courses.resources.create', ['offlineCourse' => $offlineCourse, 'channel' => ($channel ?? 'offline')]) }}" class="inline-flex shrink-0 items-center gap-2 px-4 py-2.5 bg-sky-600 text-white rounded-xl font-semibold hover:bg-sky-700 transition-colors">
    ...
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        @if($resources->isEmpty())
            <div class="p-12 text-center text-slate-500">
                ...
                <a href="{{ route('instructor.offline-courses.resources.create', ['offlineCourse' => $offlineCourse, 'channel' => ($channel ?? 'offline')]) }}" class="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-sky-600 text-white rounded-xl font-semibold hover:bg-sky-700">إضافة مورد لهذا الكورس</a>
            </div>
        @else
                ...
                @foreach($resources as $resource)
                    <li class="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-50/50">
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0">
                            ...
                            @endif
                            <a href="{{ route('instructor.offline-courses.resources.edit', ['offlineCourse' => $offlineCourse, 'resource' => $resource, 'channel' => ($channel ?? 'offline')]) }}" class="px-3 py-1.5 text-sm bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200">تعديل</a>
                            <form action="{{ route('instructor.offline-courses.resources.destroy', ['offlineCourse' => $offlineCourse, 'resource' => $resource, 'channel' => ($channel ?? 'offline')]) }}" method="post" class="inline" onsubmit="return confirm('حذف هذا المورد؟');">
                                @csrf
                                @method('DELETE')
