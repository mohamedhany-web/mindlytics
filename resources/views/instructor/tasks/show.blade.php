...
@section('title', $task->title . ' - المهام من الإدارة')
        ...
        <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-5 sm:p-6">
                    ...
                    @foreach($task->deliverables as $d)
                        <li class="flex flex-wrap items-start justify-between gap-3 p-4 bg-slate-50 rounded-xl border border-slate-100">
                            <div>
                                ...
                                <p class="text-xs text-slate-500 mt-2">
                                    ...
                                    @if($d->delivery_type === 'link' && $d->link_url)
                                        · <a href="{{ $d->link_url }}" target="_blank" class="text-sky-600 hover:underline">فتح الرابط</a>
                                    @endif
                                    @if($d->file_path)
                                        · <a href="{{ Storage::url($d->file_path) }}" target="_blank" class="text-sky-600 hover:underline">تحميل الملف</a>
                                    @endif
                                </p>
                                @if($d->feedback)
                                    <p class="text-sm mt-2 p-2 bg-amber-50 rounded-lg text-amber-800 border border-amber-100"><strong>ملاحظات الإدارة:</strong> {{ $d->feedback }}</p>
                                @endif
                            </div>
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold
                                ...
                                @elseif($d->status === 'needs_revision') يحتاج مراجعة
                                @else مقدّم
                                @endif
                            </span>
                        </li>
            ...
            <form action="{{ route('instructor.tasks.submit-deliverable', $task) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="2" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-slate-800 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20" placeholder="وصف مختصر للتسليم">{{ old('description') }}</textarea>
                </div>
