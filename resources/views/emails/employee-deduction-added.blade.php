...
<body>
    <div class="box">
        ...
        <p>مرحباً {{ $deduction->employee->name ?? 'الموظف' }}،</p>
        <p>تم تسجيل خصم جديد على راتبك من إدارة المنصة.</p>

        <div class="card">
            <div class="label">رقم الخصم</div>
            <div class="value">{{ $deduction->deduction_number }}</div>
        </div>
        <div class="card">
            <div class="label">عنوان الخصم</div>
            <div class="value">{{ $deduction->title }}</div>
        </div>
        @if($deduction->description)
            <div class="card">
                <div class="label">الوصف</div>
                <div class="value" style="font-weight: normal;">{{ $deduction->description }}</div>
            </div>
        @endif
        <div class="card">
        ...
        <div class="card">
            ...
            <div class="amount">{{ number_format($deduction->amount, 2) }} ج.م</div>
        </div>
        <div class="card">
            <div class="label">تاريخ الخصم</div>
            <div class="value">{{ $deduction->deduction_date?->format('Y-m-d') }}</div>
        </div>
        <div class="card">
            <div class="label">الحالة</div>
            <div class="value">
                @if($deduction->status === 'pending') معلقة
                @elseif($deduction->status === 'applied') مطبقة
                @else ملغاة
                @endif
            </div>
        </div>

        <a href="{{ url(route('employee.accounting.index')) }}" class="btn">عرض المحاسبة والاتفاقيات</a>
