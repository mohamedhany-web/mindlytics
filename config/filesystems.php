...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | قرص تسجيلات محاضرات الأوفلاين/الأونلاين (فيديو المدرب → Cloudflare R2)
    |--------------------------------------------------------------------------
    | يُفضّل r2. اربط AWS_URL بدومين CDN عام مثل https://cdn.yourdomain.com
    */
    'offline_lecture_recordings_disk' => env('FILESYSTEM_DISK_OFFLINE_LECTURE_RECORDINGS', 'r2'),

    /*
    |--------------------------------------------------------------------------
    | الحد الأقصى لحجم فيديو المحاضرة (ميجابايت)
    |--------------------------------------------------------------------------
    */
    'offline_lecture_recording_max_mb' => (int) env('OFFLINE_LECTURE_RECORDING_MAX_MB', 2048),

    /*
    |--------------------------------------------------------------------------
    | رفع مباشر من المتصفح إلى R2 (Presigned PUT)
    |--------------------------------------------------------------------------
    | يحتاج CORS على الـ bucket. إن كان false يُرفع عبر سيرفر المنصة (أوثق محلياً).
    */
    'offline_lecture_recording_direct_upload' => filter_var(
        env('OFFLINE_LECTURE_RECORDING_DIRECT_UPLOAD', false),
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | قرص صور ريفيوهات الكورسات (التسويق)
    |--------------------------------------------------------------------------
    | افتراضياً Cloudflare R2. للتطوير المحلي بدون AWS_*: public
    */
    'course_reviews_disk' => env('FILESYSTEM_DISK_COURSE_REVIEWS', 'r2'),
